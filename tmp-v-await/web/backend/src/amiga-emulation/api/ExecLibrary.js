...
class ExecLibrary {
    constructor(emulator) {
        ...
        // Semaphore tracking
        this.publicSemaphores = new Map(); // name -> address
     ...
     */
    setDoorMessageCallback(callback) {
     ...
     */
    setLibraryOpenedCallback(callback) {
        this.onLibraryOpened = callback;
    }
    openLibrary(nameAddr, version) {
    ...
    */
    stackSwap(structAddr) {
        console.log(`[ExecLibrary] StackSwap(struct=0x${structAddr.toString(16)})`);
        const callerPC = this.emulator.getRegister(MoiraEmulator_1.CPURegister.PC);
