...
class IconLibrary {
    constructor(emulator, bbsRoot = '/Users/spot/Code/amiexpress-web') {
        this.diskObjects = new Map();
        this.nextDiskObjectAddr = 0x60000; // DiskObjects at 384KB
     ...
     */
    GetDiskObject() {
        const namePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A0);
        ...
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, diskObjAddr);
    }
    /**
    ...
    PutDiskObject() {
        const namePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A0);
        const diskObjPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A1);
     ...
     */
    FreeDiskObject() {
        const diskObjPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A0);
     ...
     */
    FindToolType() {
        const toolTypeArrayPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A0);
        const typeNamePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A1);
        ...
        while (true) {
            ...
            if (upperTooltypeStr.startsWith(typeName)) {
                ...
                if (tooltypeStr.length === typeName.length || tooltypeStr[typeName.length] === '=') {
                    ...
                    this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, tooltypeStrPtr);
                    return;
                }
            }
            arrayIndex++;
     ...
     */
    MatchToolValue() {
        const typeStringPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.A0);
     ...
     */
    createFakeDiskObject(toolTypes) {
    ...
    parseInfoFile(infoPath) {
        try {
            ...
            const output = (0, child_process_1.execSync)(`strings "${infoPath}"`, { encoding: 'utf-8' });
