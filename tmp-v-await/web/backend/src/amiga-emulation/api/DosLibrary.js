...
const MoiraEmulator_1 = require("../cpu/MoiraEmulator");
...
class DosLibrary {
    constructor(emulator) {
        ...
        this.useNewFileSystem = false; // Feature flag for gradual migration
        ...
        this.ERROR_SEEK_ERROR = 219; // Seek not possible (console/device)
        ...
        this.argStringPtr = 0; // Pointer to argument string
        ...
        this.inheritedInput = 1; // STDIN_HANDLE
        this.inheritedOutput = 2; // STDOUT_HANDLE
     ...
     */
    enableNewFileSystem(baseDir) {
        console.log('[dos.library] Enabling new file system with FileManager/PathManager');
        this.pathManager = new PathManager_1.PathManager(baseDir);
        this.fileManager = new FileManager_1.FileManager(baseDir, this.pathManager);
     ...
     */
    Open() {
        const namePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const mode = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
     ...
     */
    Close() {
        const handle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
    ...
    Read() {
        const handle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const bufferAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const length = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
     ...
     */
    setInheritedHandles(input, output) {
     ...
     */
    DateStamp() {
        const dateStampPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        // Get current time
        const now = new Date();
     ...
     */
    Delay() {
        const ticks = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
    ...
    Seek() {
        const handle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const rawOffset = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const rawMode = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
     ...
     */
    Rename() {
        const oldNamePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const newNamePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
     ...
     */
    UnLock() {
        const lockId = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
     ...
     */
    DupLock() {
        const lock = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
    ...
    Examine() {
        const lockId = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const fibPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
    ...
    Info() {
        const lock = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const infoPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
     ...
     */
    Exit() {
        const returnCode = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
     ...
     */
    UnLoadSeg() {
        const segList = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
    ...
    SetComment() {
        const namePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const commentPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const name = this.readString(namePtr);
    ...
    SetProtection() {
        const namePtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const protect = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const name = this.readString(namePtr);
    ...
    GetArgStr() {
        ...
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, this.argStringPtr);
    }
    /**
     ...
     */
    GetCliProgramName() {
        const bufPtr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const bufLen = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
     ...
     */
    setCliInfo(argStringPtr, programName) {
     ...
     */
    WaitForChar() {
        const fileHandle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const timeout = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        ...
        if (fileHandle === this.STDIN_HANDLE || fileHandle === this.STDOUT_HANDLE) {
            ...
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, hasData ? -1 : 0);
        ...
        if (!file) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // FALSE - invalid handle
            this.lastError = this.ERROR_OBJECT_NOT_FOUND;
        ...
        if (file.isConsole) {
            ...
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, atEOF ? 0 : -1);
    ...
    FGetC() {
        ...
        const file = this.openFiles.get(fileHandle);
        if (!file) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, -1); // EOF
            this.lastError = this.ERROR_OBJECT_NOT_FOUND;
        ...
        if (file.isConsole) {
            if (this.inputBuffer.length > 0) {
                ...
                this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, char);
    ...
    FPutC() {
        const fileHandle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const char = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2) & 0xFF;
    ...
    FGets() {
        const fileHandle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const bufAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const bufLen = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
        console.log(`[dos.library] FGets(fh=${fileHandle}, buf=0x${bufAddr.toString(16)}, len=${bufLen})`);
        if (bufLen === 0) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // NULL
        ...
        if (this.useNewFileSystem && this.fileManager) {
            ...
            const line = Buffer.from(Array.from({ length: bytesRead }, (_, i) => this.emulator.readMemory(bufAddr + i))).toString('latin1');
        ...
        this.emulator.writeMemory(bufAddr + bytesRead, 0);
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, bufAddr); // Return buffer pointer
        const line = String.fromCharCode(...Array.from({ length: bytesRead }, (_, i) => this.emulator.readMemory(bufAddr + i)));
    ...
    FPuts() {
        const fileHandle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const strAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const str = this.readString(strAddr);
        console.log(`[dos.library] FPuts(fh=${fileHandle}, str="${str}")`);
        const file = this.openFiles.get(fileHandle);
        if (!file) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, -1); // Error
            this.lastError = this.ERROR_OBJECT_NOT_FOUND;
        ...
        if (file.isConsole) {
            ...
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // Success
     ...
     */
    PutStr() {
        const strAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        if (!strAddr) {
    ...
    FRead() {
        const fileHandle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const bufAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const blockLen = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
        const numBlocks = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D4);
        console.log(`[dos.library] FRead(fh=${fileHandle}, buf=0x${bufAddr.toString(16)}, blocklen=${blockLen}, blocks=${numBlocks})`);
        const file = this.openFiles.get(fileHandle);
        if (!file) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // 0 blocks read
            this.lastError = this.ERROR_OBJECT_NOT_FOUND;
            console.log(`[dos.library] FRead: Invalid file handle`);
            return;
        }
        if (!file.buffer || file.position >= file.buffer.length) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // EOF
        ...
        for (let block = 0; block < numBlocks; block++) {
            ...
            for (let i = 0; i < bytesPerBlock; i++) {
                if (file.position >= file.buffer.length) {
                    ...
                    this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, blocksRead);
    ...
    FWrite() {
        ...
        const file = this.openFiles.get(fileHandle);
        if (!file) {
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // 0 blocks written
            this.lastError = this.ERROR_OBJECT_NOT_FOUND;
        ...
        if (file.isConsole) {
            ...
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, numBlocks);
    ...
    VFPrintf() {
        const fileHandle = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const fmtAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const argvAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
        ...
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, formatted.length);
     ...
     */
    VPrintf() {
        const fmtAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const argvAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const fmt = this.readString(fmtAddr);
    ...
    formatString(fmt, argvAddr) {
        ...
        for (let i = 0; i < fmt.length; i++) {
            if (fmt[i] === '%' && i + 1 < fmt.length) {
                ...
                if (longFormat)
                    i++; // Skip the format character after 'l'
            }
            else {
                result += fmt[i];
            }
     ...
     */
    writeBCPLString(address, str, maxLen) {
    ...
    NameFromLock() {
        ...
        if (lock === 0) {
            ...
            if (bufLen < path.length + 1) {
                ...
                this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // FALSE
                return;
            }
            this.writeString(bufAddr, path);
            this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, -1); // TRUE
            return;
        }
     ...
     */
    FilePart() {
        const pathAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        ...
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, resultAddr);
    }
    /**
     ...
     */
    Fault() {
        const code = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const headerAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const bufAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
        const bufLen = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D4);
        this.lastError = code;
        if (code === 0) {
        ...
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, message.length);
    }
    /**
     ...
     */
    ReadArgs() {
        const templateAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const arrayAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const rdargsAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
     ...
     */
    FreeArgs() {
        const rdargsAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        ...
        this.emulator.setRegister(MoiraEmulator_1.CPURegister.D0, 0); // void return
    }
    /**
     ...
     */
    DateToStr() {
        const datetimeAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
     ...
     */
    AddPart() {
        const dirnameAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D1);
        const filenameAddr = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D2);
        const size = this.emulator.getRegister(MoiraEmulator_1.CPURegister.D3);
