...
const HunkLoader_1 = require("./loader/HunkLoader");
const ExecLibrary_1 = require("./api/ExecLibrary");
const AEDoorLibrary_1 = require("./api/AEDoorLibrary");
const DosLibrary_1 = require("./api/DosLibrary");
const IconLibrary_1 = require("./api/IconLibrary");
const LibraryTraps_1 = require("./api/LibraryTraps");
const XIMProtocol_1 = require("./XIMProtocol");
const KickstartRom_1 = require("./KickstartRom");
const NodeStatusManager_1 = require("../nodes/NodeStatusManager");
const UserStructures_1 = require("./structures/UserStructures");
const GlobalStructures_1 = require("./structures/GlobalStructures");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class AmigaDoorSession {
    dumpInstruction(pc, count = 8) {
        if (!this.emulator) {
            return;
        }
        const bytes = [];
    ...
    constructor(socket, config) {
        ...
        this.sharedBBSData = null;
        this.isRunning = false;
        this.executionTimer = null;
        this.iterationCount = 0;
        this.d0Was30 = false; // Track if D0 has been set to 30 (for debugging)
        this.sentInitialMessage = false; // Track if we've sent initial IPC message to RTW
        this.rtwLoopCount = 0; // Track iterations in RTW loop at 0x1158-0x1160
        this.doorPortAddress = 0; // AEDoorPort message port address
        this.rtwPortInjected = false; // Track if we've injected RTW reply port
        this.aePortAddress = 0; // AEDoorPort2 message port address
        this.trapVerified = false; // Track if we've verified trap instructions in memory
    ...
    setupSocketHandlers() {
        ...
        this.socket.on('keys:state', (data) => {
    ...
    async start() {
        try {
            ...
            const kickstart = new KickstartRom_1.KickstartRom();
            ...
            this.sharedUserData = new UserStructures_1.SharedUserData(this.emulator);
            ...
            this.sharedBBSData = new GlobalStructures_1.SharedBBSData(this.emulator);
    ...
    async initializeExec() {
        ...
        this.dosLibrary.setOutputCallback((text) => {
        ...
        NodeStatusManager_1.nodeStatusManager.initializeInEmulator(this.emulator, this.execLibrary, 0xB0000);
        ...
        this.iconLibrary = new IconLibrary_1.IconLibrary(this.emulator, bbsRoot);
    ...
    async loadDoor() {
        ...
        const hunkLoader = new HunkLoader_1.HunkLoader();
     ...
     */
    handleDoorMessage(portAddr, msgAddr) {
        if (!this.emulator || !this.execLibrary)
            return;
        console.log(`[AmigaDoorSession] ===============================================`);
     ...
     */
    processCommand(command, data, str, msgAddr, replyPortAddr) {
     ...
     */
    writeStringToMessage(msgAddr, str) {
        if (!this.emulator)
            return;
