...
export class TrackerEffectProcessor {
  ...
  private updatePortamento(): void {
    ...
    if (this.state.portamentoDirection === 'up') {
      this.synth.frequency.value = currentFreq + step;
    } else if (this.state.portamentoDirection === 'down') {
      this.synth.frequency.value = Math.max(20, currentFreq - step);
    } else if (this.state.portamentoDirection === 'tone') {
      // Slide towards target
      if (currentFreq < this.state.portamentoTarget) {
        this.synth.frequency.value = Math.min(this.state.portamentoTarget, currentFreq + step);
      } else if (currentFreq > this.state.portamentoTarget) {
        this.synth.frequency.value = Math.max(this.state.portamentoTarget, currentFreq - step);
      }
    }
  }
  ...
  private updateVibrato(tick: number): void {
    ...
    const offset = Math.sin(this.state.vibratoPhase) * this.state.vibratoDepth * 10;
  ...
  private updateTremolo(tick: number): void {
    ...
    const offset = Math.sin(this.state.tremoloPhase) * this.state.tremoloDepth;
  ...
  private effectExtended(type: number, param: number): void {
    switch (type) {
      ...
      case 0x4: // Set vibrato waveform
        this.state.vibratoWaveform = ['sine', 'sawtooth', 'square', 'random'][param % 4] as any;
        break;
