...
function resolveGlobalWallSettingsFile(...filenames: string[]): string {
  for (const filename of filenames) {
    const candidate = path.join(process.cwd(), 'doors', 'gwall', filename);
    if (existsSync(candidate)) {
      return candidate;
    }
  }
  return path.join(process.cwd(), 'doors', 'gwall', filenames[0]);
}
...
function loadConfig(): void {
  const progdirCfg = resolveGlobalWallSettingsFile('GWALL.cfg', 'gwall.cfg');
...
function readSettings(): void {
  const settingsPath = resolveGlobalWallSettingsFile('GWall.cfg', 'gwall.cfg');
...
function saveSettings(): void {
  const settingsPath = path.join(process.cwd(), 'doors', 'gwall', 'GWall.cfg');
