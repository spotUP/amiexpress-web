...
class InputManager {
     ...
     */
    async getSingleLineInput(prompt, x, y, options = {}) {
        ...
        return new Promise((resolve) => {
            const inputHandler = async (user, keyEvent) => {
     ...
     */
    async getMultiLineInput(prompt, x, y, options = {}) {
        ...
        return new Promise((resolve) => {
            const inputHandler = (user, keyEvent) => {
                const key = keyEvent.key;
                // Ctrl+D - Done
                if (key === '\x04') {
    ...
    renderSingleLine(prompt, value, cursorPos, x, y, maxLength, showCounter) {
    ...
    renderMultiLine(prompt, lines, currentLine, cursorPos, x, y, maxLineLength, maxLines) {
        let output = `\x1b[${y};${x}H`;
        output += `\x1b[2K`;
     ...
     */
    showAutocomplete(suggestions, x, y, selectedIndex) {
