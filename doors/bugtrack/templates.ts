...
export class TemplateManager {
   ...
   */
  getTemplatesByCategory(category: string): BugTemplate[] {
   ...
   * Get template by ID
   */
  getTemplate(id: string): BugTemplate | undefined {
   ...
   */
  getMostUsedTemplates(limit: number = 5): BugTemplate[] {
    return [...this.templates]
   ...
   * Add custom template
   */
  addCustomTemplate(template: Omit<BugTemplate, 'id' | 'isBuiltIn' | 'usageCount'>): BugTemplate {
    const customTemplate: BugTemplate = {
      ...template,
      id: `custom_${Date.now()}`,
      isBuiltIn: false,
      usageCount: 0
  ...
  load(filepath: string): void {
    ...
    if (fs.existsSync(filepath)) {
      ...
      const customTemplates: BugTemplate[] = JSON.parse(data);
   ...
   */
  applyTemplate(templateId: string): Partial<{
    category: string;
    subcategory: string;
    priority: string;
    description: string;
