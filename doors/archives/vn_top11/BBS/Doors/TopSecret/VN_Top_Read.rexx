...
Version="[37m[[34mNastyTopSecret [33mv1.1[37m\[32mRead.02[37m]" /* Dont change this */
Temp="RAM:"
...
If word(getvar(131),2)="SYSOP" then do
 if sysop=user then do ; Slot="S" ; end
 Else do ; Slot=Slot ; end
end

if Slot="S" then Transmit "SYSOP MODE ACTIVATED!"

Transmit d2c(12)
Transmit "Nasty Top Secret Message Reader "version
Transmit ""
Sendmessage "[37mScanning your [34m[[33mPrivate MailBox[34m][37m"
Call ScanMail

QUIT:
if mailwaiting=1 then do
 Call DeleteMail
end
Else do
 Loopback(31,"No [34m[[33mPrivate MailBox[34m][32m Mail today.[37m")
 Transmit ""
End
SHUTDOWN
EXIT
...
Transmit d2c(13)"     [36mTo[33m: [32m([33mEnter[32m)[0m=[33mSysop[32m? [0m"msgFrom
Transmit "[36mSubject[33m: [32m([33mBlank[32m)[0m=[33mabort[32m? [0m"msgsubj
Call Readwrtnumba
Return
...
Getchar
KEY=Upper(result)
If KEY=N then do ; loopback(4,"No[37m) ") ; Call DeleteTemp ; Loop=Loop-1 ; Return ;End
loopback(4,"Yes[37m)")
Transmit ""
Sendmessage "[36mLAST CHANCE!   ([33mEnter[36m) to Start, ([33mA[36m)bort? "
Getchar
KEY=Upper(result)
if KEY=A then do ; loopback(8,"Aborted[36m)") ; Call DeleteTemp ; Loop=Loop-1 ; Return ; End
loopback(8,"Downloading[36m)")
Transmit "[37m"
Transmit "Prepare For Transfer."
/* open    <<<-----------------------------Whats this here for? */
...
address command "copy "TopSecretDir||slot".msg."loop" to "TopSecretDir"S.msg."sysnummail+1
address command "copy "TopSecretDir||slot".hdr."loop" to "TopSecretDir"S.hdr."sysnummail+1
...
Quitmsg:
Transmit 'Cant "Quit" FORCE READ MAIL.'
Again:
Loop=Loop-1
Return
...
Writethemsg:
Transmit ""
SendString "[37mQuote in Reply ([35my[37m/N[37m)"
...
DeleteMail:
Sendmessage "Want to Delete All [34m[[33mPrivate MailBox[34m][37m Mail ([35my[37m/N[37m)"
getchar ; quote=Upper(Result)
