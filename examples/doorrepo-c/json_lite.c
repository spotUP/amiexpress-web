 ...
 * quote. Handles \" \\ \/ \n \t \r \uXXXX (a \uXXXX code point above 0xFF
                            ...
                            const char **obj_start, unsigned long *obj_len)
{
    ...
    if (json == (const char *) 0 || cursor == (unsigned long *) 0
    ...
    /* --- Replay pass -------------------------------------------------
     * Reconstructs the bracket/string context as of *cursor by
     * re-walking from the very start of `json`, bounded strictly to
     * *cursor bytes (already proven <= json_len above, so this can never
     * run unbounded even on adversarial input). This is what lets a
     * plain byte-offset cursor correctly recognise "the next '{' is a
     * direct child of the target array" on every call, not just the
     * first - a call starting mid-array otherwise has no memory of
     ...
     * signature stays exactly the plain `unsigned long *cursor` this was
     * built against. */
    depth = 0;
    in_string = 0;
    p = json;
    scanned = 0;
    while (p < json + *cursor && scanned < JSON_LITE_MAX_SCAN_BYTES) {
        char c = *p;

        if (in_string) {
            if (c == '\\' && p + 1 < json + *cursor) {
                p += 2;
            } else {
                if (c == '"') {
                    in_string = 0;
