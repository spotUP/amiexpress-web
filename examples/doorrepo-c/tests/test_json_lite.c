...
TEST(array_zero_rows_returns_end_immediately)
{
    ...
    int rc = json_next_array_object("{\"rows\":[]}", &cursor, &obj_start, &obj_len);
...
TEST(array_one_row_fields_extracted_and_scan_ends)
{
    ...
    rc = json_next_array_object(json, &cursor, &obj_start, &obj_len);
...
TEST(array_three_rows_cursor_advances_correctly)
{
    ...
    for (i = 0; i < 3; i++) {
        int rc = json_next_array_object(json, &cursor, &obj_start, &obj_len);
...
TEST(array_truncated_mid_object_fails_cleanly)
{
    ...
    int rc = json_next_array_object(
        "{\"rows\":[{\"id\":1,\"archiveNam", &cursor, &obj_start, &obj_len);
...
TEST(array_unterminated_string_inside_row_fails_cleanly)
{
    ...
    unsigned long cursor = 0;
    int rc = json_next_array_object(
        "{\"rows\":[{\"id\":1,\"note\":\"never closes}]}", &cursor, &obj_start, &obj_len);
...
TEST(array_mismatched_brackets_fail_cleanly)
{
    ...
    rc = json_next_array_object("{\"rows\":[{\"id\":1}}", &cursor, &obj_start, &obj_len);
...
TEST(array_refuses_null_args)
{
    ...
    unsigned long obj_len;
    unsigned long cursor = 0;
    ASSERT_TRUE(json_next_array_object((const char *) 0, &cursor, &obj_start, &obj_len) != 0,
                "NULL json");
    ASSERT_TRUE(json_next_array_object("{}", (unsigned long *) 0, &obj_start, &obj_len) != 0,
                "NULL cursor");
    ASSERT_TRUE(json_next_array_object("{}", &cursor, (const char **) 0, &obj_len) != 0,
                "NULL obj_start");
    ASSERT_TRUE(json_next_array_object("{}", &cursor, &obj_start, (unsigned long *) 0) != 0,
                "NULL obj_len");
}
