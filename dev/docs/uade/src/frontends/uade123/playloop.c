...
static void _write_key_bool(struct z_string *zs, const char *key, const int x)
{
	z_assert(z_string_cat_c_str(zs, "'"));
	_escape_python_str(zs, key);
	z_assert(z_string_cat_c_str(zs, "': "));
	_escape_python_str(zs, x ? "True" : "False");
	z_assert(z_string_cat_c_str(zs, ", "));
}

static void _write_key_bytes(struct z_string *zs, const char *key, const char *s)
{
	z_assert(z_string_cat_c_str(zs, "'"));
	_escape_python_str(zs, key);
	z_assert(z_string_cat_c_str(zs, "': b'"));
	_escape_python_str(zs, s);
	z_assert(z_string_cat_c_str(zs, "', "));
}

static void _write_key_double(struct z_string *zs, const char *key,
			      const double x)
{
	char buf[32];
...
static void print_info(struct uade_state *state)
{
	...
	if (uade_info_mode) {
		/*
		 * Write out metadata in Python AST format so that automation
		 * (testing) frameworks can use it easily.
		 */
		...
		_write_key_bool(ast, "uade:content_detection",
				info->detectioninfo.content);
		if (info->duration > 0.0)
			_write_key_double(ast, "uade:duration",
					  info->duration);
		_write_key_bool(ast, "uade:is_custom",
				info->detectioninfo.custom);
		_write_key_int(ast, "uade:max_subsong", info->subsongs.max);
		_write_key_int(ast, "uade:min_subsong", info->subsongs.min);
		_write_key_bytes(ast, "uade:modulefname", info->modulefname);
		_write_key_bytes(ast, "uade:playerfname", info->playerfname);
		...
		printf("subsongs: cur %d min %d max %d\n",
		       ...
		       info->subsongs.max);
		printf("modulefname: %s\n", info->modulefname);
		printf("playerfname: %s\n", info->playerfname);
