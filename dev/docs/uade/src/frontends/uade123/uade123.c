...
int main(int argc, char *argv[])
{
	...
	struct option long_options[] = {
		...
		{"write-audio-fd",   1, NULL, OPT_WRITE_AUDIO_FD},
		{NULL,               0, NULL, 0}
	};
...
#define GET_OPT_STRING(x) if (strlcpy((x), optarg, sizeof(x)) >= sizeof(x)) { uade_die("Too long a string for option %c.\n", ret); }
	...
	while (1) {
		ret = getopt_long(argc, argv,
				  "@:1cde:f:gG:hij:km:np:P:qrs:S:t:u:vw:x:y:z",
				  long_options, 0);
		if (ret == -1)
			break;
		...
		case 'm':
			uade_config_set_option(uc_cmdline, UC_MUTEMASK, optarg);
		...
		case OPT_WRITE_AUDIO_FD:
			uade_config_set_option(uc_cmdline, UC_WRITE_AUDIO_FD,
					       optarg);
			break;
...
"                     on terminal. \n"
" -m x, --mute=x      Mute one or several channels, where x consists of digits\n"
"                     0-3. Example: mute 3rd and 4th channels by specifying -m 23\n"
