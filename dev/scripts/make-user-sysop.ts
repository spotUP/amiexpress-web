 ...
 *
 * Usage: npx ts-node -P dev/scripts/tsconfig.json dev/scripts/make-user-sysop.ts <username>
 */
...
function main() {
  ...
  if (!username) {
    console.log(`${colors.red}Usage: npx ts-node -P dev/scripts/tsconfig.json dev/scripts/make-user-sysop.ts <username>${colors.reset}`);
    console.log(`\nExample: npx ts-node -P dev/scripts/tsconfig.json dev/scripts/make-user-sysop.ts testsysop`);
    process.exit(1);
  }
