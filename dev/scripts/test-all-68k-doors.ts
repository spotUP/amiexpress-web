 ...
 *
 * Usage: cd web/backend && npx tsx ../../dev/scripts/test-all-68k-doors.ts [--timeout=15] [--filter=pattern]
 */
...
function resolveAmigaPath(amigaPath: string): string {
  ...
  resolved = resolved.replace(/^Doors:/i, 'Doors/');
  resolved = resolved.replace(/^doors:/i, 'Doors/');

  // Build full path
  const fullPath = path.join(BBS_ROOT, resolved);

  // Try different extensions if file doesn't exist
  if (!fs.existsSync(fullPath)) {
    // Try with .020 extension (68020 version)
    if (fs.existsSync(fullPath + '.020')) {
      return fullPath + '.020';
    }
    // Try with .000 extension (68000 version)
    if (fs.existsSync(fullPath + '.000')) {
      return fullPath + '.000';
    }
  }
...
async function testDoor(door: DoorConfig): Promise<TestResult> {
  ...
  if (!door.location || !fs.existsSync(doorPath)) {
    return {
      ...
      duration: 0,
      output: '',
      error: `Door binary not found: ${doorPath}`
    };
  }
  ...
  try {
    // Create session with minimal tracing
    ...
    const result = await Promise.race([
      session.run(door.args ? door.args + '\n' : '\n'),
