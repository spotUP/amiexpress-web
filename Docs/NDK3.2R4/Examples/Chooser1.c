;/* Chooser Example
sc LINK Chooser1.c LIB lib:reaction.lib NOSTACKCHECK
quit
*/
 ...
 **
 **  This opens a window with chooser.gadget. We will use reaction.lib's
 **  ChooserLabels() and FreeChooserLabels() utility functions to create the
 **  item labels.
 **
 **/
	...
	else if ( AppPort = CreateMsgPort() )
	{
		...
		if (chooserlist1)
		{
			...
			objects[OID_MAIN] = WindowObject,
				...
				WA_Title, "ReAction Chooser Example 1",
				WA_Activate, TRUE,
				WA_DepthGadget, TRUE,
				WA_DragBar, TRUE,
				WA_CloseGadget, TRUE,
