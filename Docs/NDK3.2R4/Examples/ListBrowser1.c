;/*
sc ListBrowser1.c LINK LIB lib:reaction.lib NOSTACKCHECK
quit
*/
...
main()
{
	...
	if (screen = LockPubScreen(NULL))
	{
		...
		if (win = OpenWindowTags(NULL,
			...
			WA_Title, "ReAction ListBrowser Example 1",
			WA_MinWidth, 50,
			WA_MinHeight, 50,
			WA_MaxWidth, -1,
			WA_MaxHeight, -1,
			TAG_DONE))
		{
			...
			if (ListBrowserBase = (struct Library*) OpenClass("gadgets/listbrowser.gadget", 0))
			{
				...
				if (listbrowser_gad = (struct Gadget *)NewObject(LISTBROWSER_GetClass(), NULL,
														...
														GA_RelWidth, -(win->BorderLeft + win->BorderRight + 10),
														...
														TAG_END))
				{
					...
					if (LabelBase = (struct Library*) OpenClass("images/label.image", 0))
					{
