...
#else
  /* The microsecond quantities are (inherited from devices/timer.h)
     unsigned integers. We need to make sure that the difference will
     always come out as a signed quantity, or otherwise the results
     will be wildly off the mark. */
  return ((pst1->tv_sec - pst2->tv_sec) * 1000.0
	  + ((int)pst1->tv_usec - (int)pst2->tv_usec) / 1000.0);
#endif /* AMIGA */
}
...
gettimeofday_resolution (void)
{
  ...
  return 0.1;
#else
  /* The granularity is actually in the microsecond range. */
  return 0.000001;
#endif /* AMIGA */
}
