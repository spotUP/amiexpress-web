...
connect_with_timeout (int fd, const struct sockaddr *addr, socklen_t addrlen, double timeout)
{
	...
	if(timeout > 0)
	{
		...
		if(ioctl(fd, FIONBIO, &non_blocking_io) == 0)
		{
			...
			/* Convert the timeout into seconds and microseconds. */
			fractional = modf(timeout, &integral);

			tv.tv_sec	= integral;
			tv.tv_usec	= 1000000 * fractional;

			FD_ZERO(&write_fds);
			FD_SET(fd, &write_fds);

			/* We will wait for the file to either become writable
			 * (= connection completed or failed) or for the attempt
			 * to time out.
			 */
			...
			while(TRUE)
			{
				/* Wait for the connection status to change (success/failure),
				...
				if (nfds == 1)
				{
					...
					/* Figure out if the connection has failed or succeeded. */
			...
			ioctl(fd, FIONBIO, &non_blocking_io);
...
#else
{
  ...
  fractional = modf(maxtime, &integral);

  tmout.tv_sec = integral;
  tmout.tv_usec = 1000000 * fractional;
}
#endif /* AMIGA */
