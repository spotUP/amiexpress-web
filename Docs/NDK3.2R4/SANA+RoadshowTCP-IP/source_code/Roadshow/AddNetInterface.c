...
/* Use a minimum stack size of 8000 bytes on AmigaOS4, just to be sure. */
TEXT default_stack = "$STACK: 8000";
...
/* The 68k build needs to make sure that it has enough stack space
   available to work properly. The default 4096 byte stack size appears
   to be insufficient, so we make our own... */
...
_start(VOID)
{
	...
	if(error == OK)
	{
		...
		/* The AmigaOS 2.x/3.x version of this command requires a certain
		   minimum amount of stack space, and we will have to make sure
		   that it will be available. */
		#if defined(_SWAP_STACK_H)
		{
			const ULONG required_stack_size = 8000;

			if(get_current_stack_size(cc) < required_stack_size)
			{
				struct StackSwapStruct * stk;
				ULONG new_stack_size;
				APTR new_stack;

				/* Make the stack size a multiple of 32 bytes. */
				new_stack_size = 32 + ((required_stack_size + 31UL) & ~31UL);
				...
				if(stk != NULL && new_stack != NULL)
				{
					...
					result = swap_stack_and_call(cc,(stack_swapped_func_t)cmd,stk,cc->cc_SysBase);
				}
				else
				{
...
/* Get the stack size currently available to this process (in bytes).
   Adapted from "The Amiga Guru Book" (1993), section 2.1.2.1. */
STATIC ULONG
get_current_stack_size(struct CommandContext * cc)
{
	DECLARE_SYSBASE(cc);
	...
	/* Is this a CLI process? Stack boundaries may be different from
	   the Task stack boundaries. */
	if(pr->pr_Task.tc_Node.ln_Type == NT_PROCESS && pr->pr_CLI != (BPTR)NULL)
	{
		const BYTE * upper = ((BYTE *)pr->pr_ReturnAddr) + sizeof(ULONG);
		ULONG total = *(ULONG *)pr->pr_ReturnAddr;

		lower = upper - total;

		/* Careful: If we replaced the stack then the upper and lower
		            CLI stack boundaries will no longer be relevant.
		            We need to fall back onto the current Task stack
		            boundaries. */
		if(current_sp < lower || upper <= current_sp)
			lower = (BYTE *)pr->pr_Task.tc_SPLower;
	}
	else
	{
		lower = (BYTE *)pr->pr_Task.tc_SPLower;
...
STATIC LONG ASM
cmd(REG(a0, struct CommandContext * cc))
{
...
add_interface(struct CommandContext * cc,STRPTR name,enum add_interface_mode_t operating_mode,LONG timeout)
{
	...
	STRPTR link_status_command = NULL;
	...
	while(FGets(cc->cc_File,str,sizeof(str)-2) != NULL)
	{
		...
		/* Invoke a shell command whenever the interface link
		   status (online, offline, up, down) change? */
		if(args.LinkStatusCommand != NULL)
			link_status_command = args.LinkStatusCommand;
	}
	...
	if(link_status_command != NULL)
	{
		error = add_tag(cc,IFA_LinkStatusCommand,(ULONG)link_status_command);
		if(error != OK)
		{
			if(NOT cc->cc_Quiet)
			{
				error_printf(cc,get_str(cc,MSG_ADDNETINTERFACE_NOT_ENOUGH_MEMORY_TO_CONFIGURE_LINK_STATUS_COMMAND_TXT),
					interface_name);
			}
