...
cmd(struct CommandContext * cc)
{
	...
	if(args.LinkStatusCommand)
	{
		if(Local_ConfigureInterfaceTags(cc,args.Interface,
			IFC_LinkStatusCommand, args.LinkStatusCommand,
		TAG_DONE) != 0)
		{
			if(NOT quiet)
			{
				...
				Local_ErrorPrintf(cc,get_str(cc,MSG_CONFIGURENETINTERFACE_COULD_NOT_SET_LINK_STATUS_COMMAND_TXT),
					cc->cc_ProgramName,args.Interface,args.LinkStatusCommand);
