...
- Latest: SDK dev loop hardened. Installer validates door type (TS/JS/PY/AREXX) and entrypoint. Door Doctor (`npm run door:doctor`) now includes PY/AREXX checks (entry extensions, optional py_compile via DOCTOR_COMPILE_PY=1, optional runtime smoke via DOCTOR_RUN=1). Skeleton generator now supports ts/py/rexx templates (`npm run door:create <name> [ts|py|rexx]`). Door Manager info view shows last reload status/log and supports hot reload with `R`/`L`. Legacy sdk watcher removed.
...
- Next: Add CI/harnessed runtime tests for PY/AREXX (with mock BBS IO), UI badges/log panel/search/filter, and richer doctor run harness. Tests not rerun this pass.
