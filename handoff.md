...
- Latest: SDK/dev loop improvements for PY/AREXX + UI polish. Door Doctor now supports runtime smokes (`DOCTOR_RUN=1`) and py_compile (`DOCTOR_COMPILE_PY=1`); skeleton generator supports ts/py/rexx templates; added `dev/scripts/run-door-harness.js` and package scripts `door:doctor:strict`/`door:harness`. Door Manager list now shows type/hot badges, supports filter (`F`), and info view retains reload logs (`L`) and status. Legacy sdk watcher removed earlier.
...
- Next: Add CI/harnessed runtime tests for PY/AREXX with mock BBS IO, deepen door doctor to exercise door harness I/O, and extend Door Manager with per-door log panel/search/filter (list now has filter + badges). Tests not rerun this pass.
