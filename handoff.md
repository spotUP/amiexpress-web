...
- Latest: GWALL installer now copies SDK doors instead of symlinking and auto-seeds required configs. `dev/scripts/install-sdk-doors.js` copies each sdk/doors/<door> into doors/<door>, removes old symlinks, and for gwall writes default `GWall.cfg` (style/shortcode from env `GWALL_BBS_CODE`/`BBS_SHORTCODE`/`BBS_CODE`, default `AMI`) plus `GWALL.cfg` (network host/port/timeout). GWALL runtime already tolerates missing configs with env fallbacks.
- Last prompt: “why wasn’t the config copied … ensure SDK installs bring configs” → addressed above. Earlier menu/config crash and runtime GWALL prompt fixed previously.
- Next: Rerun `node dev/scripts/install-sdk-doors.js` to materialize copies/configs, then launch GWALL to verify it skips configuration and reaches the remote wall. Tests not rerun this pass.
