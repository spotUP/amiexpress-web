...
- `GLOBALWALL` now defaults to HTTPS when contacting `scenewall.bbs.io` (and respects a new optional `PROTOCOL=` entry in `GWall.cfg`). The door picks either `http` or `https` before issuing `/GlobalWall/api/WallItems`, so it should stop reporting “server is not currently responding” when the service is reachable over HTTPS.
- Added logging around the door’s HTTP/HTTPS fetch so we record non-200 status codes before the door replies “The server is not currently responding”; the backend log will now show the actual response code so we can troubleshoot whether the remote wall endpoint is rejecting the request or timing out.
