...
- Latest: SDK workflow now operates directly on live `doors/`. New watcher `dev/scripts/watch-doors.js` (used by `npm run dev:doors`) monitors `doors/<id>`, builds if needed (unless `--no-build`), and reruns `install-sdk-doors.js --door <id>` to refresh Commands/.info/configs without touching `sdk/doors`. The installer now prefers `doors/` as the source (only falls back to `sdk/doors` if missing) and skips copying when already working from `doors/`.
...
- Next: Run `npm run dev:doors`, edit a TS/Python/ARexx door in `doors/`, and confirm it hot-reloads. 68k binaries remain ignored (no package.json). Tests not rerun this pass.
