...
- Latest: SDK/dev loop improvements for PY/AREXX + UI polish. Door Doctor supports runtime smokes (DOCTOR_RUN=1) and py_compile (DOCTOR_COMPILE_PY=1); skeleton generator supports ts/py/rexx templates; added `dev/scripts/run-door-harness.js`, fixture doors (`dev/fixtures/doors/{py-hello,rexx-hello}`), sync helper, and scripts `door:doctor:strict`/`door:harness`/`door:ci`. Door Manager list now shows type/hot badges, supports filter (F), and offers a log panel (`L` from list) plus reload logs in info view. Legacy sdk watcher removed earlier.
...
- Next: Wire `npm run door:ci` into CI (AREXX run skips if interpreter missing). Harness now carries mock BBS env (BBS_HARNESS/BBS_USER/BBS_INPUT), Log panel renders with paging. Remaining niceties: richer log scrolling in list mode (currently 20-line pages), better search/filter hints, and optional AREXX interpreter in CI. Tests not rerun beyond `door:ci`.
