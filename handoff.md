...
- Latest: Added hot-reload loop for SDK doors. New `dev/scripts/watch-sdk-doors.js` (exposed as `npm run dev:doors`) watches `sdk/doors`, rebuilds a changed door (if it has `npm run build`), and reruns `install-sdk-doors.js --door <name>` to refresh Commands/.info and `doors/<name>` copies. Requires `chokidar` (now in root devDependencies). `install-sdk-doors.js` accepts `--door <name>` and `--quiet`.
- Recent: Door resolution prefers installed `doors/` over `sdk/doors`; client bundler resolves SDK deps via BBS tree. GWALL SDK package now ships default configs (`GWall.cfg` / `GWALL.cfg`) via package files; installer seeds configs when copying.
- Next: Exercise the watcher: `npm run dev:doors` then edit a door under `sdk/doors/<id>` and confirm it auto-copies/builds. Also validate running doors with `sdk/doors` absent to ensure fallback remains solid. Tests not rerun this pass.
