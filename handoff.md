...
- Latest: GWALL SDK package now ships default configs: added `GWall.cfg` (style, shortcode default `AMI`, colours) and `GWALL.cfg` (network host/port/timeout) into `sdk/doors/gwall/` plus a `files` whitelist in package.json so they appear in release archives. Installer already seeds local configs when copying SDK doors.
- Last prompt: “config missing from gwall release archive” → fixed above. Earlier installer/runtime guards remain in place.
- Next: Rebuild/package gwall from `sdk/doors/gwall` (e.g., `npm pack`) and confirm archive contains both cfg files; rerun `dev/scripts/install-sdk-doors.js` if distributing via local copy. Tests not rerun this pass.
