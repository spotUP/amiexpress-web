...
- Latest: Door resolution now prefers installed `doors/` copies over `sdk/doors` so deployments without the SDK still run smoothly. Updated door APIs and handler manifest loading to use `doors/` first; SDK is a dev fallback. Client door bundler now resolves `@amiexpress/bbs-door-sdk` from `doors`-only installs by probing BBS root `node_modules`/`sdk` and using `require.resolve`.
- Recent: GWALL SDK package now ships default configs (`GWall.cfg` and `GWALL.cfg`) via the package `files` list; installer seeds configs when copying SDK doors.
- Next: Validate a stored-door run (run a client door from `doors/<id>` with no `sdk/doors` present) and ensure bundling still works. Tests not rerun this pass.
