    ...
    try {
      ...
      if (result.rows[0]) {
        ...
        return {
          ...
          firstLogin: user.firstlogin || user.firstLogin,
          calls: Number(user.calls),
          callsToday: Number(user.callstoday || user.callsToday),
          newUser: user.newuser || user.newUser,
          expert: user.expert,
          ansi: user.ansi,
          linesPerScreen: Number(user.linesperscreen || user.linesPerScreen),
          computer: user.computer,
          screenType: user.screentype || user.screenType,
          protocol: user.protocol,
          editor: user.editor,
          zoomType: user.zoomtype || user.zoomType,
          availableForChat: user.availableforchat || user.availableForChat,
          quietNode: user.quietnode || user.quietNode,
          autoRejoin: Number(user.autorejoin || user.autoRejoin),
          confAccess: user.confaccess || user.confAccess,
          areaName: user.areaname || user.areaName,
          uuCP: user.uucp || user.uuCP,
