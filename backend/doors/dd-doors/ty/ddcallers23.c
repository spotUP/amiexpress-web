...
int main(int argc, char *argv[])
{
	...
	if (update) {
		...
		if (user.user_account_id || incsysop) {
			...
			if ((clog.cl_ulbytes / 1048576) > 999) 
			...
			if ((clog.cl_dlbytes / 1048576) > 999) 
...
static int dooutput(void)
{
	...
	fputs("---------------------------[ tHE yARD lAST cALLERS 1.0 (c) Antti Häyrynen ]--\n", outfd1);
	...
	fputs("[34m--------------------------[ [36mtHE yARD lAST cALLERS 1.0 (c) Antti Häyrynen [34m]----\n", outfd2);
