...
describe('TP-6 - the R answer at the graphics prompt', () => {
  ...
  function bbsTitleRoot(prefix: string) {
    const created = tempBbsRoot(prefix);
    fs.mkdirSync(path.join(created.root, 'Screens'), { recursive: true });
    ...
    fs.writeFileSync(
      path.join(created.root, 'Screens', 'BBSTITLE.rip'),
      '!|1B00000000000000\r\n',
      'latin1',
    );
    return created;
  ...
  it('answering R on telnet does not ship RIPscrip source across the screen', async () => {
    const { root, cleanup } = bbsTitleRoot('tp6-rip-telnet-');
  ...
  it('answering R in a browser still arms the RIP canvas', async () => {
    ...
    const { cleanup } = bbsTitleRoot('tp6-rip-web-');
