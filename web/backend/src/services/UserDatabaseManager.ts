...
export class UserDatabaseManager {
  ...
  /**
   * Serialize userKeys struct to binary buffer (56 bytes with 68K alignment)
   */
  ...
  /**
   * Serialize userMisc struct to binary buffer (248 bytes with 68K alignment)
   */
  ...
  appendUser(
    ...
    console.log(
      `[UserDatabaseManager] Appended user "${user.name}" to user.data/keys/misc`
    );
  }
  ...
  updateUser(
    ...
    console.log(
      `[UserDatabaseManager] Updated user "${user.name}" at slot ${slotNumber}`
    );
  }
  ...
  writeConfAccessToDisk(slotNumber: number, confAccess: string): boolean {
    ...
    try {
      ...
      console.log(
        `[UserDatabaseManager] Wrote confAccess="${confAccess}" to slot ${slotNumber}`
      );
      return true;
    } finally {
      fs.closeSync(fd);
      ...
      return {
        messagesPosted:
          ...
          200
            ...
            : this.readInt16At(
                fd,
                userOffset + UserDatabaseManager.MESSAGES_POSTED_OFFSET
              ),
        uploads: this.readInt16At(
          fd,
          userOffset + UserDatabaseManager.UPLOADS_OFFSET
        ),
        downloads: this.readInt16At(
          fd,
          userOffset + UserDatabaseManager.DOWNLOADS_OFFSET
        ),
        timesCalled: this.readInt16At(
          fd,
          userOffset + UserDatabaseManager.TIMES_CALLED_OFFSET
        ),
        timeLastOn: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.TIME_LAST_ON_OFFSET
        ),
        timeUsed: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.TIME_USED_OFFSET
        ),
        timeLimit: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.TIME_LIMIT_OFFSET
        ),
        timeTotal: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.TIME_TOTAL_OFFSET
        ),
        bytesDownload: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.BYTES_DOWNLOAD_OFFSET
        ),
        bytesUpload: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.BYTES_UPLOAD_OFFSET
        ),
        dailyBytesLimit: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.DAILY_BYTES_LIMIT_OFFSET
        ),
        dailyBytesDld: this.readInt32At(
          fd,
          userOffset + UserDatabaseManager.DAILY_BYTES_DLD_OFFSET
        ),
        expert: this.readByteAt(
          fd,
          userOffset + UserDatabaseManager.EXPERT_OFFSET
        ),
        lineLength: this.readInt16At(
          fd,
          userOffset + UserDatabaseManager.LINE_LENGTH_OFFSET
        ),
      };
    } finally {
      fs.closeSync(fd);
  ...
  writeUserStatToDisk(
    ...
    try {
      ...
      console.log(
        `[UserDatabaseManager] Wrote ${field}=${value} to disk at slot ${slotNumber}`
      );
      return true;
    } finally {
      fs.closeSync(fd);
