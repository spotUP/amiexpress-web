...
console.error('[executeAmigaDoor] Unable to persist session for door input:', err);
    ...
    // After AquaScan exits, update AquaScan.UserData with current DateStamp.
    // AquaScan has no Write() LVO calls — it reads UserData but the BBS must update it.
    // This covers standalone N-command runs (finishConferenceScan handles confScan runs).
    if (/aquascan/i.test(doorPath) && session.user?.slotNumber && session.user.slotNumber > 0) {
      try {
        ...
        fs.closeSync(fdUs);
console.log(`[executeAmigaDoor] Updated AquaScan.UserData slot ${session.user.slotNumber} -> days=${ds.days} min=${ds.minutes} tick=${ds.ticks}`);
      } catch (err) {
console.error('[executeAmigaDoor] Failed to update AquaScan.UserData:', err);
      }
    }
