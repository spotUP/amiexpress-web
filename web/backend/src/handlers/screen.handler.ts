...
// ============================================================
// Defined once in mci-dispatch.ts (the dispatch entries emit them; the
// walker below and the `~SP.` pre-pass consume them). Only SP is still
// referenced by name here - the rest are emitted by the dispatch table.
const SENTINEL_SP = MCI_SENTINELS.SP;
...
console.error('[parseMciCodes] Error getting message base descriptions:', error);
  ...
  // mode only: the inline branch inside buildMciDispatch ADDS ~CC_/~SS_/~SR_
  // sentinel emitters to prefixDispatch, and those are caller-handled codes in
  // every other sense - counting them here would say the tokenizer owns them.
