...
async function processFileUpload(
  ...
  if (session.inDoorManager && !isRegularFileUpload) {
    console.log('[processFileUpload] Door Manager is active (door archive upload), skipping');
    return;
  }
...
export function registerFileHandlers(socket: Socket) {
  ...
  socket.on('file-uploaded', async (data: { filename: string; originalname: string; size: number; path?: string }) => {
    ...
    // But allow regular file uploads to proceed normally even if inDoorManager is true
    if (session.inDoorManager && !isRegularFileUpload) {
      console.log('[file-uploaded] Door Manager is active (door archive upload), skipping normal file upload handler');
      return;
    }
