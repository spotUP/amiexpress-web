...
function resolveGlobalWallDir(): string {
  return resolveDoorResourcePath('doors', 'gwall') || path.join(PROJECT_ROOT, 'doors', 'gwall');
}

function resolveGlobalWallFile(filename: string): string {
  return resolveDoorResourcePath('doors', 'gwall', filename) || path.join(resolveGlobalWallDir(), filename);
}

function resolveGwallConfig(...filenames: string[]): string {
  for (const filename of filenames) {
    const candidate = resolveDoorResourcePath('doors', 'gwall', filename);
    if (candidate) {
      return candidate;
    }
  }
