...
export class DoorManager {
  ...
  async start(): Promise<void> {
    try {
      this.socket.emit('ansi-output', '\x1b[?25l\x1b[2J\x1b[H'); // Hide cursor, clear screen
...
console.error('[Door Manager] Error details:', {
  ...
  private setupInputHandlers(): void {
    this.inputHandler = (data: string) => { this.handleInput(data); };
  ...
  private handleInput(data: string): void {
    ...
    // ESC = navigate back; from the main list it exits
    if (data === '\x1b') {
      if (this.state.mode === 'list') { this.cleanup(); this.socket.emit('door-exit'); return; }
      if (this.state.mode === 'upload') { this.state.mode = 'list'; this.showList(); return; }
      this.handleInput('b'); return;
    }
  ...
  private cleanup(): void {
    this.socket.emit('ansi-output', '\x1b[?25h'); // Restore cursor
