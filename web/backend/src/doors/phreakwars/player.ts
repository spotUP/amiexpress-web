...
export const gameStates = new Map<string, PhreakWarsGameState>();

/**
 * Create new game state for player
 */
export function createNewGameState(): PhreakWarsGameState {
  return {
    player: {
      handle: '',
      skillLevel: 0.0,
 ...
 */
export function updateSkillLevel(gameState: PhreakWarsGameState): void {
 ...
 */
export function calculateGameProgress(gameState: PhreakWarsGameState): number {
  let progress = 0;
 ...
 */
export function updateAchievements(gameState: PhreakWarsGameState): void {
 ...
 */
export function getNextMilestone(gameState: PhreakWarsGameState): string {
  if (gameState.player.skillLevel < 1.0) return 'Reach skill level 1.0 for Novice Hacker';
  if (gameState.player.skillLevel < 3.0) return 'Reach skill level 3.0 for Apprentice Hacker';
  if (gameState.player.skillLevel < 5.0) return 'Reach skill level 5.0 for Journeyman Hacker';
  if (gameState.player.skillLevel < 7.0) return 'Reach skill level 7.0 for Expert Hacker';
  if (gameState.player.skillLevel < 9.0) return 'Reach skill level 9.0 for Master Hacker';
  if (gameState.player.skillLevel < 10.0) return 'Reach skill level 10.0 for LEGENDARY HACKER';
  return '';
}
 ...
 */
export function checkAndResetDailyLimits(gameState: PhreakWarsGameState): void {
  const now = new Date();
 ...
 */
export function checkDailyLimit(gameState: PhreakWarsGameState, limitType: keyof typeof DAILY_LIMITS, currentCount: number): boolean {
 ...
 */
export function deletePlayer(userId: string): PhreakWarsGameState {
