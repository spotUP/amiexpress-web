...
class NodeStateGlobals {
    ...
    static writeToMemory(emulator, baseAddr, session) {
        ...
        emulator.writeMemory32(timeLimitAddr, ((_a = session.user) === null || _a === void 0 ? void 0 : _a.timeLimit) || 3600);
        // logonTime (LONG) - logon timestamp
        const logonTimeAddr = baseAddr + 16;
        const logonTime = ((_b = session.user) === null || _b === void 0 ? void 0 : _b.lastLoginAt) ? Math.floor(new Date(session.user.lastLoginAt).getTime() / 1000) : Math.floor(Date.now() / 1000);
        ...
        console.log(`  timeLimit: ${((_e = session.user) === null || _e === void 0 ? void 0 : _e.timeLimit) || 3600}`);
    }
}
