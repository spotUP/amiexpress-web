...
console.error(`[DoorLifecycleManager] Failed addresses: ${failedAddrs.map(a => '0x' + a.toString(16)).join(', ')}`);
        ...
        // TEMP DEBUG: register key checkpoints for DOORSMENU main flow tracing
        // These trap-stops let executeUntilTrap halt at specific PCs without ILLEGAL.
        // Trap dispatcher will check, find no handler, and execution continues —
        // we just observe in pcAfterBatch in the loop.
        const dmCheckpoints = [
          0x2050, // arg pushes start (ctor runner returned)
          0x2062, // JSR _main itself
          0x2068, // after JSR _main (main returned normally)
          0x2074, // dtor bsr (exit path)
          0x209a, // startup final RTS
          0x209c, // ctor/dtor runner entry
          0x20b2, // runner JSR (a0) — fires once per ctor/dtor call
          0x20ce, // runner final RTS
          0x32e8, // _main entry
          0x162a + 0x2008, // main exit branch target (memory)
        ];
        for (const cp of dmCheckpoints) {
          this.emulator.addTrapAddress(cp);
        }
        console.log(`[DIAG] Registered DM checkpoints (PRE-EXEC): ${dmCheckpoints.map(c=>'0x'+c.toString(16)).join(', ')}`);
...
console.error(`[DoorLifecycleManager] CRITICAL: Memory[0x4] became ZERO at iter ${this.executionState.iterationCount}! Fixing.`);
        ...
        // (DM checkpoints were installed pre-execution above)
        // Log every checkpoint hit, then REMOVE the trap-stop so execution continues
        const dmCheckpointSet = new Set([
          0x2050,
          0x2062,
          0x2068,
          0x2074,
          0x209a, // startup final RTS
          0x209c, // ctor runner entry
          0x20b2, // runner JSR (a0) — fires once per ctor call
          0x20ce, // runner final RTS
          0x32e8, // _main entry
          0x162a + 0x2008, // main exit branch target
        ]);
        if (dmCheckpointSet.has(pcAfterBatch)) {
          const sp = this.emulator.getRegister(15);
          const a5 = this.emulator.getRegister(13);
          const a0 = this.emulator.getRegister(8);
          const a2 = this.emulator.getRegister(10);
          ...
          const labels: Record<number, string> = {
            0x2050: "arg-pushes-start (ctor runner returned)",
            0x2062: "JSR _main",
            0x2068: "after JSR _main (main returned)",
            0x2074: "dtor bsr (exit path)",
            0x209a: "startup final RTS",
            0x209c: "ctor/dtor runner entry",
            0x20b2: "runner JSR(a0) — about to call ctor/dtor",
            0x20ce: "runner final RTS",
            0x32e8: "_main entry",
            [0x162a + 0x2008]: "main exit branch target",
          };
          if (!(this as any)._cpHitCount) (this as any)._cpHitCount = new Map<number, number>();
          const map = (this as any)._cpHitCount as Map<number, number>;
          const cnt = (map.get(pcAfterBatch) || 0) + 1;
          map.set(pcAfterBatch, cnt);
          // Cap log spam: log first 10 hits per checkpoint
          if (cnt <= 10) {
            const extra = pcAfterBatch === 0x20b2 ? ` A0=0x${a0.toString(16)} A2=0x${a2.toString(16)} D2=0x${d2.toString(16)}` : '';
            console.log(`[DIAG] CHECKPOINT 0x${pcAfterBatch.toString(16)} (#${cnt}): ${labels[pcAfterBatch]} SP=0x${sp.toString(16)} A5=0x${a5.toString(16)} D0=0x${d0.toString(16)}${extra}`);
          }
          // Remove trap-stop and execute one instruction so we don't infinite-loop
          this.emulator.removeTrapAddress(pcAfterBatch);
          this.emulator.executeInstruction();
          // Re-add for next time only if we want repeat hits (only 0x20b2 needs that)
          if (pcAfterBatch === 0x20b2 && cnt < 20) {
            this.emulator.addTrapAddress(0x20b2);
          }
        }
        ...
        // TEMP DEBUG: when PC reaches FreeVec trap (0x7fd4e) for the first time,
        // dump BSS argc/argv state. This fires after the SAS/C parser ctor ran
        // but before main() (if main() was going to be called). It tells us
        // whether the parser populated argc=2.
        if ((pcBeforeBatch === 0x7fd4e || pcAfterBatch === 0x7fd4e) && !(this as any)._freeVecDumped) {
          (this as any)._freeVecDumped = true;
          const argcBSS = this.emulator.readMemory32(0x9874);
          const argvBSS = this.emulator.readMemory32(0x985c);
          const envpBSS = this.emulator.readMemory32(0x9870);
          const sp = this.emulator.getRegister(15);
          const a5 = this.emulator.getRegister(13);
          console.log(`[DIAG] FreeVec(1st) reached. BSS argc=${argcBSS} argv=0x${argvBSS.toString(16)} envp=0x${envpBSS.toString(16)} SP=0x${sp.toString(16)} A5=0x${a5.toString(16)}`);
          // Stack-walk: see if return chain shows we ever entered main
          for (let i = 0; i < 8; i++) {
            const stackVal = this.emulator.readMemory32(sp + i * 4);
            console.log(`[DIAG]   [SP+${i*4}]=0x${stackVal.toString(16)}`);
          }
          // Dump argv[] from BSS
          if (argvBSS !== 0 && argvBSS >= 0x1000 && argvBSS < 0x1000000) {
            for (let i = 0; i < 4; i++) {
              const argvI = this.emulator.readMemory32(argvBSS + i * 4);
              let str = '';
              if (argvI !== 0 && argvI >= 0x1000 && argvI < 0x1000000) {
                for (let j = 0; j < 40; j++) {
                  const c = this.emulator.readMemory(argvI + j);
      ...
      if (msgAddr && msgAddr !== 0) {
        ...
        if (this.ximProtocol) {
          ...
          if (msg) {
            ...
            // Per-message dispatch logs are in the hot path — gate behind
            // debugLog (DEBUG_68K=1) so normal-mode runs don't pay the disk
            // I/O cost. Each unconditional console.log here added ~3-5ms per
            // XIM message at backend.log = 22MB; over the 5-7 messages
            // between conftop's banner #1 and the form-feed clear, that
            // compounded into the ~118ms gap that left banner #1 visible.
            debugLog(`[DoorLifecycleManager] After handleMessage: cmd=${humanName} isWaiting=${isWaiting} replyHandled=${replyHandled} msgAddr=0x${msg.msgAddr.toString(16)}`);
            if (!isWaiting && !replyHandled) {
              // Reply to door via mn_ReplyPort so door's WaitPort/GetMsg unblocks
              debugLog(`[DoorLifecycleManager] Calling replyMsg for cmd=${humanName}`);
              execLib.replyMsg(msg.msgAddr);
              debugLog(`[DoorLifecycleManager] replyMsg completed for cmd=${humanName}`);
            } else if (replyHandled) {
              debugLog(`[DoorLifecycleManager] SKIPPING replyMsg - already handled by system command`);
            } else {
              debugLog(`[DoorLifecycleManager] SKIPPING replyMsg - waiting for input`);
            }
