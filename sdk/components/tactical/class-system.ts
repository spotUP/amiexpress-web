...
import { WeaponType, WeaponRank, GrowthRates, UnitStats } from '../../engines/tactical/tactical-combat-engine';
...
export enum SkillTrigger {
  Always = 'Always',           // Passive skill
  OnAttack = 'OnAttack',       // When unit initiates combat
  OnDefend = 'OnDefend',       // When unit is attacked
  OnHit = 'OnHit',             // When attack lands
  OnCrit = 'OnCrit',           // When critical hit
  PerTurn = 'PerTurn',         // Once per turn
  Conditional = 'Conditional'   // Based on HP, stats, etc.
}
...
export interface Skill {
  ...
  effect: SkillEffect;
  activation?: number; // Percentage chance (0-100)
}

/**
 * Skill effect on combat or stats
 */
export interface SkillEffect {
  statModifiers?: Partial<UnitStats>;
  hitModifier?: number;
  avoidModifier?: number;
  critModifier?: number;
  damageModifier?: number;
  healAmount?: number;
  custom?: (context: any) => void;
}
...
export interface CharacterClass {
  id: string;
  name: string;
  tier: 1 | 2 | 3; // Base, Advanced, Master
  movementType: MovementType;
  baseStats: Partial<UnitStats>;
  growthRates: GrowthRates;
  weaponProficiencies: WeaponType[];
  maxLevel: number;
  promotesTo?: string[]; // IDs of classes this can promote to
  promotionLevel?: number; // Level required for promotion
  learnedSkills: { level: number; skillId: string }[];
}

/**
 * Promotion bonus stats
 */
...
export class ClassSystem extends EventEmitter {
  ...
  private promotionBonuses: Map<string, PromotionBonus> = new Map();
   ...
   */
  registerClass(classData: CharacterClass): void {
   ...
   */
  setPromotionBonus(classId: string, bonus: PromotionBonus): void {
   ...
   * Get classes by tier
   */
  getClassesByTier(tier: 1 | 2 | 3): CharacterClass[] {
   ...
   */
  getPromotionOptions(classId: string): CharacterClass[] {
    ...
    return currentClass.promotesTo
      ...
      .filter(c => c !== undefined) as CharacterClass[];
  }

  /**
   * Get promotion bonus for a class
   */
  getPromotionBonus(classId: string): PromotionBonus | undefined {
   ...
   */
  canPromote(currentClassId: string, level: number): boolean {
   ...
   */
  getSkillsAtLevel(classId: string, level: number): Skill[] {
   ...
   */
  getAllSkillsUpToLevel(classId: string, level: number): Skill[] {
    const classData = this.getClass(classId);
    if (!classData) {
      return [];
    }
   ...
   */
  calculateSkillModifiers(skillIds: string[]): Partial<UnitStats> {
    const modifiers: Partial<UnitStats> = {};

    for (const skillId of skillIds) {
      const skill = this.getSkill(skillId);
      if (skill && skill.effect.statModifiers) {
        for (const [stat, value] of Object.entries(skill.effect.statModifiers)) {
          modifiers[stat as keyof UnitStats] =
            (modifiers[stat as keyof UnitStats] || 0) + (value || 0);
        }
      }
    }
