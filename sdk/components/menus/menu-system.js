...
class MenuSystem {
    ...
    render() {
        ...
        // Draw menu box
        const maxWidth = Math.max(this.config.title.length, ...this.items.map((item) => item.text.length + 4));
     ...
     * @private
     */
    renderBox(pos, width, height, style) {
        const chars = style === 'retro-neon'
            ? { tl: '╔', tr: '╗', bl: '╚', br: '╝', h: '═', v: '║' }
            : { tl: '┌', tr: '┐', bl: '└', br: '┘', h: '─', v: '│' };
     ...
     * @private
     */
    renderItem(item, selected, enabled, style) {
        let output = '';
        // Selection indicator
        if (selected) {
     ...
     * @private
     */
    async handleInput(key, door, userId) {
        const visibleItems = this.items.filter((item) => item.visible);
        if (key === 'ArrowUp' || key === 'w' || key === 'W') {
            ...
            // Check hotkeys
            const hotkeyItem = visibleItems.find((item) => item.key && item.key.toLowerCase() === key.toLowerCase());
