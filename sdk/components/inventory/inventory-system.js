 ...
 */
class InventorySystem extends events_1.EventEmitter {
    constructor(config = {}) {
        super();
        this.items = new Map();
    ...
    findItemsByName(name) {
        ...
        return this.getAllItems().filter(item => item.name.toLowerCase().includes(lowerName));
    }
    /**
    ...
    sortItems(property, descending = false) {
        ...
        items.sort((a, b) => {
            let aVal = property in a ? a[property] : a.properties[property];
            let bVal = property in b ? b[property] : b.properties[property];
            if (aVal < bVal)
     ...
     */
    createSlot(name, capacity = 1, allowedTypes) {
