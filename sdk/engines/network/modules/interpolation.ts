 ...
 */
export class InterpolationEngine extends EventEmitter implements IInterpolationEngine {
  readonly name = 'interpolation';

  private config: Required<InterpolationConfig>;
  private entities: Map<string, EntityBuffer> = new Map();
   ...
   */
  private cubicInterpolate(snapshots: EntitySnapshot[], time: number): Vector3 {
   ...
   */
  private hermiteInterpolate(before: EntitySnapshot, after: EntitySnapshot, t: number): Vector3 {
   ...
   */
  private catmullRomInterpolate(snapshots: EntitySnapshot[], time: number): Vector3 {
   ...
   */
  getBufferStats(entityId: string): { count: number; oldestMs: number; newestMs: number } | null {
    const buffer = this.entities.get(entityId);
    if (!buffer || buffer.snapshots.length === 0) {
      return null;
    }
