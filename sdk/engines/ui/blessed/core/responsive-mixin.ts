...
import { TouchGestureHandler, SwipeOptions, LongPressOptions } from './touch-gestures';
...
export class ResponsiveBehavior {
  ...
  private breakpointHandlers: Set<BreakpointChangeHandler> = new Set();
  private resizeHandlers: Set<ResizeHandler> = new Set();
  ...
  initialize(): void {
    ...
    if (screen.responsiveLayout && this.options.responsive) {
      this.unsubscribeResize = screen.responsiveLayout.onResize(
        this.handleScreenResize.bind(this)
      );
    }
   ...
   */
  protected handleBreakpointChange(
    breakpoint: BreakpointName,
    previousBreakpoint: BreakpointName
  ): void {
  ...
  private handleScreenResize(width: number, height: number): void {
  ...
  private applyTouchFriendlySizing(): void {
    ...
    if (this.element.options) {
      this.element.options.height = enforceMinTouchHeight(
        this.element.options.height,
        true
      );
    }
  }
 ...
 */
export function applyResponsiveMixin(
  element: any,
  ...
  element.onBreakpointChange = (handler: BreakpointChangeHandler) =>
    behavior.onBreakpointChange(handler);
  element.enableSwipe = (swipeOptions: SwipeOptions) =>
    behavior.enableSwipe(swipeOptions);
  element.enableLongPress = (lpOptions: LongPressOptions) =>
    behavior.enableLongPress(lpOptions);
 ...
 */
export function hasResponsiveBehavior(element: any): boolean {
 ...
 */
export function getResponsiveBehavior(element: any): ResponsiveBehavior | undefined {
  return element._responsiveBehavior;
}
