...
import { ServerDoor } from '../server';
import { BBSUser, KeyEvent } from '../common';
 ...
 */
export async function runDoorWithSession(door: ServerDoor, doorSession: DoorSession): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const cleanup = () => {
      ...
      (door as any).off('output', handleDoorOutput);
      door.pause();
    };
    ...
    const handleDoorOutput = (payload: { user: BBSUser; text: string } | string) => {
    ...
    (door as any).on('output', handleDoorOutput);
    ...
    door.onDisconnect((user: BBSUser) => {
...
function mapUser(session: DoorSession): BBSUser {
...
const SPECIAL_KEYS: Record<string, { key: string; code: number }> = {
...
function decodeInput(data: string): KeyEvent[] {
...
function translateCSI(seq: string): { key: string; code: number } | null {
  ...
  if (final === '~') {
    if (content === '2') return { key: 'Insert', code: 45 };
    if (content === '3') return { key: 'Delete', code: 46 };
    if (content === '5') return { key: 'PageUp', code: 33 };
    if (content === '6') return { key: 'PageDown', code: 34 };
  } else if (final === 'F') {
...
function logChunk(prefix: string, text: string): void {
