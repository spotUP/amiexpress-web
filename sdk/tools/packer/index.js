...
exports.main = exports.ReleasePacker = void 0;
...
class ReleasePacker {
     ...
     */
    async pack() {
        const outputFile = path.join(this.config.outputDir, `${this.config.name}-v${this.config.version}.zip`);
...
`.trim();
     ...
     */
    formatDescriptionForNfo(desc) {
        const lines = this.wordWrap(desc, 72);
        return lines.map((line) => `║  ${this.padRight(line, 74)}║`).join('\r\n');
    }
}
exports.ReleasePacker = ReleasePacker;
/**
 * CLI entry point
 */
async function main(doorName) {
