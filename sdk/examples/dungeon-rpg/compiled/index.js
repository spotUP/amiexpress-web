...
class DungeonRPG {
    constructor() {
        ...
        this.physics = new bbs_door_sdk_1.PhysicsEngine({ gravity: 0 });
        this.audio = new bbs_door_sdk_1.AudioEngine();
        this.ai = new bbs_door_sdk_1.AIEngine({ updateInterval: 200 });
        this.input = new bbs_door_sdk_1.InputEngine();
        this.levelMgr = new bbs_door_sdk_1.LevelManager();
        this.saveMgr = new bbs_door_sdk_1.SaveManager({ userId: 0, gameId: 'dungeon-rpg' });
        this.inventory = new bbs_door_sdk_1.InventorySystem({ capacity: 20, maxWeight: 100 });
        this.dialogue = new bbs_door_sdk_1.DialogueSystem();
        this.quests = new bbs_door_sdk_1.QuestSystem();
...
####################
    `.trim(), {
            '#': { type: 'wall', solid: true, char: '#', color: bbs_door_sdk_1.AnsiColor.White },
            '.': { type: 'floor', solid: false, char: '.', color: bbs_door_sdk_1.AnsiColor.Black },
            'S': { type: 'spawn', solid: false, char: '.', color: bbs_door_sdk_1.AnsiColor.Black },
            'E': { type: 'enemy', solid: false, char: '.', color: bbs_door_sdk_1.AnsiColor.Black },
            'N': { type: 'npc', solid: false, char: '.', color: bbs_door_sdk_1.AnsiColor.Black },
            'X': { type: 'exit', solid: false, char: '.', color: bbs_door_sdk_1.AnsiColor.Black }
        });
    ...
    setupDoorEvents() {
        this.door.onConnect(async (user) => {
            this.userId = user.id;
            this.saveMgr = new bbs_door_sdk_1.SaveManager({ userId: user.id, gameId: 'dungeon-rpg' });
    ...
    showTitle() {
        ...
        this.gfx.drawText(30, 8, '╔════════════════╗', bbs_door_sdk_1.AnsiColor.Cyan);
        this.gfx.drawText(30, 9, '║ DUNGEON  RPG  ║', bbs_door_sdk_1.AnsiColor.Cyan);
        this.gfx.drawText(30, 10, '╚════════════════╝', bbs_door_sdk_1.AnsiColor.Cyan);
        this.gfx.drawText(20, 14, 'Complete SDK Example - All Features!', bbs_door_sdk_1.AnsiColor.Yellow);
        this.gfx.drawText(30, 17, 'Press any key...', bbs_door_sdk_1.AnsiColor.White);
    ...
    render() {
        ...
        for (const npc of this.npcs.values()) {
            this.gfx.drawChar(npc.x, npc.y, '@', bbs_door_sdk_1.AnsiColor.Cyan);
        }
        // Render enemies
        for (const enemy of this.enemies.values()) {
            this.gfx.drawChar(enemy.x, enemy.y, 'G', bbs_door_sdk_1.AnsiColor.Red);
        }
        // Render player
        this.gfx.drawChar(this.player.x, this.player.y, '@', bbs_door_sdk_1.AnsiColor.Yellow);
        // Render HUD
        this.hud.reset();
        ...
        // Controls help
        this.gfx.drawText(42, 6, 'Controls:', bbs_door_sdk_1.AnsiColor.White);
        this.gfx.drawText(42, 7, 'Arrows: Move', bbs_door_sdk_1.AnsiColor.White);
        this.gfx.drawText(42, 8, 'Space: Attack', bbs_door_sdk_1.AnsiColor.White);
        this.gfx.drawText(42, 9, 'I: Inventory', bbs_door_sdk_1.AnsiColor.White);
        this.gfx.drawText(42, 10, 'Q: Quests', bbs_door_sdk_1.AnsiColor.White);
        this.gfx.drawText(42, 11, 'T: Talk', bbs_door_sdk_1.AnsiColor.White);
        this.gfx.drawText(42, 12, 'S: Save', bbs_door_sdk_1.AnsiColor.White);
        this.door.sendAnsi(this.gfx.render(), this.userId);
    }
    movePlayer(dx, dy) {
    ...
    showDialogue() {
        ...
        this.gfx.clear(bbs_door_sdk_1.AnsiColor.Black);
        this.gfx.drawText(5, 5, `${node.speaker}:`, bbs_door_sdk_1.AnsiColor.Cyan);
        this.gfx.drawText(5, 7, node.text, bbs_door_sdk_1.AnsiColor.White);
        const choices = this.dialogue.getAvailableChoices();
        choices.forEach((choice, i) => {
            this.gfx.drawText(5, 10 + i, `${i + 1}. ${choice.text}`, bbs_door_sdk_1.AnsiColor.Yellow);
        });
        this.door.sendAnsi(this.gfx.render(), this.userId);
    ...
    showInventory() {
        ...
        this.inMenu = true;
        this.gfx.clear(bbs_door_sdk_1.AnsiColor.Black);
        this.gfx.drawText(5, 2, '=== INVENTORY ===', bbs_door_sdk_1.AnsiColor.Cyan);
        const items = this.inventory.getAllItems();
        items.forEach((item, i) => {
            this.gfx.drawText(5, 4 + i, `${item.name} x${item.quantity}`, bbs_door_sdk_1.AnsiColor.White);
        });
        this.gfx.drawText(5, 15, 'Press any key to close', bbs_door_sdk_1.AnsiColor.Yellow);
        this.door.sendAnsi(this.gfx.render(), this.userId);
    ...
    showQuests() {
        ...
        this.inMenu = true;
        this.gfx.clear(bbs_door_sdk_1.AnsiColor.Black);
        this.gfx.drawText(5, 2, '=== QUESTS ===', bbs_door_sdk_1.AnsiColor.Cyan);
        const quests = this.quests.getActiveQuests();
        let y = 4;
        quests.forEach(quest => {
            this.gfx.drawText(5, y++, quest.name, bbs_door_sdk_1.AnsiColor.Yellow);
            quest.objectives.forEach(obj => {
                const progress = `${obj.progress}/${obj.target}`;
                this.gfx.drawText(7, y++, `${obj.description}: ${progress}`, bbs_door_sdk_1.AnsiColor.White);
            });
            y++;
        });
        this.gfx.drawText(5, 18, 'Press any key to close', bbs_door_sdk_1.AnsiColor.Yellow);
        this.door.sendAnsi(this.gfx.render(), this.userId);
    ...
    showMessage(text) {
        ...
        this.gfx.drawText(42, 14, '─'.repeat(38), bbs_door_sdk_1.AnsiColor.White);
        ...
        lines.slice(0, 3).forEach((line, i) => {
            this.gfx.drawText(42, 15 + i, line.padEnd(maxWidth), bbs_door_sdk_1.AnsiColor.Yellow);
        });
        this.door.sendAnsi(this.gfx.render(), this.userId);
        ...
        setTimeout(() => {
            if (this.userId) {
                for (let i = 0; i < 4; i++) {
                    this.gfx.drawText(42, 14 + i, ' '.repeat(38), bbs_door_sdk_1.AnsiColor.Black);
                }
            }
        }, 3000);
    }
