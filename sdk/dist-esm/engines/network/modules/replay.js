...
export class ReplaySystem extends EventEmitter {
    ...
    setupEventHandlers() {
        ...
        if (!socket)
            return;
        socket.on('replay:loaded', (replay) => {
            this.currentReplay = replay;
            this.emit('replay:loaded', replay);
        });
        socket.on('replay:saved', (metadata) => {
            this.emit('replay:saved', metadata);
        });
        socket.on('replay:shared', (data) => {
     ...
     */
    startRecording() {
        if (!this.config.enabled || this._isRecording)
            return;
     ...
     */
    setGameInfo(gameId, gameMode, players) {
        this.gameId = gameId;
    ...
    async loadReplay(replayId) {
        return new Promise((resolve, reject) => {
            ...
            socket.emit('replay:load', { replayId }, (response) => {
     ...
     */
    async saveReplayToStorage(replay) {
     ...
     */
    play() {
        if (!this.currentReplay || this.currentReplay.frames.length === 0)
            return;
        if (!this._playback) {
            this._playback = {
     ...
     */
    resume() {
        if (!this._playback || !this._playback.isPaused)
            return;
        this._playback.isPaused = false;
        this.startPlaybackLoop();
     ...
     */
    seek(tick) {
        if (!this._playback || !this.currentReplay)
            return;
     ...
     * Get frames in range
     */
    getFramesInRange(fromTick, toTick) {
        if (!this.currentReplay)
            return [];
    ...
    async getReplayList(limit = 20) {
        return new Promise((resolve) => {
            ...
            socket.emit('replay:list', { limit }, (response) => {
    ...
    startPlaybackLoop() {
        ...
        this.playbackTimer = setInterval(() => {
            if (!this._playback || !this.currentReplay || this._playback.isPaused)
                return;
