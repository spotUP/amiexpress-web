...
| D16 | Restricted file rejection: comment starts `Restricted` → `\t\tAttempt to download RESTRICTED file [<path>]` to callersLog + skip — express.e checklist path | UNVERIFIED — `findFilesInConference` may not honor "Restricted" prefix in description | Medium (admins flag restricted files via description prefix; if ignored, security gap) | Audit `findFilesInConference` / `checkForFileSize` |
