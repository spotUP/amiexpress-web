...
| `caller_activity` / callersLog | `callersLog()` SQL only @ `database-helpers.ts:20` | BBS:Node{X}/CallersLog written by `callersLogManager.*` | **RESOLVED 2026-05-20** — bifurcated loggers stay (SQL = web activity widget, disk = express.e parity), but the 3 call sites that were SQL-only now also write to disk: `conference.handler.ts:308` (Joined conference), `file.handler.ts:229` (Deleted file), `file.handler.ts:360` (Moved file). 5 grep-style regression tests in `tests/callers-log-dual-write.test.ts` pin the dual-write shape. Other call sites (login/logoff, door exec, password failure, post-upload, post-download) were already dual-writing correctly. |
