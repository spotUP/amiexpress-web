...
- **`$02` followed by one of the 16 standard PETSCII colour bytes** (`$90 $05 $1C $9F $9C $1E $1F $9E $81 $95 $96 $97 $98 $99 $9A $9B`, the "PETSCII code" column above) sets the **background AND the border** to that VIC colour. The two are always tied - no independent border control exists in the convention. **`$02` followed by anything else: THIS PROJECT'S CHOSEN BEHAVIOUR, not a sourced CCGMS one.** No CCGMS/PyCGMS source was read for the non-colour case; we make the `$02` inert and process the following byte normally, so a stray `$02` in art or door output can never eat the control code behind it. If a CCGMS source is ever consulted and disagrees, this is the line to change (`PetsciiMachine.apply`, `takeBackgroundColor` in the backend converters).
...
- **Clients without the convention.** On SyncTERM's C64 mode and sblendorio the `$02` is inert and the colour byte that follows lands as a PEN change - ink corruption for the rest of the screen. `AnsiToPetsciiTransducer` therefore re-asserts the current pen immediately after every `$02 <colour>` it sends: a no-op on the oracle and on any client that does implement the convention, and the repair on any client that does not.

`PetsciiMachine` implements both codes (`background`/`border` mutate together, and a change reports a full repaint since the whole canvas re-paints). `AnsiToPetsciiTransducer` sends a screen background at exactly one moment - a full clear (`ESC[2J`, `ESC[?1049h`/`?47h`, `ESC c`) - emitting `$93` then `$02 <colour>` for the ANSI background SGR in force (40-47 / 100-107 / 48;5 / 48;2), and re-sends it after any `$0E` it emits while that background stands. **No ANSI background in force means SGR 0 / SGR 49, which is the BBS asking for the terminal default - black - not "leave the screen alone":** the clear commits black, so a colour can never be stranded on the screen with nothing tracking it for a later unrelated `$0E` to drop mid-art. Nothing is emitted when the screen is already black, so the common case costs zero bytes. Per-cell ANSI backgrounds remain dropped: C64 colour RAM stores ink only.

The reverse direction (`web/backend/src/utils/petscii.util.ts`, PETSCII art -> xterm for the non-canvas web paths) consumes `$02 <colour>` too, rendering it as the background SGR `ESC[48;2;r;g;bm` from the same palette, and `$0E` emits `ESC[49m` when - and only when - such a background is in force.
