...
export class BubbleBobbleGame {
  ...
  private handlePlatformCollision(entity: { x: number; y: number; vy: number; isOnGround?: boolean }): void {
    ...
    for (const wall of this.data.walls) {
      if (Math.abs(entity.x - wall.x) < 1 && Math.abs(entity.y - wall.y) < 1) {
        // Push back
        if (entity.x < wall.x) entity.x = wall.x - 1;
        else entity.x = wall.x + 1;
      }
    }
  }
  ...
  private updateEnemies(): void {
    for (const enemy of this.data.enemies) {
      ...
      // Screen wrapping
      if (enemy.x < 0) enemy.x = GAME_WIDTH - 1;
      if (enemy.x >= GAME_WIDTH) enemy.x = 0;
  ...
  private checkBubbleEnemyCollision(bubble: Bubble): Enemy | null {
    for (const enemy of this.data.enemies) {
      if (enemy.state !== 'normal' && enemy.state !== 'angry') continue;

      const dist = Math.sqrt(
        Math.pow(bubble.x - enemy.x, 2) + Math.pow(bubble.y - enemy.y, 2)
      );
  ...
  private checkCollisions(): void {
    ...
    for (const bubble of this.data.bubbles) {
      if (bubble.state === 'popping') continue;

      const dist = Math.sqrt(
        Math.pow(player.x - bubble.x, 2) + Math.pow(player.y - bubble.y, 2)
      );
    ...
    if (player.invincibleTimer <= 0) {
      for (const enemy of this.data.enemies) {
        if (enemy.state === 'bubbled' || enemy.state === 'dead') continue;
  ...
  private popEnemyBubble(bubble: Bubble): void {
  ...
  private spawnItem(x: number, y: number): void {
    ...
    if (rand < 0.4) itemType = 'apple';
    else if (rand < 0.6) itemType = 'orange';
    else if (rand < 0.75) itemType = 'cherry';
    else if (rand < 0.85) itemType = 'grape';
    else if (rand < 0.92) itemType = 'watermelon';
    else if (rand < 0.96) {
      // Power-ups
      const powerups: ItemType[] = ['shoe', 'candy', 'umbrella'];
      itemType = powerups[Math.floor(Math.random() * powerups.length)];
    } else if (rand < 0.99) itemType = 'crown';
    else itemType = 'diamond';
  ...
  render(): void {
    ...
    for (const enemy of this.data.enemies) {
      if (enemy.state === 'dead') continue;
      if (enemy.state === 'bubbled') continue; // Drawn as bubble
      ...
      for (let x = 0; x < GAME_WIDTH; x++) {
        ...
        if (char === SPRITES.platform) {
          ...
          const enemyHere = this.data.enemies.find(e =>
            Math.floor(e.x) === x && Math.floor(e.y) === y && e.state !== 'dead' && e.state !== 'bubbled'
          );
          ...
          const itemHere = this.data.items.find(i =>
