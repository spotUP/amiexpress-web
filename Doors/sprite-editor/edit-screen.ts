...
import {
  Cell, Sprite, PALETTE, decompilePixels,
...
export class EditScreen {
  ...
  private buildLayout(): void {
    ...
    this.toolbar = createToolbar(this.screen, this.toolbarPanel, this.toolbarState, (next) => {
      this.tool = next.tool;
      this.fg = next.colour;
      this.paint();
    });
   ...
   */
  private applyToolAt(cellRow: number, cellCol: number): void {
    ...
    if (this.tool === 'paint') {
      this.tryOp(() => this.mode === 'pixel'
        ? setPixel(this.doc, pixelRow, cellCol, this.fg)
        : setCell(this.doc, cellRow, cellCol, { char: GLYPHS[this.glyph], fg: this.fg, bg: this.bg }));
    } else if (this.tool === 'erase') {
      this.tryOp(() => this.mode === 'pixel'
        ? setPixel(this.doc, pixelRow, cellCol, null)
        : setCell(this.doc, cellRow, cellCol, null));
    } else if (this.tool === 'fill') {
      this.tryOp(() => floodFill(this.doc, pixelRow, cellCol, this.fg));
    } else { // pick - reads a colour into ToolbarState, never touches the doc
      const picked = this.mode === 'pixel'
        ? (decompilePixels(currentFrame(this.doc)) ?? [])[pixelRow]?.[cellCol] ?? null
        : currentFrame(this.doc)[cellRow][cellCol]?.fg ?? null;
      if (picked !== null) this.fg = picked;
  ...
  private handleFramesClick(data: { x: number; y: number }): void {
    ...
    this.apply(selectFrame(this.doc, index));
  }

  private wireMouse(): void {
    this.canvasBox.on('click', (data: any) => this.handleCanvasClick(data));
    this.canvasBox.on('mousemove', (data: any) => this.handleCanvasDrag(data));
