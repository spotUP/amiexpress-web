...
import {
  themeStyles, themeById, attachDoorChrome,
  type Theme, type ThemeTokens, type ThemeStyles,
  type DoorChrome, type DoorChromeOptions, type FooterHint,
...
function stepAnsiZoom(current: number, delta: 1 | -1): number {
...
export class ANSIEditorDoor {
  ...
  private requesterChrome: DoorChrome | null = null;
  ...
  private async showBBSFileBrowser(): Promise<void> {
    ...
    this.attachRequesterChrome(
      'BBS SCREEN DIRECTORIES', dirList, hintRow,
      [{ key: 'Enter', does: 'Browse' }, { key: 'ESC', does: 'Cancel' }],
      [{ key: 'Ent', does: 'Browse' }, { key: 'ESC', does: 'Back' }]
    );
   ...
   */
  private async reopenEditorPreservingContent(): Promise<void> {
  ...
  private async showBBSDirectoryBrowser(directory: string, label: string): Promise<void> {
    ...
    this.attachRequesterChrome(
      label.toUpperCase(), fileList, hintRow,
      [{ key: 'Enter', does: 'Open' }, { key: 'B', does: 'Back' }, { key: 'ESC', does: 'Cancel' }],
      [{ key: 'Ent', does: 'Open' }, { key: 'B', does: 'Back' }, { key: 'ESC', does: 'Bye' }]
    );
   ...
   */
  private chromeFor(
    options: Omit<DoorChromeOptions, 'width' | 'styles' | 'render'>
  ): DoorChrome {
    return attachDoorChrome(THEME, {
      ...options,
  ...
  private closeRequesterChrome(): void {
  ...
  private async showFileBrowser(): Promise<void> {
    ...
    this.attachRequesterChrome(
      'YOUR ANSI FILES', fileList, hintRow,
      [{ key: 'Enter', does: 'Open' }, { key: 'D', does: 'Delete' }, { key: 'ESC', does: 'Cancel' }],
      [{ key: 'Ent', does: 'Open' }, { key: 'D', does: 'Del' }, { key: 'ESC', does: 'Bye' }]
    );
  ...
  private async openEditor(initialContent: string): Promise<void> {
      ...
      extraMenus: [
        {
          ...
          items: ANSI_ZOOM_STEPS.map(z => ({
            ...
            this.terminalMode!.menuItem() as any,
          ]),
        },
      ],
    ...
    this.chrome = this.chromeFor({
      title: editorTitle,
      masthead: { setContent: (text: string) => editor.setLabel(text) },
