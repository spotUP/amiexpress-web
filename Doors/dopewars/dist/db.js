...
exports.getPlayersAt = getPlayersAt;
...
exports.upsertCombat = upsertCombat;
exports.deleteCombat = deleteCombat;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
function openDb(doorDir) {
    const dataDir = path.join(doorDir, 'data');
    fs.mkdirSync(dataDir, { recursive: true });
    const db = new better_sqlite3_1.default(path.join(dataDir, 'dopewars.db'));
...
function upsertPlayer(db, state) {
    db.prepare(`
...
function getPlayersAt(db, location) {
    return db.prepare(`
    SELECT id, bbs_handle AS name, location, health, turn
    FROM dw_players WHERE active=1 AND location=?
  `).all(location);
}
function deactivatePlayer(db, id) {
    db.prepare(`UPDATE dw_players SET active=0, last_seen=CURRENT_TIMESTAMP WHERE id=?`).run(id);
}
function getInventory(db, playerId) {
    return db.prepare(`SELECT * FROM dw_inventory WHERE player_id=?`).all(playerId);
}
function insertHighScore(db, playerId, bbsHandle, score, turns) {
    db.prepare(`INSERT INTO dw_high_scores (player_id, bbs_handle, score, turns) VALUES (?,?,?,?)`).run(playerId, bbsHandle, score, turns);
}
function getHighScores(db, limit = 20) {
    return db.prepare(`
...
function upsertCombat(db, playerId, data) {
    db.prepare(`
  ...
  `).run(playerId, data.opponentId ?? null, data.copIndex ?? null, data.numDeputies ?? null, JSON.stringify(data.fightArray));
}
function deleteCombat(db, playerId) {
