...
export class DopewarsServer extends EventEmitter {
  ...
  async init(doorDir: string, cfg: DopewarsConfig): Promise<void> {
    ...
    for (const row of getActivePlayers(this.db)) {
      ...
      this.wasm.restorePlayerState(
        idx,
        row.cash ?? 0, row.debt ?? 0, row.bank ?? 0,
        row.health ?? 100, row.coat_size ?? 100,
        row.location ?? 0, row.turn ?? 0
      );
  ...
  async getMarket(id: string): Promise<MarketState> {
    ...
    const drugNames: Record<number, string> = {};
    for (const p of raw.prices) drugNames[p.index] = p.name;
