...
var Context = class _Context extends BaseContext {
  constructor() {
    ...
    this.isOffline = false;
    this._workletPromises = /* @__PURE__ */ new Set();
...
var connectedSignals = /* @__PURE__ */ new WeakMap();
...
function disconnectSignal(signal, destination, outputNum, inputNum) {
  if (destination instanceof Param || isAudioParam(destination) || destination instanceof Signal && destination.override || destination === void 0) {
    if (connectedSignals.has(signal)) {
      let connections = connectedSignals.get(signal);
      if (destination) {
        connections = connections.filter((conn) => {
          return conn.destination === destination && (isUndef(outputNum) || conn.outputNum === outputNum) && (isUndef(inputNum) || conn.inputNum === inputNum);
        });
      }
      ...
      connections.forEach((connection) => {
        if (connection.destination instanceof Signal || connection.destination instanceof Param) {
          connection.destination.overridden = false;
        }
        connection.destination.setValueAtTime(connection.previousValue, 0);
      });
      connectedSignals.set(signal, connectedSignals.get(signal).filter((conn) => !connections.includes(conn)));
    }
  }
...
onContextInit((context2) => {
  context2.listener = new ListenerInstance({ context: context2 });
});
...
var ToneAudioWorklet = class _ToneAudioWorklet extends ToneAudioNode {
  constructor(options) {
    ...
    let workletPromise = _ToneAudioWorklet._workletPromises.get(this.context);
    if (workletPromise === void 0) {
      workletPromise = this.context.addAudioWorkletModule(blobUrl);
      _ToneAudioWorklet._workletPromises.set(this.context, workletPromise);
    }
    workletPromise.then(() => {
      if (!this.disposed) {
...
onContextInit((context2) => {
  context2.destination = new DestinationInstance({ context: context2 });
});
onContextClose((context2) => {
...
onContextInit((context2) => {
  context2.draw = new DrawInstance({ context: context2 });
});
onContextClose((context2) => {
...
onContextInit((context2) => {
  context2.transport = new TransportInstance({ context: context2 });
});
onContextClose((context2) => {
...
var Vibrato = class _Vibrato extends Effect {
  constructor() {
    ...
    this._lfo = new LFO({
      ...
      frequency: options.frequency,
      phase: -90
      // offset the phase so the resting position is in the center
...
function* PatternGenerator(numValues, pattern = "up") {
...
var Sampler = class _Sampler extends Instrument {
  ...
  triggerAttack(notes, time, velocity = 1) {
    ...
    notes.forEach((note) => {
      ...
      const duration = this._loop ? void 0 : buffer.duration / playbackRate;
...
var Player = class _Player extends Source {
  ...
  _getProgressAtTime(time) {
    ...
    if (this._loop) {
      const loopEnd = this.loopEnd === 0 ? this.buffer.duration : this.toSeconds(this.loopEnd);
      const loopStart = this.toSeconds(this.loopStart);
  ...
  get progress() {
    const now2 = this.now();
    return this._getProgressAtTime(now2);
  }
  /**
  ...
  set playbackRate(rate) {
    this._playbackRate = rate;
    const now2 = this.now();
    this._progressTracker.setValueAtTime(rate, now2);
    const stopEvent = this._state.getNextState("stopped", now2);
    if (stopEvent && stopEvent.implicitEnd) {
      this._state.cancel(stopEvent.time);
      this._activeSources.forEach((source) => source.cancelStop());
      const progress = this._getProgressAtTime(now2);
      const remainingTime = this._buffer.duration - progress;
      const newStopTime = now2 + remainingTime / rate;
...
var AudioEngine = class {
  constructor(config = {}, socket) {
    ...
    this.sfxEcho = null;
    this.sfxSend = null;
    this.soundLibrary = /* @__PURE__ */ new Map();
    this.socket = null;
  ...
  async init() {
    ...
    if (isBrowser) {
      ...
      if (!this.masterGain || !this.masterGain.gain) {
        ...
        this.sfxGain = new Gain(this.config.sfxVolume);
        this.sfxGain.connect(this.masterGain);
        const reverb = this.config.sfxReverb;
        const echo = this.config.sfxEcho;
        const sendLevel = Math.max(reverb?.wet ?? 0, echo?.wet ?? 0);
        if (sendLevel > 0) {
          this.sfxSend = new Gain(sendLevel).connect(this.masterGain);
          let sendDestination = this.sfxSend;
          if (reverb && reverb.wet > 0) {
            this.sfxReverb = new Reverb({
          ...
          if (echo && echo.wet > 0) {
            this.sfxEcho = new FeedbackDelay({
  ...
  buildSoundLibrary() {
    ...
    this.soundLibrary.set("click", {
      synth: new Synth({
        ...
        envelope: { attack: 1e-3, decay: 0.03, sustain: 0, release: 0.02 }
    ...
    this.soundLibrary.set("error", oneShot(() => {
      ...
      synth.triggerAttackRelease("C3", 0.08, now());
      synth.triggerAttackRelease("B2", 0.12, now() + 0.08);
    ...
    this.soundLibrary.set("success", oneShot(() => {
      ...
      synth.triggerAttackRelease("G5", 0.08, now());
      synth.triggerAttackRelease("C6", 0.15, now() + 0.1);
    ...
    this.soundLibrary.set("typing", {
      synth: new NoiseSynth({
        noise: { type: "white" },
        envelope: { attack: 1e-3, decay: 0.015, sustain: 0, release: 0.01 }
      }).connect(this.sfxGain),
      pattern: () => {
    ...
    this.soundLibrary.set("confirm", oneShot(() => {
      ...
      synth.triggerAttackRelease("G5", 0.1, now() + 0.12);
      setTimeout(() => synth.dispose(), 400);
    }));
    this.soundLibrary.set("cancel", oneShot(() => {
      ...
      synth.triggerAttackRelease("C4", 0.1, now() + 0.08);
    ...
    this.soundLibrary.set("sword-swing", oneShot(() => {
      ...
      const filter = new Filter(800, "bandpass").connect(this.sfxGain);
      noise.connect(filter);
      filter.frequency.rampTo(2e3, 0.1);
    ...
    this.soundLibrary.set("arrow", oneShot(() => {
      ...
      synth.volume.value = -8;
      synth.frequency.setValueAtTime(2e3, now());
      synth.frequency.exponentialRampTo(800, 0.15, now());
      synth.triggerAttackRelease(2e3, 0.15);
      setTimeout(() => synth.dispose(), 300);
    }));
    this.soundLibrary.set("magic-cast", oneShot(() => {
      const synth = new PolySynth(Synth).connect(this.sfxGain);
      synth.set({
        oscillator: { type: "sine" },
        envelope: { attack: 0.05, decay: 0.3, sustain: 0.1, release: 0.4 }
      });
      synth.volume.value = -6;
      synth.triggerAttackRelease(["C5", "E5", "G5"], 0.2, now());
      synth.triggerAttackRelease(["D5", "F#5", "A5"], 0.2, now() + 0.1);
      synth.triggerAttackRelease(["E5", "G#5", "B5"], 0.3, now() + 0.2);
      setTimeout(() => synth.dispose(), 800);
    }));
    this.soundLibrary.set("shield-block", oneShot(() => {
      const metal = new MetalSynth({
        envelope: { attack: 1e-3, decay: 0.15, release: 0.1 },
        harmonicity: 5.1,
        modulationIndex: 32,
        resonance: 4e3,
        octaves: 1.5
      ...
      metal.triggerAttackRelease(0.15, now());
    ...
    this.soundLibrary.set("critical-hit", oneShot(() => {
      ...
      const synth = new Synth({
        ...
        envelope: { attack: 1e-3, decay: 0.2, sustain: 0, release: 0.1 }
      }).connect(this.sfxGain);
      synth.volume.value = -6;
    ...
    this.soundLibrary.set("punch", oneShot(() => {
      const membrane = new MembraneSynth({
        ...
        octaves: 6,
        oscillator: { type: "sine" },
        envelope: { attack: 1e-3, decay: 0.15, sustain: 0, release: 0.1 }
      }).connect(this.sfxGain);
    ...
    this.soundLibrary.set("slash", oneShot(() => {
      const noise = new NoiseSynth({
        noise: { type: "white" },
        envelope: { attack: 1e-3, decay: 0.06, sustain: 0, release: 0.03 }
      }).connect(this.sfxGain);
      const filter = new Filter(3e3, "highpass").connect(this.sfxGain);
      noise.connect(filter);
    ...
    this.soundLibrary.set("parry", oneShot(() => {
      const metal = new MetalSynth({
        envelope: { attack: 1e-3, decay: 0.08, release: 0.05 },
        harmonicity: 3,
        modulationIndex: 16,
        resonance: 3e3,
        octaves: 1
      ...
      metal.triggerAttackRelease(0.08, now());
    ...
    const explosionSynth = new NoiseSynth({
      noise: { type: "pink" },
    ...
    this.soundLibrary.set("pickup", oneShot(() => {
      ...
      synth.triggerAttackRelease("E5", 0.05, now());
      synth.triggerAttackRelease("A5", 0.1, now() + 0.05);
      setTimeout(() => synth.dispose(), 300);
    }));
    this.soundLibrary.set("drop", oneShot(() => {
      ...
      synth.triggerAttackRelease("D4", 0.12, now() + 0.08);
      setTimeout(() => synth.dispose(), 350);
    }));
    this.soundLibrary.set("equip", oneShot(() => {
      const metal = new MetalSynth({
        envelope: { attack: 1e-3, decay: 0.1, release: 0.05 },
        harmonicity: 2,
        modulationIndex: 8,
        resonance: 2e3,
        octaves: 1
      ...
      metal.triggerAttackRelease(0.1, now());
    ...
    this.soundLibrary.set("potion-drink", oneShot(() => {
      ...
      synth.volume.value = -8;
      synth.triggerAttackRelease("G3", 0.1, now());
      synth.triggerAttackRelease("C4", 0.1, now() + 0.12);
      synth.triggerAttackRelease("E4", 0.15, now() + 0.24);
    ...
    this.soundLibrary.set("chest-open", oneShot(() => {
      ...
      synth.triggerAttackRelease(["C5", "E5", "G5"], 0.3, now() + 0.2);
      setTimeout(() => {
        noise.dispose();
        synth.dispose();
      }, 800);
    ...
    this.soundLibrary.set("gold-collect", oneShot(() => {
      const synth = new Synth({
        oscillator: { type: "sine" },
        envelope: { attack: 1e-3, decay: 0.08, sustain: 0.02, release: 0.08 }
      }).connect(this.sfxGain);
      synth.triggerAttackRelease("E6", 0.04, now());
      synth.triggerAttackRelease("A6", 0.08, now() + 0.04);
    ...
    this.soundLibrary.set("land", oneShot(() => {
      const membrane = new MembraneSynth({
        ...
        octaves: 4,
        oscillator: { type: "sine" },
        envelope: { attack: 1e-3, decay: 0.1, sustain: 0, release: 0.05 }
      }).connect(this.sfxGain);
    ...
    this.soundLibrary.set("dash", oneShot(() => {
      ...
      const filter = new Filter(1e3, "bandpass").connect(this.sfxGain);
      noise.connect(filter);
      noise.volume.value = -8;
      filter.frequency.rampTo(3e3, 0.08);
    ...
    this.soundLibrary.set("teleport", oneShot(() => {
      ...
      synth.frequency.exponentialRampTo(2e3, 0.2, now());
    ...
    this.soundLibrary.set("swim", oneShot(() => {
      ...
      const filter = new Filter(600, "lowpass").connect(this.sfxGain);
      noise.connect(filter);
    ...
    this.soundLibrary.set("door-open", oneShot(() => {
      ...
      synth.frequency.linearRampTo(120, 0.3, now());
    ...
    this.soundLibrary.set("alarm", oneShot(() => {
      ...
      synth.triggerAttackRelease("A4", 0.15, now() + 0.3);
      synth.triggerAttackRelease("E5", 0.15, now() + 0.45);
      setTimeout(() => synth.dispose(), 800);
    }));
    this.soundLibrary.set("card-deal", oneShot(() => {
      ...
      const filter = new Filter(2e3, "highpass").connect(this.sfxGain);
      noise.connect(filter);
    ...
    this.soundLibrary.set("card-flip", oneShot(() => {
      const noise = new NoiseSynth({
        noise: { type: "white" },
        envelope: { attack: 1e-3, decay: 0.08, sustain: 0, release: 0.03 }
      }).connect(this.sfxGain);
      const synth = new Synth({
        oscillator: { type: "triangle" },
    ...
    this.soundLibrary.set("card-shuffle", oneShot(() => {
      ...
      const filter = new Filter(3e3, "lowpass").connect(this.sfxGain);
      noise.connect(filter);
      for (let i = 0; i < 8; i++) {
        setTimeout(() => {
          filter.frequency.value = 2e3 + Math.random() * 2e3;
          noise.triggerAttackRelease(0.02);
        }, i * 25 + Math.random() * 10);
      }
      setTimeout(() => {
    ...
    const flapSynth = new NoiseSynth({
      noise: { type: "white" },
      envelope: { attack: 1e-3, decay: 0.12, sustain: 0, release: 0.02 }
    }).connect(this.sfxGain);
    ...
    this.soundLibrary.set("chips-win", oneShot(() => {
      for (let i = 0; i < 6; i++) {
        setTimeout(() => {
          const metal = new MetalSynth({
            envelope: { attack: 1e-3, decay: 0.1, release: 0.08 },
            harmonicity: 2.5 + Math.random(),
            modulationIndex: 6,
            resonance: 1500 + Math.random() * 1e3,
            octaves: 1
          }).connect(this.sfxGain);
    ...
    this.soundLibrary.set("chips-pot", oneShot(() => {
      const metal = new MetalSynth({
        envelope: { attack: 1e-3, decay: 0.12, release: 0.06 },
        harmonicity: 2.8,
        modulationIndex: 10,
        resonance: 1800,
        octaves: 1.2
      ...
      metal.triggerAttackRelease(0.12, now());
      setTimeout(() => metal.dispose(), 300);
    }));
    ...
    this.soundLibrary.set("slot-spin", oneShot(() => {
      ...
      synth.frequency.exponentialRampTo(300, 0.3, now());
      synth.frequency.exponentialRampTo(80, 0.5, now() + 0.5);
      synth.triggerAttackRelease(100, 1);
    ...
    this.soundLibrary.set("jackpot", oneShot(() => {
      ...
      synth.volume.value = -6;
      synth.triggerAttackRelease(["C5", "E5"], 0.12, now());
      synth.triggerAttackRelease(["E5", "G5"], 0.12, now() + 0.12);
      synth.triggerAttackRelease(["G5", "C6"], 0.12, now() + 0.24);
      synth.triggerAttackRelease(["C5", "E5", "G5", "C6"], 0.4, now() + 0.4);
      setTimeout(() => synth.dispose(), 1200);
    }));
    this.soundLibrary.set("blip", {
      synth: new Synth({
        oscillator: { type: "square" },
        envelope: { attack: 1e-3, decay: 0.03, sustain: 0, release: 0.01 }
      }).connect(this.sfxGain),
      pattern: () => {
    ...
    this.soundLibrary.set("zap", oneShot(() => {
      ...
      synth.frequency.setValueAtTime(1500, now());
      synth.frequency.exponentialRampTo(100, 0.1, now());
    ...
    this.soundLibrary.set("warp", oneShot(() => {
      ...
      synth.frequency.setValueAtTime(100, now());
      synth.frequency.exponentialRampTo(3e3, 0.15, now());
      synth.frequency.exponentialRampTo(200, 0.1, now() + 0.15);
    ...
    this.soundLibrary.set("1up", oneShot(() => {
      ...
      synth.triggerAttackRelease("G5", 0.08, now() + 0.08);
      synth.triggerAttackRelease("E6", 0.08, now() + 0.16);
      synth.triggerAttackRelease("C6", 0.08, now() + 0.24);
      synth.triggerAttackRelease("D6", 0.08, now() + 0.32);
      synth.triggerAttackRelease("G6", 0.15, now() + 0.4);
      setTimeout(() => synth.dispose(), 800);
    }));
    this.soundLibrary.set("death", oneShot(() => {
      ...
      synth.volume.value = -6;
      synth.triggerAttackRelease("B4", 0.15, now());
      synth.triggerAttackRelease("G4", 0.15, now() + 0.15);
      synth.triggerAttackRelease("E4", 0.15, now() + 0.3);
      synth.triggerAttackRelease("C4", 0.4, now() + 0.45);
      setTimeout(() => synth.dispose(), 1200);
    }));
    ...
    this.soundLibrary.set("start", oneShot(() => {
      ...
      synth.triggerAttackRelease("G4", 0.1, now() + 0.1);
      synth.triggerAttackRelease("C5", 0.15, now() + 0.2);
      setTimeout(() => synth.dispose(), 500);
    }));
    this.soundLibrary.set("powerup", {
      ...
      pattern: () => {
        ...
        synth.triggerAttackRelease("C4", "0.1", now());
        synth.triggerAttackRelease("E4", "0.1", now() + 0.1);
        synth.triggerAttackRelease("G4", "0.1", now() + 0.2);
        synth.triggerAttackRelease("C5", "0.3", now() + 0.3);
      }
    });
    this.soundLibrary.set("gameover", {
      ...
      pattern: () => {
        ...
        synth.triggerAttackRelease("D4", "0.3", now() + 0.3);
        synth.triggerAttackRelease("C4", "0.3", now() + 0.6);
        synth.triggerAttackRelease("B3", "1.0", now() + 0.9);
      }
    });
    this.soundLibrary.set("level-up", oneShot(() => {
      ...
      synth.triggerAttackRelease("C6", 0.2, now() + 0.24);
      setTimeout(() => synth.dispose(), 700);
    }));
  ...
  playChord(notes, duration = "4n") {
    ...
    if (!this.config.enabled || !this.initialized)
      return;
    const synth = new PolySynth().connect(this.sfxGain);
    synth.triggerAttackRelease(notes, duration);
    setTimeout(() => synth.dispose(), 2e3);
  }
  /**
  ...
  playCustomSound(params) {
    ...
    synth.volume.value = gainToDb(params.volume);
    synth.triggerAttackRelease(params.frequency, params.duration);
    setTimeout(() => {
      synth.dispose();
    }, (params.duration + 1) * 1e3);
  }
  /**
  ...
  setMusicState(state, intensity = 0.5, transition = "crossfade") {
    ...
    if (transition === "immediate") {
      ...
      this.musicGain.gain.rampTo(targetVolume, 1);
    } else {
      this.musicGain.gain.rampTo(targetVolume, 2);
    }
    const baseTempo = 120;
  ...
  getAudioContext() {
    if (typeof esm_exports === "undefined" || !esm_exports?.getContext)
      return null;
...
var ClientDoor = class _ClientDoor extends EventEmitter {
  ...
  constructor(config) {
    ...
    this.targetFPS = 30;
    this.rpcRequests = /* @__PURE__ */ new Map();
    this.nextRpcId = 1;
    this.config = {
      minSecurity: 0,
      maxTime: 0,
  ...
  initialize() {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener(DOOR_UNLOAD_EVENT, (event) => {
        ...
        if (!doorId || doorId === this.config.name?.toLowerCase() || doorId === this.config.id) {
          this.shutdown();
        }
      });
    }
  ...
  mainLoop() {
    if (this.state !== "running")
      return;
    const now2 = Date.now();
    const delta = now2 - this.lastFrameTime;
   ...
   */
  onConnect(handler2) {
    this.on("connect", handler2);
  }
  /**
   * Handle user disconnection
   */
  onDisconnect(handler2) {
    this.on("disconnect", handler2);
  }
  /**
  ...
  async waitForInput(timeout = 0) {
    ...
    return new Promise((resolve) => {
      ...
      const handler2 = (data) => {
        ...
        this.off("input", handler2);
        resolve(data.key);
      };
      this.on("input", handler2);
      if (timeout > 0) {
...
var audio = new AudioEngine({
  masterVolume: 0.7,
  sfxVolume: 0.55,
  // Each filled pipe ticks, and the ticks come fast when the water runs.
  //
...
var stopSfx = null;
console.log("[Pipe Dream] Client door initializing...");
door.on("init", () => {
  console.log("[Pipe Dream] Client door init event");
});
...
console.log("[Pipe Dream] Starting client door...");
door.start();
