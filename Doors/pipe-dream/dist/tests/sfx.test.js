...
function emptyBoard() {
    ...
    const game = new PipeDreamGame(data, () => { }, () => { }, () => { });
    game.initLevel();
    data.state = 'playing';
...
export async function layingAndDiscardingSoundDifferent() {
...
export async function placingOnAnObstacleIsSilent() {
...
export async function movingTheCursorIsSilent() {
...
export async function theWaterStartingSounds() {
    const { game, data } = emptyBoard();
    data.flowDelay = 1;
    game.update();
...
export async function eachFilledPipeSounds() {
    const { game, data } = emptyBoard();
...
export async function theLeakSoundsGameOverAndNotADeath() {
    const { game, data } = emptyBoard();
    data.flowDelay = 0;
    game.update();
    game.cues.clear();
    // Nothing is connected to the start, so the water has nowhere to go.
    for (let i = 0; i < 500 && data.state !== 'gameover'; i++)
        game.update();
