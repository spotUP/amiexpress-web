...
import { CELL_WIDTH, FILLED_ABOVE, cellSprite, paint, asCursor, fillStage, BG_COLORS, PIPE_GLYPHS, } from '../game/sprites';
function visible(text) {
    return text.replace(/\{[^}]*\}/g, '');
}
const ALL_PIPES = Object.keys(PIPE_GLYPHS);
/** Every cell is exactly the grid's cell width - the grid depends on it. */
export async function everyCellIsExactlyThreeColumns() {
    const cells = [
    ...
    for (const sprite of cells) {
        assert.strictEqual(sprite.text.length, CELL_WIDTH, `a cell is ${sprite.text.length} columns, not ${CELL_WIDTH}: ${JSON.stringify(sprite.text)}`);
 ...
 */
export async function waterFillsTheCellNotJustTheGlyph() {
...
export async function theFillThresholdIsHonoured() {
...
export async function anObstacleIsUnmistakable() {
...
export async function theStartCellShowsItsDirection() {
    for (const arrow of ['>', '<', '^', 'v']) {
        const sprite = cellSprite('start', 0, false, arrow);
        assert.ok(sprite.text.includes(arrow), `the start cell should show ${arrow}, got ${JSON.stringify(sprite.text)}`);
 ...
 */
export async function everyPipeIsPlainAscii() {
    for (const pipe of ALL_PIPES) {
        for (const ch of PIPE_GLYPHS[pipe]) {
            const code = ch.charCodeAt(0);
            assert.ok(code >= 0x20 && code <= 0x7e, `${pipe} uses a non-ASCII character (0x${code.toString(16)}) that will not render the same everywhere`);
        }
    }
}
/** Corners are drawn as bends, not as letters. */
export async function cornersAreDrawnAsBendsNotLetters() {
    for (const corner of ['cornerNE', 'cornerNW', 'cornerSE', 'cornerSW']) {
        assert.ok(!/[A-Za-z]/.test(PIPE_GLYPHS[corner]), `${corner} is drawn as ${JSON.stringify(PIPE_GLYPHS[corner])} - a letter is not a pipe bend`);
    }
}
/** The renderer paints sprites rather than hand-built strings. */
export async function theRendererUsesTheSpriteLayer() {
    ...
    assert.ok(!/cellStr = '\{gray-fg\}XXX\{\/\}'/.test(game), 'the old hand-built cell strings should be gone');
}
