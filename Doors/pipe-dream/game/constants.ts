...
export const PIPE_SPRITES: Record<PipeType, string[]> = {
  vertical: [
    ' | ',
    ' | ',
    ' | ',
...
export const PIPE_CHARS: Record<PipeType, string> = {
  vertical: '|',
  horizontal: '-',
  cornerNE: 'L',
  cornerNW: 'J',
...
export const BASIC_PIPES: PipeType[] = [
  'vertical',
  'horizontal',
  'cornerNE',
  'cornerNW',
...
export function getPipesForLevel(level: number): PipeType[] {
  const config = getLevelConfig(level);
  const pipes = [...BASIC_PIPES];

  if (config.hasCross) pipes.push('cross');
