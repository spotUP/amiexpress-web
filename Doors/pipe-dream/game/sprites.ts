 ...
 */
export const PIPE_GLYPHS: Record<PipeType, string> = {
  vertical: ' | ',
  horizontal: '---',
  cornerNE: " '-",
  cornerNW: "-' ",
...
export function asCursor(sprite: Sprite): string {
  return `{lightwhite-bg}{black-fg}${sprite.text}{/}`;
}

/** How far along the flooding this cell is. */
export function fillStage(fillLevel: number): 'dry' | 'partial' | 'flooded' {
  if (fillLevel > FILLED_ABOVE) return 'flooded';
  if (fillLevel > 0) return 'partial';
  return 'dry';
}
...
export function cellSprite(
  ...
  const bg =
    stage === 'flooded' ? BG_COLORS.flooded :
    stage === 'partial' ? BG_COLORS.partial :
    BG_COLORS.pipe;

  const fg =
    stage === 'flooded' ? SPRITE_FG.flooded :
