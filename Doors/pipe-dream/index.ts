...
const door = new Door({ name: 'Pipe Dream', version: '1.0.0', author: 'AmiExpress BBS' });
...
function initScreen(): void {
  screen = blessed.screen({
    smartCSR: true, dockBorders: true, title: 'Pipe Dream', fullUnicode: false,
  ...
  gameArea = blessed.box({ parent: screen, top: 1, left: 0, width: '100%', height: '100%-4', tags: true, style: { bg: 'black' } });
...
function showMenu(): void {
  ...
  menuBox = blessed.box({
    parent: gameArea, top: 'center', left: 'center', width: 56, height: menuContent.length + 2,
...
async function showHighscores(): Promise<void> {
  ...
  gameData.highscores.slice(0, 10).forEach((score, i) => {
    content.push(`{cyan-fg}${(i+1).toString().padStart(2)}.{/}   {white-fg}${score.name.padEnd(4)}{/}  {yellow-fg}${score.score.toString().padStart(6)}{/}   {green-fg}${score.level.toString().padStart(2)}{/}`);
  });
...
function showHelp(): void {
  const content = [
    ...
    '{cyan-fg}OBJECTIVE:{/}', 'Connect pipes to guide the flow!',
...
function showGameOver(): void {
  if (menuBox) menuBox.destroy();
  menuBox = blessed.box({
    parent: gameArea, top: 'center', left: 'center', width: 32, height: 9,
...
door.onError((error: Error) => { console.error('[Pipe Dream] Error:', error); cleanup(); });
