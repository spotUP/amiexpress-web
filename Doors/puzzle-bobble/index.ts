...
const door = new Door({ name: 'Puzzle Bobble', version: '1.0.0', author: 'AmiExpress BBS' });
...
function initScreen(): void {
  screen = blessed.screen({
    smartCSR: true, dockBorders: true, title: 'Puzzle Bobble', fullUnicode: false,
...
function formatHUD(): string {
  ...
  const comboStr = gameData.combo > 1 ? `{magenta-fg}x${gameData.combo}{/}` : '';
...
function showMenu(): void {
  ...
  menuBox = blessed.box({
    ...
    tags: true, border: { type: 'line' }, style: { fg: 'white', bg: 'black', border: { fg: 'magenta' } },
...
function showHelp(): void {
  const content = [
    ...
    '{cyan-fg}OBJECTIVE:{/}', 'Match 3+ bubbles to pop them!',
...
door.onError((error: Error) => { console.error('[Puzzle Bobble] Error:', error); cleanup(); });
