...
function bubbleAt(row: number, col: number, color: BubbleColor) {
  return {
    x: col, y: row, color,
...
/** Firing. */
export async function shootingSounds(): Promise<void> {
...
export async function shootingWhileABubbleIsInFlightIsSilent(): Promise<void> {
...
export async function stickingAndPoppingSoundDifferent(): Promise<void> {
  ...
  for (let i = 0; i < 200 && stuck.data.shootingBubble; i++) stuck.game.update();
  ...
  for (let i = 0; i < 200 && popped.data.shootingBubble; i++) popped.game.update();
...
export async function theCeilingDroppingSounds(): Promise<void> {
  const { game, data } = emptyBoard();
...
export async function clearingTheGridSoundsTheLevel(): Promise<void> {
  const { game, data } = emptyBoard();
...
export async function reachingTheShooterSoundsGameOver(): Promise<void> {
  const { game, data } = emptyBoard();
  data.grid[0][0] = bubbleAt(0, 0, 'red');
