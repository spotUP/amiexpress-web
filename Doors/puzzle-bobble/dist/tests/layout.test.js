...
export async function theGameAreaHasNoGhostBorderAndDoesNotWrap() {
    const opts = boxOptions(indexSource(), 'gameArea');
    assert.ok(/border:\s*undefined/.test(opts), 'gameArea must pass border: undefined explicitly, or Panel injects one ' +
        'and steals two columns from a row that is already full width');
    assert.ok(/wrap:\s*false/.test(opts), 'gameArea must set wrap: false, or a full-width row wraps and the board ' +
        'renders on every other line');
}
/** A one-row HUD must not draw a border, or it has no room for content. */
export async function theHudHasNoGhostBorder() {
    const opts = boxOptions(indexSource(), 'hudBox');
    assert.ok(/border:\s*undefined/.test(opts), 'hudBox must pass border: undefined explicitly - on a one-row box the ' +
