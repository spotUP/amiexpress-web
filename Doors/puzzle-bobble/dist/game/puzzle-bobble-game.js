...
export class PuzzleBobbleGame {
    ...
    checkBubbleCollision(bubble) {
        for (let row = 0; row < this.data.grid.length; row++) {
            for (let col = 0; col < this.data.grid[row].length; col++) {
                ...
                const dist = Math.sqrt(Math.pow(bubble.x - cellX, 2) + Math.pow(bubble.y - cellY, 2));
                if (dist < 1.5) {
    ...
    findPlacementSpot(hitRow, hitCol, bubbleX, bubbleY) {
        ...
        for (const [nRow, nCol] of neighbors) {
            if (nRow >= 0 && nRow < GRID_HEIGHT && nCol >= 0 && nCol < GRID_WIDTH) {
                if (!this.data.grid[nRow]?.[nCol]) {
                    ...
                    const dist = Math.sqrt(Math.pow(bubbleX - (nCol + offsetX), 2) +
    ...
    placeBubble(bubble, row, col) {
        ...
        if (matches.length >= MIN_MATCH) {
            ...
            const comboBonus = this.data.combo > 1 ? SCORES[`combo${Math.min(this.data.combo, 5)}`] || 0 : 0;
            this.data.score += baseScore + comboBonus;
    ...
    findMatches(startRow, startCol, color) {
        const matches = [];
        ...
        const explore = (row, col) => {
            ...
            if (row < 0 || row >= GRID_HEIGHT || col < 0 || col >= GRID_WIDTH)
                return;
            const cell = this.data.grid[row]?.[col];
            if (!cell || cell.isPopping || cell.isFalling || cell.color !== color)
                return;
            visited.add(key);
