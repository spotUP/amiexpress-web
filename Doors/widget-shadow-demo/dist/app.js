...
async function createApp(session) {
    ...
    const underPanel = new blessed_1.DockablePanel({
        parent: screen,
        title: ' Yellow Panel ',
        left: 10,
        top: 4,
    ...
    const overPanel = new blessed_1.DockablePanel({
        parent: screen,
        title: ' Drag Me ',
        left: 'center',
        top: 'center',
    ...
    overPanel.key('left', function () {
        ...
        overPanel.left = Math.max(0, current - 2);
        scheduleRender();
    });
    overPanel.key('up', function () {
        const current = getSafePosition(overPanel, 'top');
        overPanel.top = Math.max(0, current - 1);
        scheduleRender();
    });
    overPanel.key('right', function () {
        const bounds = getScreenBounds();
        const current = getSafePosition(overPanel, 'left');
        overPanel.left = Math.min(bounds.maxX, current + 2);
        scheduleRender();
    });
    overPanel.key('down', function () {
        const bounds = getScreenBounds();
        const current = getSafePosition(overPanel, 'top');
        overPanel.top = Math.min(bounds.maxY, current + 1);
        scheduleRender();
    });
