 ...
 */
export const PADDLE_EASE = 0.65;

/** Closer than this and the paddle simply sits on the target. */
export const PADDLE_SNAP = 0.05;
 ...
 */
export function easePaddle(
  current: number,
  target: number,
  ease: number = PADDLE_EASE,
  snap: number = PADDLE_SNAP
): number {
  const distance = target - current;
  if (Math.abs(distance) < snap) return target;
