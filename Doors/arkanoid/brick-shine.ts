 ...
 * tall the wall is.
 */
export function shineDuration(rows: number): number {
  return rows * SHINE_ROW_STEP + SHINE_VISIBLE;
}
 ...
 */
export function shineDelayFor(y: number, topY: number, rowHeight: number): number {
  const row = Math.max(0, Math.round((y - topY) / Math.max(1, rowHeight)));
  return SHINE_VISIBLE + row * SHINE_ROW_STEP;
}

/** Is a brick lit at this point in its countdown? */
export function isShining(shineFrame: number): boolean {
  return shineFrame > 0 && shineFrame < SHINE_VISIBLE;
}
