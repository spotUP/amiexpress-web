...
export class PowerUpSystem {
    ...
    bounce(powerUp) {
        ...
        if (!open(powerUp.x + vx * step, powerUp.y))
            powerUp.vx = -vx;
        if (!open(powerUp.x, powerUp.y + vy * step))
            powerUp.vy = -vy;
     ...
     */
    findSpawnPosition(filled) {
        const d = this.data;
        const inField = (p) => p.x > 0 && p.x < FIELD_WIDTH - 1 && p.y > 0 && p.y < FIELD_HEIGHT - 1;
