 ...
 */
export function directionForKey(key, keyMap) {
    if (isDirectionToken(key))
        return key;
    if (!keyMap)
        return null;
    for (const direction of DIRECTIONS) {
        if (keyMap[direction] === key)
            return direction;
    }
    return null;
}
 ...
 */
export function canBindKey(key, direction) {
 ...
 */
export function controlBindings(keyMap) {
...
export function helpControlLines(keyMap) {
