 ...
 */
import { DRAW_BASE_POINTS, POINTS_PER_BONUS_PERCENT, CAPTURE_POINTS, LETTER_END_OF_LEVEL_POINTS, LETTER_WORD_COMPLETE_POINTS, SPARE_LETTER_POINTS, EXTRA_LIFE_PERCENT, MAX_HIGHSCORES, } from './constants';
export const ATTRACT_ORDER = ['points', 'ranking', 'invite'];
/** How long each panel stays up, in game ticks (~30 per second). */
export const ATTRACT_FRAMES = {
    points: 180,
    ranking: 180,
    invite: 120,
};
...
function scoreRow(label, value, width, colour) {
...
export function pointTablePanel(width) {
    return [
...
export function rankingPanel(scores, width) {
 ...
 */
export function invitePanel(data, width, blinkOn) {
    return [
        '',
        '',
...
export function creditLine(width) {
...
export function attractScreen(phase, data, width, frame) {
