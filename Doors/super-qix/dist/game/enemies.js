...
export class EnemySystem {
    ...
    splitQix(qix) {
        const d = this.data;
        if (d.qixList.length >= QIX_MAX_COPIES)
            return null;
        d.qixIdCounter = (d.qixIdCounter || 0) + 1;
        const copy = {
            id: d.qixIdCounter,
    ...
    maybeSplitQix() {
        const d = this.data;
        if (d.level < QIX_SPLIT_FROM_LEVEL)
            return;
        if (d.qixList.length >= QIX_MAX_COPIES)
            return;
        if (Math.random() >= QIX_SPLIT_CHANCE_PER_TICK)
            return;
        const original = d.qixList[0];
        if (original)
            this.splitQix(original);
    }
    /**
    ...
    updateQix(qix, speedScale = 1) {
        ...
        if (Math.random() < QIX_NUDGE_CHANCE) {
            ...
            const towardsMarker = Math.atan2(this.data.marker.y - qix.y, this.data.marker.x - qix.x);
     ...
     */
    updateSparx(sparx, speedScale = 1) {
        const d = this.data;
        if (d.borderPath.length === 0)
            return;
    ...
    checkQixCollision(marker, stix) {
        ...
        const touches = (a, b) => Math.round(a.x) === Math.round(b.x) && Math.round(a.y) === Math.round(b.y);
        for (const qix of d.qixList) {
            const parts = [{ x: qix.x, y: qix.y }, ...qix.segments];
            for (const part of parts) {
                if (touches(part, marker))
                    return true;
                for (const point of stix) {
                    if (touches(part, point))
                        return true;
                }
            }
        }
