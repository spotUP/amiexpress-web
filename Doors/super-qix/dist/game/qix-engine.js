...
export class QixEngine {
     ...
     */
    applyRejoinMultiplier(endPoint) {
        const d = this.data;
        const start = d.currentStix?.points[0];
        if (!start)
            return;
        const distance = Math.max(Math.abs(endPoint.x - start.x), Math.abs(endPoint.y - start.y));
     ...
     */
    returnMarkerToWalkableGround() {
        const d = this.data;
        if (d.marker.isDrawing)
            return;
        if (this.drawingSystem.isWalkable({ x: d.marker.x, y: d.marker.y }))
            return;
        ...
        for (let y = 0; y < FIELD_HEIGHT; y++) {
            for (let x = 0; x < FIELD_WIDTH; x++) {
                if (!this.drawingSystem.isWalkable({ x, y }))
                    continue;
    ...
    render() {
        ...
        for (const row of grid) {
            ...
            const flush = () => {
                ...
                let piece = run;
                if (fg)
                    piece = `{${fg}-fg}${piece}{/${fg}-fg}`;
                if (bg)
                    piece = `{${bg}-bg}${piece}{/${bg}-bg}`;
                line += piece;
                run = '';
            };
