...
let menuIdleLoop: ReturnType<typeof setInterval> | null = null;
...
function renderAttract(): void {
  if (gameArea) gameArea.setContent("");
 ...
 */
function startMenuIdle(): void {
  stopMenuIdle();
  menuIdleFrames = 0;
  menuIdleLoop = setInterval(() => {
    if (gameData.state !== "menu") return;
    if (++menuIdleFrames >= ATTRACT_IDLE_FRAMES) startAttract();
  }, GAME_TICK_MS);
}

function stopMenuIdle(): void {
  if (menuIdleLoop) {
...
const REMAP_ORDER: Direction[] = ["up", "down", "left", "right"];

/**
 * Open the remapper and ask for the first direction.
 */
function showRemapScreen(): void {
...
function renderRemapScreen(): void {
 ...
 * none at all.
 */
function handleRemapInput(key: InputKey): void {
  if (key === "escape") {
    showMenu();
    return;
  }
    ...
    if (gameData.state === "playing") {
      ...
      if (inputManager?.isKeyStateActive()) {
        for (const dir of ["up", "down", "left", "right"] as Direction[]) {
          ...
          if (inputManager.isHeld(gameData.keyMap[dir])) engine?.handleDirection(dir);
        }
      }
      engine?.update();
