...
exports.DITHER_BENEFIT = exports.DITHER_STRENGTH = exports.STICKINESS = void 0;
exports.createColorMemory = createColorMemory;
exports.fitColorMemory = fitColorMemory;
exports.pickColor = pickColor;
exports.ditherOffset = ditherOffset;
exports.pickColorDithered = pickColorDithered;
/**
 ...
 */
exports.STICKINESS = 12000;
function createColorMemory(width, height, stickiness = exports.STICKINESS) {
 ...
 */
function fitColorMemory(memory, width, height) {
 ...
 * match does.
 */
function pickColor(palette, r, g, b, incumbents, stickiness = exports.STICKINESS) {
    let best = 0;
    ...
    for (const incumbent of incumbents) {
        if (incumbent < 0 || incumbent >= palette.length)
            continue;
        if (incumbent === best)
            return best;
        if (distance(palette[incumbent], r, g, b) - bestDist <= stickiness)
            return incumbent;
    }
    return best;
}
 ...
 */
exports.DITHER_BENEFIT = 0.7;
/** The offset applied at a given position, in RGB units, -0.5..+0.5 scaled. */
function ditherOffset(x, y, strength = exports.DITHER_STRENGTH) {
 ...
 */
function pickColorDithered(palette, r, g, b, x, y, incumbents, stickiness = exports.STICKINESS) {
    let best = 0;
    let bestDist = Infinity;
    for (let i = 0; i < palette.length; i++) {
        const d = distance(palette[i], r, g, b);
    ...
    for (let i = 0; i < palette.length; i++) {
        ...
        if (mix <= 0)
            continue; // the pixel is not toward this colour
        if (mix > 1)
            mix = 1;
    ...
    let chosen = best;
    if (partner >= 0 && partnerDist < bestDist * exports.DITHER_BENEFIT) {
