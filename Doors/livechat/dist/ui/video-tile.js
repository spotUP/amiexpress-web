...
class VideoTile {
     ...
     */
    updateVideoDisplay() {
        console.log('[TileDiag] updateVideoDisplay user=%s hasVideo=%s hasFrame=%s -> %s', String(this.options.userId), this.options.hasVideo, this.hasFrame, this.options.hasVideo ? (this.hasFrame ? 'nothing' : 'PLACEHOLDER') : 'AVATAR');
    ...
    setVideoFrame(frame) {
        ...
        console.log('[TileDiag] setVideoFrame user=%s rows=%d widths=%s box=%sx%s', String(this.options.userId), rows.length, JSON.stringify(widths), String(this.videoBox.width), String(this.videoBox.height));
