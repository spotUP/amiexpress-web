...
function createDrawingCanvas(s, sock, st, cl, tb, bbs, ib, gcdn, ucl, usb, asm, MH, SW, SH, IH) {
    ...
    const canvas = blessed_1.default.canvas({ parent: s, top: MH, left: SW, right: 0, bottom: SH + IH, label: ' Drawing [Resize: Corner] - Click/Drag | C: Colors | X: Clear | ESC: Exit ', border: { type: 'line' }, hidden: true, mouse: true, fillChar: '\u2588', clearChar: ' ', ch: ' ', style: { fg: 'white', bg: 'black', border: { fg: 'magenta' } } });
    ...
    canvas.on('mouse', (e) => {
        if (!dm)
            return;
        const cl = canvas.aleft || 0;
        const ct = canvas.atop || 0;
        const x = e.x - cl - 1;
        const y = e.y - ct - 1;
    ...
    sock.on('draw:clear', (d) => { if (d.channel === cdc)
        canvas.clearCanvas(); });
    ...
    function exit() {
        ...
        if (pc && !idc(pc)) {
            ...
            if (pn)
                sock.emit('room:join', { roomName: pn });
