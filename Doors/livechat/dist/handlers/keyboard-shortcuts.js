...
function setupKeyboardShortcuts(s, cl, dc, ib, sbt, chl, ul, ep, sh, ssb, asm, sfs, sso, scon, cu, SW, chatLog, typingBar, menuBar, relayout) {
    let sv = true;
    function ucl() {
    ...
    s.key(['f4', 'C-e'], () => { if (!ep.isVisible())
        ep.show(s, (e) => { const c = ib.getValue(); ib.setValue(c + (e.display || e.code) + ' '); ib.focus(); s.render(); }, () => { ib.focus(); s.render(); }); });
    /**
     ...
     */
    const arrivedAt = (target) => {
        target.focus();
        if (menuBar?.getTabStop?.() === target)
            menuBar.openFirst?.();
        s.render();
        return true;
    };
    const cycleFocusForward = () => { const ps = fp(); const cf = s.getFocused(); let ci = fpi(ps, cf); if (ci === -1)
        ci = 0; const ni = (ci + 1) % ps.length; return arrivedAt(ps[ni]); };
