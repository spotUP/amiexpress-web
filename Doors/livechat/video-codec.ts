 ...
 */
export function isKeyframeDue(framesSinceKeyframe: number, interval: number = KEYFRAME_INTERVAL): boolean {
...
function encodePlane(
  ...
  while (i < size) {
    if (usable) {
      let same = 0;
      while (i + same < size && cells[i + same] === usable[i + same]) same++;
    ...
    while (i + run < size && cells[i + run] === value && run < 0xff) {
      if (usable && cells[i + run] === usable[i + run]) {
        let same = 0;
        while (i + run + same < size && cells[i + run + same] === usable[i + run + same]) same++;
        if (same > 3) break;
      }
      run++;
    }
...
export function decodeFrame(
  ...
  if (isDelta && (!previous || previous.length !== size)) return null;

  const cells = new Uint8Array(size);
  if (isDelta && previous) cells.set(previous);
 ...
 */
export function encodeRichFrame(
  frame: RichFrame,
  width: number,
  height: number,
  previous?: RichFrame | null
): ArrayBuffer {
  const size = width * height;
...
export function decodeRichFrame(
  buffer: ArrayBuffer | Uint8Array,
  previous?: RichFrame | null
): RichPacket | null {
  ...
  if ((bytes[2] & FLAG_RICH) === 0) return null;
  ...
  const usable = previous
    ...
    ? previous
    : null;
  if (isDelta && !usable) return null;

  const dots = new Uint8Array(size);
  const colors = new Uint8Array(size);
  if (usable) { dots.set(usable.dots); colors.set(usable.colors); }
...
export function isRichPacket(buffer: ArrayBuffer | Uint8Array): boolean {
  const bytes = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
  return bytes.byteLength >= HEADER_BYTES && (bytes[2] & FLAG_RICH) !== 0;
}

/** Whether a packet claims to be a delta, without decoding it. */
export function isDeltaPacket(buffer: ArrayBuffer | Uint8Array): boolean {
  const bytes = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
