...
export function createFileSharing(s: Screen, sock: any, st: any, un: string, asm: (m: string) => void, acm: (m: string) => void, aa: (a: string) => void, aud: any, sm: (w: any) => void, hm: (w: any) => void) {
  const o = blessed.box({ parent: s, top: 'center', left: 'center', width: '70%', height: '70%', label: ' Share Files [Drag to Move | Resize: Corner | ESC: Close] ', border: { type: 'line' }, shadow: true, hidden: true, mouse: true, draggable: true, ch: ' ', zIndex: 9990, style: { fg: 'white', bg: 'black', border: { fg: PANEL_BORDER } } });
  o.enableResize();
