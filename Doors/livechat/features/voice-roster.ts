...
function sameUser(a: number | string, b: number | string): boolean {
  return String(a) === String(b);
}

/** Display name for a channel id, matching what the sidebar shows. */
export function channelDisplayName(channelId: string): string {
...
export function seedRoster(
  channelId: string,
  participants: Array<{ userId: number | string; username?: string }> | undefined,
  self: { userId: number | string; username: string }
): VoiceChannelRoster {
  ...
  const entries: VoiceParticipantEntry[] = [];
  for (const p of participants || []) {
    if (entries.some(e => sameUser(e.userId, p.userId))) continue;
    entries.push({
...
export function addParticipant(
  roster: VoiceChannelRoster,
  user: { userId: number | string; username?: string }
): boolean {
  if (roster.participants.some(p => sameUser(p.userId, user.userId))) return false;
...
export function removeParticipant(roster: VoiceChannelRoster, userId: number | string): boolean {
...
export function setSpeaking(
  rosters: Iterable<VoiceChannelRoster>,
  userId: number | string,
  isSpeaking: boolean
): boolean {
  let changed = false;
