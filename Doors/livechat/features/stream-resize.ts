...
export interface ReshapeVideoApi {
  startStream(source: { type: string }, options: Record<string, unknown>): Promise<unknown>;
}
 ...
 */
export async function reshapeStream(
  video: ReshapeVideoApi,
  dims: StreamDims,
  options: ReshapeOptions
): Promise<void> {
  await video.startStream(
    { type: 'webcam' },
    {
      width: dims.width,
      height: dims.height,
