 ...
 * one, not two.
 */
export function wrappedHeight(line: string, width: number): number {
  if (width <= 0) return 1;
 ...
 */
export function messageIndexAtRow(
  lines: string[],
  width: number,
  scrollRow: number,
  row: number
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (target < cursor + height) return i;
    cursor += height;
  }
...
export function totalRows(lines: string[], width: number): number {
