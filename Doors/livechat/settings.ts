 ...
 */
export function applySettings(raw: Record<string, string | number | boolean>): LiveChatSettings {
  const channel = typeof raw.defaultChannel === 'string' ? raw.defaultChannel.trim() : '';
  return {
    defaultChannel: channel !== '' ? channel : DEFAULT_SETTINGS.defaultChannel,
    soundEffects: raw.soundEffects === undefined ? DEFAULT_SETTINGS.soundEffects : raw.soundEffects !== false,
    sidebarWidth: clampInt(raw.sidebarWidth, DEFAULT_SETTINGS.sidebarWidth, 8, 40),
    reconnectAttempts: clampInt(raw.reconnectAttempts, DEFAULT_SETTINGS.reconnectAttempts, 1, 10),
  };
}

let current: LiveChatSettings = DEFAULT_SETTINGS;

/** Read what the sysop set. Called once, when a user opens the door. */
export function loadSettings(): LiveChatSettings {
  try {
    current = applySettings(readDoorSettings(__dirname));
  } catch (error) {
    // A manifest someone broke by hand must not stop the door: it runs on
    // what it shipped with, and the admin reports the fault.
    console.error('[LiveChat] settings:', (error as Error).message);
...
export function settings(): LiveChatSettings {
  return current;
}
