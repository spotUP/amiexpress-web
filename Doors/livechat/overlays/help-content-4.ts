...
{bold}{green-fg}--- ADVANCED UI FEATURES ---{/green-fg}{/bold}

LiveChat now features a modern, dockable window management system.

{yellow-fg}ADJACENT (TILED) RESIZING{/yellow-fg}
...
{yellow-fg}INTELLIGENT SNAPPING{/yellow-fg}
Drag panels by their title bars near the screen edges.
A {cyan-fg}cyan ghost area{/cyan-fg} ({cyan-fg}░{/cyan-fg}) will show you exactly where the
panel will snap. Supports Left, Right, Top, and Bottom docking.

{yellow-fg}8-WAY RESIZING{/yellow-fg}
...
{yellow-fg}LAYOUT PERSISTENCE{/yellow-fg}
...
{yellow-fg}VISUAL DEPTH{/yellow-fg}
Focused panels are bright ({cyan-fg}Cyan{/cyan-fg} border, {white-fg}White{/white-fg} text).
Inactive panels automatically dim ({blue-fg}Blue{/blue-fg} border, {gray-fg}Gray{/gray-fg} text)
so you always know exactly which area is receiving input.

{yellow-fg}DOCKABLE SIDEBAR{/yellow-fg}
