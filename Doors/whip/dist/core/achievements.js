...
exports.getDefaultAchievements = getDefaultAchievements;
...
class AchievementManager {
    ...
    async meetsCondition(user, allTasks, allProjects, condition) {
        ...
        switch (condition.type) {
            ...
            case 'tasks-per-day': {
                ...
                const tasksToday = userTasks.filter(t => t.completedAt && new Date(t.completedAt).toDateString() === today);
            ...
            case 'tasks-assigned': {
                const tasksAssignedByUser = allTasks.filter(t => t.assignedTo && t.assignedTo !== user.userId);
            ...
            case 'project-size': {
                const sizedProjects = userProjects.filter(p => p.sizeLimit && p.sizeLimit <= (condition.count ?? 0) && p.status === 'released');
            ...
            case 'specific-party': {
                const partyProjects = userProjects.filter(p => p.partyId?.includes(condition.partyId ?? '') && p.status === 'released');
