...
export class PlatformStageGame {
   ...
   * Throw a coconut
   */
  private throwCoconut(): void {
    const ps = this.data.platformStage;

    if (ps.coconuts.length >= PLATFORM_STAGE.maxCoconuts) return;
   ...
   */
  private checkCoconutCollisions(): void {
    const d = this.data;
    const ps = d.platformStage;

    if (d.zeke.isJumping) return;  // Can't hit while jumping
