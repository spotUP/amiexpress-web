...
export async function createApp(session: DoorSession) {
  ...
  const chrome: DoorChrome = attachDoorChrome(theme, {
    ...
    title: `NEO-BLESSED SHOWCASE ${BUILD_VERSION}`,
    masthead: headerBar as any,
    footer: statusBar as any,
    hints: [
...
End of sample markdown.`;
  ...
  function showAutocompleteDemo() {
    ...
    usernameInput.on('select', (val: string) => {
    ...
    const wordInput = new AutocompleteTextbox({
      parent: demoBox,
    ...
    wordInput.on('select', (val: string) => {
