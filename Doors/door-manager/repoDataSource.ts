...
// ─── Per-archive detail (GET /doors/:archiveName) ───────────────────────────
 ...
 */
export function mergeDoorDetailIntoEntry(entry: CatalogEntry, detail: DoorDetailFields): CatalogEntry {
  const keep = <T>(current: T | null, incoming: T | null): T | null => (current ?? incoming);
  return {
    ...entry,
