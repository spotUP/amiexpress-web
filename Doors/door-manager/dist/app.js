 ...
 */
async function fetchInstalledDoors(bbs) {
    if (bbs.getDoorList) {
...
async function createApp(session) {
    ...
    const doorPanel = new blessed_1.DockablePanel({
        parent: screen,
    ...
    const doorList = (0, blessed_helpers_1.createList)({
        parent: doorPanel,
        top: 2,
        left: 0,
        width: '100%-2',
    ...
    const infoPanel = new blessed_1.DockablePanel({
        parent: screen,
        title: ' Door Info ',
        bottom: 3,
        left: 0,
 ...
 */
function showDoorMenu(screen, door, bbs, onRefresh) {
 ...
 */
function showDeleteConfirmation(screen, door, bbs, onRefresh) {
    const modal = new blessed_1.ConfirmModal({
        ...
        onConfirm: async () => {
            ...
            const statusMsg = new blessed_1.Message({
                parent: screen,
                top: 'center',
                left: 'center',
                width: 40,
            ...
            try {
                ...
                statusMsg.destroy();
                // Show result
                const resultMsg = new blessed_1.Message({
                    parent: screen,
                    top: 'center',
                    left: 'center',
                    width: 50,
                ...
                statusMsg.destroy();
                const errorMsg = new blessed_1.Message({
                    parent: screen,
                    top: 'center',
                    left: 'center',
                    width: 50,
 ...
 */
function showInfoEditor(screen, door, bbs) {
    // Create main editor box
    const editorBox = (0, blessed_helpers_1.createBox)({
        parent: screen,
        top: 0,
        left: 0,
        width: '100%',
    ...
    // Editor panel
    const editorPanel = (0, blessed_helpers_1.createBox)({
        parent: editorBox,
        top: 3,
        left: '30%',
        width: '70%',
    ...
    // Status line
    const statusLine = (0, blessed_helpers_1.createText)({
        parent: editorPanel,
        bottom: 1,
        left: 1,
        width: '100%-4',
    ...
    const loadFile = async (index) => {
        ...
        if (bbs.readInfoFile) {
            try {
                ...
                updateStatus(`{red-fg}Error loading file: ${error.message}{/red-fg}`);
            }
        }
        else {
    ...
    const saveFile = async () => {
        ...
        try {
            ...
            updateStatus(`{red-fg}Error saving: ${error.message}{/red-fg}`);
        }
    };
    // Start editing an entry
    const startEdit = (editKey) => {
        if (state.entries.length === 0)
            return;
    ...
    const showEditInput = (label, initialValue) => {
        ...
        prompt.showInput(`${label}:`, initialValue, (err, value) => {
    ...
    const switchPanel = () => {
        if (activePanel === 'files') {
            activePanel = 'editor';
            entryList.focus();
            fileList.style.border.fg = 'gray';
            editorPanel.style.border.fg = 'green';
        }
        else {
            activePanel = 'files';
            fileList.focus();
            fileList.style.border.fg = 'cyan';
            editorPanel.style.border.fg = 'gray';
        }
        screen.render();
    };
    // Key handlers
    ...
    screen.key(['q', 'Q'], () => {
        if (state.hasChanges) {
            ...
            const warnBox = (0, blessed_helpers_1.createBox)({
                parent: screen,
                top: 'center',
                left: 'center',
                width: 50,
    ...
    entryList.on('select', (item, index) => {
        state.selectedIndex = index;
        startEdit(false); // Edit value by default
    });
    // Track selection changes
    entryList.on('select item', (item, index) => {
        state.selectedIndex = index;
    });
 ...
 */
function showDoorDetails(screen, door, bbs) {
    // Container box
    ...
    const detailsBox = new blessed_1.ScrollableBox({
        parent: container,
        top: 0,
        left: 0,
        width: '100%-2',
