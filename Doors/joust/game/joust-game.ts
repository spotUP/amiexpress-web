...
export class JoustGame {
  ...
  private updatePlayer(): void {
    ...
    if (player.vy < MAX_RISE_SPEED) player.vy = MAX_RISE_SPEED;
  ...
  private checkEggLanding(egg: Egg): boolean {
  ...
  private updatePterodactyl(): void {
    const config = getWaveConfig(this.data.wave);
  ...
  private isOnPlatform(entity: Position & { vy: number }): boolean {
    for (const platform of this.data.platforms) {
  ...
  private handlePlatformCollision(entity: Position & { vy: number; y: number }): void {
    for (const platform of this.data.platforms) {
      if (entity.x >= platform.x && entity.x < platform.x + platform.width) {
        // Landing on platform
  ...
  checkCollisions(): void {
    ...
    for (const egg of this.data.eggs) {
      const dist = Math.sqrt(
        Math.pow(player.x - egg.x, 2) + Math.pow(player.y - egg.y, 2)
      );
    ...
    if (ptero.isActive) {
      const dist = Math.sqrt(
        Math.pow(player.x - ptero.x, 2) + Math.pow(player.y - ptero.y, 2)
      );
  ...
  private defeatEnemy(enemy: Enemy): void {
    const scoreKey = enemy.type as keyof typeof SCORES;
  ...
  private killPlayer(): void {
    if (!this.data.player.isAlive) return;
