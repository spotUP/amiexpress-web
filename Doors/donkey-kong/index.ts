...
function createInitialGameData(): DonkeyKongData {
  return {
    ...
    player: {
      x: 4, y: 19, vx: 0, vy: 0,
      direction: 'right',
      isJumping: false, isOnGround: true, isClimbing: false,
      climbFrame: 0, walkFrame: 0,
      hasHammer: false, hammerTimer: 0, hammerFrame: 0,
...
const door = new Door({ name: 'Donkey Kong', version: '1.0.0', author: 'AmiExpress BBS' });
...
function initScreen(): void {
  screen = blessed.screen({
    smartCSR: true, dockBorders: true, title: 'Donkey Kong', fullUnicode: false,
  ...
  gameArea = blessed.box({ parent: screen, top: 1, left: 0, width: GAME_WIDTH * 2, height: GAME_HEIGHT + 1, tags: true, style: { bg: 'black' } });

  footerBox = blessed.box({
    parent: screen, bottom: 0, left: 0, width: '100%', height: 3, tags: true,
    border: { type: 'line' }, style: { border: { fg: 'gray' } },
    content: '{gray-fg}Arrows: Move/Climb | Space: Jump | P: Pause | Q: Quit{/}',
  });
}
...
function formatHUD(): string {
  ...
  const bonusStr = Math.floor(gameData.bonusTimer / 100).toString().padStart(2, '0');
...
function showMenu(): void {
  ...
  menuBox = blessed.box({
    parent: gameArea, top: 'center', left: 'center', width: 44, height: menuContent.length + 2,
    tags: true, border: { type: 'line' }, style: { fg: 'white', bg: 'black', border: { fg: 'yellow' } },
...
async function showHighscores(): Promise<void> {
  ...
  gameData.highscores.slice(0, 10).forEach((score, i) => {
    content.push(`{cyan-fg}${(i+1).toString().padStart(2)}.{/}   {white-fg}${score.name.padEnd(4)}{/}  {yellow-fg}${score.score.toString().padStart(7)}{/}   {green-fg}${score.level.toString().padStart(2)}{/}`);
  });
  content.push('', '{gray-fg}Press any key to return{/}');

  if (menuBox) menuBox.destroy();
  menuBox = blessed.box({
    parent: gameArea, top: 'center', left: 'center', width: 38, height: content.length + 2,
...
function showHelp(): void {
  const content = [
    ...
    '{cyan-fg}OBJECTIVE:{/}', 'Climb to rescue Pauline from DK!', '',
    '{green-fg}CONTROLS:{/}',
    'Left/Right - Walk',
  ...
  if (menuBox) menuBox.destroy();
  menuBox = blessed.box({
    parent: gameArea, top: 'center', left: 'center', width: 42, height: content.length + 2,
...
function handleInput(key: string): void {
  ...
  switch (gameData.state) {
    ...
    case 'stageComplete': handleStageCompleteInput(inputKey); break;
...
door.onError((error: Error) => { console.error('[Donkey Kong] Error:', error); cleanup(); });
