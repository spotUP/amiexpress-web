 ...
 */
import { EMPTY, COLORS as SPRITE_COLORS, cell, paint } from './sprites';
...
export class DonkeyKongGame {
    ...
    updateDK() {
        ...
        if (this.data.stage === 'barrels' || this.data.stage === 'conveyors') {
            ...
            if (this.data.dkThrowTimer <= 0) {
                ...
                if (this.data.dkThrowTimer < 60)
                    this.data.dkThrowTimer = 60;
            }
        }
    ...
    updateBarrels() {
        for (const barrel of this.data.barrels) {
            ...
            if (barrel.onLadder) {
                ...
                // Rolling on girder
                barrel.vy += GRAVITY;
                if (barrel.vy > MAX_FALL_SPEED)
                    barrel.vy = MAX_FALL_SPEED;
                barrel.x += barrel.vx;
                barrel.y += barrel.vy;
    ...
    updateFireballs() {
        for (const fireball of this.data.fireBalls) {
            ...
            if (fireball.isClimbing) {
                ...
                // Normal movement
                fireball.vy += GRAVITY;
                if (fireball.vy > MAX_FALL_SPEED * 0.5)
                    fireball.vy = MAX_FALL_SPEED * 0.5;
    ...
    updateElevators() {
        for (const elevator of this.data.elevators) {
            ...
            if (elevator.direction === 'up') {
                elevator.y -= 0.1;
                if (elevator.y < 8)
                    elevator.y = 8 + elevator.height;
            }
            else {
                elevator.y += 0.1;
                if (elevator.y > 8 + elevator.height)
                    elevator.y = 8;
            }
    ...
    checkCollisions() {
        ...
        // Player vs barrels
        for (const barrel of this.data.barrels) {
            const dist = Math.sqrt(Math.pow(player.x - barrel.x, 2) + Math.pow(player.y - barrel.y, 2));
            if (dist < 1.5) {
                if (player.hasHammer) {
                    // Smash barrel
    ...
    render() {
        ...
        for (const elevator of this.data.elevators) {
            ...
            if (elevator.x >= 0 && elevator.x < GAME_WIDTH && ey >= 0 && ey < GAME_HEIGHT) {
                ...
                if (elevator.x + 1 < GAME_WIDTH)
                    buffer[ey][elevator.x + 1] = cell(']', SPRITE_COLORS.elevator);
            }
        }
        // Draw conveyors
        for (const conveyor of this.data.conveyors) {
        ...
        if (this.data.dkY >= 0 && this.data.dkY < GAME_HEIGHT &&
            ...
            if (this.data.dkX + 1 < GAME_WIDTH)
                buffer[this.data.dkY][this.data.dkX + 1] = cell(SPRITES.dk, SPRITE_COLORS.dk);
        }
        // Draw barrels
        for (const barrel of this.data.barrels) {
        ...
        if (player.isAlive && (player.invincibleTimer === 0 || this.data.frameCount % 4 < 2)) {
            ...
            if (px >= 0 && px < GAME_WIDTH && py >= 0 && py < GAME_HEIGHT) {
                // Mario carries his own colour. He used to be recovered from the
                // glyph afterwards - and playerClimb and ladder are BOTH 'H', with
