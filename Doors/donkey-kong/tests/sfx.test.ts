...
import { DonkeyKongData, Barrel } from '../game/types';

/** A stage with the hazards cleared, so only what a test places is in play. */
function emptyStage(): { game: DonkeyKongGame; data: DonkeyKongData } {
  const data = createInitialGameData();
  const game = new DonkeyKongGame(
    data,
    () => { /* no display in tests */ },
    () => { /* game over is the door's screen */ },
    () => { /* stage complete likewise */ }
  );
  game.initStage();
  data.state = 'playing';
  data.barrels = [];
...
function barrelAt(x: number, y: number): Barrel {
  return {
...
export async function aJumpInMidAirIsSilent(): Promise<void> {
  const { game, data } = emptyStage();
...
export async function theHammerTurnsADeathIntoASmash(): Promise<void> {
...
export async function takingTheHammerSounds(): Promise<void> {
  const { game, data } = emptyStage();
...
export async function pullingARivetSounds(): Promise<void> {
  const { game, data } = emptyStage();
