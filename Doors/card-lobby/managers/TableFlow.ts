...
export interface TableFlowHost {
  ...
  focusLobby(): void;
  getHumanPlayers(table: LobbyTable): TablePlayer[];
  getOpenHumanSeats(table: LobbyTable): number;
  syncBotsForTable(table: LobbyTable): boolean;
  updateTableStatus(table: LobbyTable): void;
  updateAllPanels(): void;
  updateLobbyPanel(): void;
  ...
  showYesNoDialog(title: string, text: string): Promise<boolean | null>;
  showListDialog(title: string, items: string[]): Promise<number | null>;
  showMessageDialog(title: string, text: string): Promise<void>;
}

export class TableFlow {
  constructor(private host: TableFlowHost) {}

  isObserverForTable(table: LobbyTable, userId: string): boolean {
  ...
  async createTableFlow(): Promise<void> {
    ...
    const gameIndex = await this.host.showListDialog(
      'Select Game',
    ...
    const stakeIndex = await this.host.showListDialog(
      'Select Stakes',
      game.stakes.map((stake) => `${stake.label} (Buy-in ${stake.buyIn})`),
    );
    if (stakeIndex === null) return;

    const maxPlayersStr = await this.host.showPromptDialog(
      'Table Size',
  ...
  async joinSelectedTable(): Promise<void> {
  ...
  async observeSelectedTable(): Promise<void> {
    if (this.host.modalActive || !this.host.lobby || !this.host.currentProfile) return;
    if (!this.host.selectedTableId) {
      this.host.pushNotice('Select a table first.');
      return;
    }
  ...
  findSeatForHuman(table: LobbyTable): number | null {
  ...
  async leaveCurrentTable(): Promise<void> {
    ...
    if (playerIndex >= 0) {
      ...
      if (net > 0) this.host.currentProfile.wallet.lifetimeEarned += net;
      if (net < 0) this.host.currentProfile.wallet.lifetimeSpent += Math.abs(net);
  ...
  deleteTableFlow(): void {
    if (!this.host.selectedTableId || !this.host.lobby) return;
    ...
    void this.host
      ...
      .then(async (confirmed: boolean | null) => {
