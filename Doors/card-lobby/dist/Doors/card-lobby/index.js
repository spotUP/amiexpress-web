...
const blessed_1 = __importDefault(require("../../sdk/engines/ui/blessed"));
const blessed_helpers_1 = require("../../sdk/utils/blessed-helpers");
...
const cardEngine = new sdk_1.CardEngine();
const PokerAction = sdk_1.ActionType ?? {
    SIT: 'SIT',
    STAND: 'STAND',
    ADD_CHIPS: 'ADD_CHIPS',
...
class CardLobbyApp {
    ...
    buildTopBar() {
        ...
        this.topBar = blessed_1.default.listbar({
            parent: this.desktop,
            top: 0,
            left: 0,
            width: '100%',
    ...
    async reloadState() {
        const globalStore = new sdk_1.Storage({
            doorName: 'card_lobby',
            global: true,
        });
    ...
    loadTableHand(table) {
        if (!table.hand)
            return null;
        try {
            const engine = sdk_1.PokerEngine.restore(table.hand.snapshot);
    ...
    updateTablePanel() {
        ...
        if (showGameView) {
            ...
            const boardCards = handState
                ? (0, sdk_1.pokerCardsToCards)(handState.engine.state.board)
                : (0, sdk_1.pokerCardsToCards)(table.lastHand?.board ?? []);
            const playerSeat = handState?.engine.state.players.find((seat) => seat?.id === this.currentProfile?.userId);
            const playerHand = (0, sdk_1.pokerCardsToCards)(playerSeat?.hand ?? table.lastHand?.hands[this.currentProfile.userId] ?? []);
    ...
    async startHoldemHand(table) {
        try {
            ...
            const engine = new sdk_1.PokerEngine({
    ...
    showTextWindow(title, content, opts) {
        ...
        const text = blessed_1.default.scrollabletext({
            parent: modal,
            top: 1,
            left: 1,
            right: 1,
        ...
        const backButton = (0, blessed_helpers_1.createButton)({
            parent: modal,
            bottom: 0,
            right: 2,
            width: 10,
