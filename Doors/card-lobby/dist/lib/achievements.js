...
exports.recordHandResult = recordHandResult;
...
function unlockAchievements(profile) {
    ...
    const award = (id) => {
        if (held.has(id))
            return;
        const definition = constants_1.ACHIEVEMENTS.find((achievement) => achievement.id === id);
        if (!definition)
            return;
        held.add(id);
 ...
 */
function recordHandResult(profile, delta, pot) {
