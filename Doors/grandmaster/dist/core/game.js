...
class GameEngine {
    constructor(mode, settings, sounds, attackManager, startLevel = 0) {
        ...
        this.pieceManager = new pieces_1.PieceManager(settings.rotationSystem);
        this.sectionManager = new sections_1.SectionManager();
        this.gradeManager = new grading_1.GradeManager();
        this.medalManager = new medals_1.MedalManager(); // HeborisCE medal system
        this.creditRollManager = new credit_roll_1.CreditRollManager();
        this.invisiblePieceManager = new credit_roll_1.InvisiblePieceManager();
     ...
     */
    seedGarbageRows(rows) {
        (0, garbage_1.seedGarbageRows)(this.state.board, rows);
    }
    ...
    spawnPiece() {
        ...
        if (big)
            this.state.bigPiecesRemaining--;
        if (this.state.rollRollPiecesRemaining > 0)
            this.state.rollRollPiecesRemaining--;
        ...
        if (big) {
            ...
            if (overflow > 0)
                spawnPos.x = Math.max(0, spawnPos.x - overflow);
        }
        ...
        if ((0, board_1.checkCollision)(this.state.board, shape, spawnPos.x, spawnPos.y)) {
    ...
    updateFrame() {
        ...
        if ((this.state.rollRollPiecesRemaining > 0 || this.state.missionModifiers?.rollRoll === true)
            && this.rollRollFrame % items_1.ROLL_ROLL_PERIOD_FRAMES === 0) {
            this.rotate(1);
        }
        // Apply gravity
        this.applyGravity();
    ...
    applyGravity() {
        ...
        if (dropAmount > 0) {
            ...
            for (let i = 0; i < dropAmount; i++) {
                if (!(0, board_1.checkCollision)(this.state.board, shape, piece.x, piece.y + 1)) {
                    piece.y++;
                    moved = true;
                }
                else {
    ...
    isPieceGrounded() {
        ...
        const piece = this.state.currentPiece;
        const shape = this.pieceManager.getShape(piece.type, piece.rotation, !!piece.big);
        return (0, board_1.checkCollision)(this.state.board, shape, piece.x, piece.y + 1);
    }
    /**
    ...
    move(direction) {
        ...
        if (!(0, board_1.checkCollision)(this.state.board, shape, newX, piece.y)) {
            piece.x = newX;
    ...
    rotate(direction) {
        ...
        for (let kickIndex = 0; kickIndex < kicks.length; kickIndex++) {
            ...
            if (!(0, board_1.checkCollision)(this.state.board, newShape, testX, testY)) {
    ...
    detectTSpin() {
        ...
        if (this.state.tSpinFlag === 0 || this.state.rotationCount === 0)
            return 'none';
        // Also check lastMove for compatibility
        if (this.state.lastMove !== 'rotate')
            return 'none';
    ...
    lockPiece() {
        ...
        if (lineCount > 0) {
            ...
            const speedParams = (0, gravity_1.getSpeedParams)(this.state.level, this.state.mode);
            ...
            if (tSpin === 'full') {
                ...
                // T-Spin Mini bonuses
                if (lineCount === 1)
                    baseScore = 200; // T-Spin Mini Single
                else if (lineCount === 2)
                    baseScore = 400; // T-Spin Mini Double
            }
            ...
            const perfectClear = (0, board_1.isPerfectClear)(this.state.board);
            if (perfectClear) {
            ...
            if (this.attackManager) {
                this.attackManager.processLineClear(lineCount, tSpin, this.state.combo, this.state.backToBack, perfectClear);
            }
        ...
        this.onLockCallback?.({
            lineCount,
            tSpin: tSpin !== 'none',
            allClear: (0, board_1.isPerfectClear)(this.state.board),
     ...
     */
    applyShiraseGarbage() {
        (0, garbage_1.riseGarbageRow)(this.state.board);
     ...
     */
    placePieceWithTimestamp(shape, x, y, pieceType, lockTime) {
        const board = this.state.board;
    ...
    processCollectedItem(itemId) {
        ...
        // nothing at all.
        this.applyItemEffectResult((0, items_1.applyEnemyItem)(itemId, this.state.board, this.state.board));
    }
    /**
     ...
     */
    startRecording(userId, username, seed) {
        this.recorder = new replay_manager_1.ReplayRecorder(userId, username, this.state.mode, seed);
    }
    /**
