...
exports.INPUT_CHARS = exports.INPUT_BITS = void 0;
exports.encodeInput = encodeInput;
exports.decodeInput = decodeInput;
exports.inputStateToMask = inputStateToMask;
exports.maskToInputState = maskToInputState;
exports.compressInputString = compressInputString;
exports.decompressInputString = decompressInputString;
...
exports.INPUT_CHARS = {
    idle: ALPHABET[0],
    right: ALPHABET[exports.INPUT_BITS.RIGHT],
    left: ALPHABET[exports.INPUT_BITS.LEFT],
    down: ALPHABET[exports.INPUT_BITS.DOWN],
    up: ALPHABET[exports.INPUT_BITS.UP],
    swap: ALPHABET[exports.INPUT_BITS.SWAP],
    raise: ALPHABET[exports.INPUT_BITS.RAISE],
};
/** The button mask for one frame, as a character. */
function encodeInput(mask) {
...
function decodeInput(char) {
    ...
    if (mask < 0)
        throw new Error(`decodeInput: not an input character: ${JSON.stringify(char)}`);
    return mask;
}
function inputStateToMask(state) {
    return (state.right ? exports.INPUT_BITS.RIGHT : 0)
        + (state.left ? exports.INPUT_BITS.LEFT : 0)
        + (state.down ? exports.INPUT_BITS.DOWN : 0)
        + (state.up ? exports.INPUT_BITS.UP : 0)
        + (state.swap ? exports.INPUT_BITS.SWAP : 0)
        + (state.raise ? exports.INPUT_BITS.RAISE : 0);
}
function maskToInputState(mask) {
    return {
        right: (mask & exports.INPUT_BITS.RIGHT) !== 0,
        left: (mask & exports.INPUT_BITS.LEFT) !== 0,
        down: (mask & exports.INPUT_BITS.DOWN) !== 0,
        up: (mask & exports.INPUT_BITS.UP) !== 0,
        swap: (mask & exports.INPUT_BITS.SWAP) !== 0,
        raise: (mask & exports.INPUT_BITS.RAISE) !== 0,
    };
}
 ...
 */
function compressInputString(inputs) {
    if (inputs.length === 0)
        return '';
    const out = [];
    const chars = [...inputs];
 ...
 */
function decompressInputString(inputs) {
    if (inputs.length === 0)
        return '';
    const chars = [...inputs];
    const out = [];
    ...
    while (i < chars.length) {
        ...
        if (char === '(') {
            ...
            if (i >= chars.length)
                return inputs; // unterminated: not compressed
            i += 1; // skip ')'
            continue;
        }
        if (isDigit(char)) {
...
//# sourceMappingURL=input-codec.js.map