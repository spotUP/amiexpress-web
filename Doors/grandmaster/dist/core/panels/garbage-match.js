...
exports.matchOnContact = matchOnContact;
exports.getConnectedGarbagePanels = getConnectedGarbagePanels;
exports.matchGarbagePanels = matchGarbagePanels;
exports.convertGarbagePanels = convertGarbagePanels;
exports.pushGarbage = pushGarbage;
 ...
 * those.
 */
function getConnectedGarbagePanels(stack, matchingPanels) {
    const garbageIds = [];
    ...
    for (let row = 1; row < stack.panels.length; row++) {
        ...
        for (let col = 1; col <= stack.width; col++) {
            ...
            if (!panel || !panel.isGarbage || panel.state !== 'normal')
                continue;
            if (panel.garbageId === undefined || seenId.has(panel.garbageId))
                continue;
    ...
    for (let i = 0; i < garbageIds.length; i++) {
        ...
        for (const panel of matchingPanels) {
            ...
            const touchesHorizontally = (panel.column === piece.left - 1 || panel.column === piece.right + 1)
    ...
    for (let i = 0; i < garbageIds.length; i++) {
        ...
        for (let row = piece.bottom; row <= piece.top; row++) {
            ...
            for (let col = piece.left; col <= piece.right; col++) {
                ...
                if (panel)
                    garbagePanels.push(panel);
            }
        }
    }
    // Remember the highest id, so a block forced off-screen stays clearable.
    const highest = Math.max(...matchedIds);
    if (highest > stack.highestGarbageIdMatched)
        stack.highestGarbageIdMatched = highest;
 ...
 */
function matchGarbagePanels(stack, garbagePanels, garbageMatchTime, isChain, onScreenCount) {
    const ordered = (0, check_matches_1.sortByPopOrder)(garbagePanels, true);
 ...
 */
function convertGarbagePanels(stack, isChain) {
    for (let row = 1; row < stack.panels.length; row++) {
        const rowPanels = stack.panels[row];
        if (!rowPanels)
            continue;
        let garbagePanelRow = null;
 ...
 */
function pushGarbage(stack, origin, isChain, comboSize, metalCount) {
    ...
    const comboPieces = (0, consts_1.comboGarbageFor)(comboSize);
    ...
    if (isChain) {
        ...
        stack.outgoingGarbage.addChainLink(stack.stopWatch, origin.column, origin.row + rowOffset);
    }
}
//# sourceMappingURL=garbage-match.js.map