...
exports.DEFAULT_SOFT_DROP_SPEED = void 0;
exports.softDropFactor = softDropFactor;
exports.softDropIntervalMs = softDropIntervalMs;
/** Cells per second when the setting is absent (TIMING.SOFT_DROP_RATE = 50ms). */
exports.DEFAULT_SOFT_DROP_SPEED = 20;
/** The reference's per-system multiplier (world.c:401-408). */
function softDropFactor(rotationSystem) {
 ...
 */
function softDropIntervalMs(softDropSpeed, rotationSystem) {
    const speed = Number.isFinite(softDropSpeed) && softDropSpeed > 0
        ? softDropSpeed
        : exports.DEFAULT_SOFT_DROP_SPEED;
    const cellsPerSecond = speed * softDropFactor(rotationSystem);
    return Math.min(500, Math.max(10, Math.round(1000 / cellsPerSecond)));
}
//# sourceMappingURL=soft-drop.js.map