...
exports.MISSION_FIELDS = exports.HIDDEN_MODES = void 0;
exports.blankMission = blankMission;
exports.fieldValue = fieldValue;
exports.cycleField = cycleField;
exports.setField = setField;
const mission_types_1 = require("./mission-types");
/** The HIDDEN speeds, in the order the editor cycles them. */
exports.HIDDEN_MODES = ['OFF', 'SLOW', 'FAST', 'FASTEST'];
/**
...
function fieldValue(mission, field) {
    switch (field) {
 ...
 */
function cycleField(mission, field, step) {
    const next = { ...mission, modifiers: { ...mission.modifiers } };
    switch (field) {
        case 'objective': {
            const at = mission_types_1.MISSION_OBJECTIVES.indexOf(mission.objective);
            const to = (at + step + mission_types_1.MISSION_OBJECTIVES.length) % mission_types_1.MISSION_OBJECTIVES.length;
            next.objective = mission_types_1.MISSION_OBJECTIVES[to];
        ...
        case 'hidden': {
            const at = exports.HIDDEN_MODES.indexOf(next.modifiers.hidden ?? 'OFF');
            const to = (at + step + exports.HIDDEN_MODES.length) % exports.HIDDEN_MODES.length;
            const mode = exports.HIDDEN_MODES[to];
            if (mode === 'OFF')
                delete next.modifiers.hidden;
            else
                next.modifiers.hidden = mode;
            break;
        }
        case 'big':
            if (next.modifiers.big)
                delete next.modifiers.big;
            else
                next.modifiers.big = true;
            break;
        case 'hideNext':
            if (next.modifiers.hideNext)
                delete next.modifiers.hideNext;
            else
                next.modifiers.hideNext = true;
            break;
        case 'rollRoll':
            if (next.modifiers.rollRoll)
                delete next.modifiers.rollRoll;
            else
                next.modifiers.rollRoll = true;
            break;
        default:
 ...
 */
function setField(mission, field, typed) {
    const spec = exports.MISSION_FIELDS.find((entry) => entry.field === field);
    if (!spec)
    ...
    if (spec.kind === 'number') {
        ...
        if (field === 'norm')
            ...
            next.timeLimitSeconds = value;
        else if (field === 'startLevel')
            next.startLevel = value;
        else if (field === 'garbageRows')
            next.garbageRows = value;
        if (next.objective === 'survive' && next.timeLimitSeconds <= 0) {
