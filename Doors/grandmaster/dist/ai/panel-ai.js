...
exports.PanelAi = exports.MAX_AI_LEVEL = exports.AI_LEVELS = void 0;
...
exports.MAX_AI_LEVEL = exports.AI_LEVELS.length - 1;
...
class BoardScanner {
    ...
    isPanicking() {
        ...
        for (let row = height - PANIC_ROWS + 1; row <= height; row++) {
            for (let col = 1; col <= width; col++) {
                if (this.isSettledBlock(row, col))
                    return true;
            }
        }
        return false;
    ...
    findColorCol(color, row) {
        for (let col = 1; col <= this.stack.width; col++) {
            const panel = this.stack.panels[row]?.[col];
            if (panel && !panel.isGarbage && panel.color === color)
                return col;
        }
        return -1;
    }
        ...
        for (let row = height; row >= 2; row--) {
            for (let col = 1; col <= width; col++) {
                ...
                for (let target = col - 1; target >= 1; target--) {
                    if (!this.isAir(row, target))
                        break;
                    if (this.isAir(row - 1, target))
                        return { x: col, y: row, dx: target, dy: row };
                }
                // And rightwards.
 ...
 */
class PanelAi {
    constructor(stack, level = 5) {
        this.stack = stack;
        this.inputQueue = [];
        this.blockMoveQueue = [];
        this.level = exports.AI_LEVELS[Math.max(0, Math.min(level, exports.MAX_AI_LEVEL))];
    ...
    update() {
        if (this.stack.gameEnded())
            return 0;
        if (this.stack.clock % this.level.thinkInterval !== 0)
            return 0;
        if (this.inputQueue.length === 0 && this.blockMoveQueue.length === 0) {
            this.plan();
        }
        if (this.inputQueue.length > 0) {
            return PanelAi.maskFor(this.inputQueue.shift());
        }
        if (this.blockMoveQueue.length > 0) {
            const move = this.blockMoveQueue.shift();
     ...
     */
    expandBlockMove(move) {
        if (move.dy > move.y)
            return; // upstream errors here; nothing calls it that way
        if (move.dx > move.x) {
    ...
    queueCursorMove(x, y) {
        ...
        for (let i = 0; i < x - curX; i++)
            this.inputQueue.push('right');
        for (let i = 0; i < curX - x; i++)
            this.inputQueue.push('left');
        for (let i = 0; i < y - curY; i++)
            this.inputQueue.push('up');
        for (let i = 0; i < curY - y; i++)
            this.inputQueue.push('down');
    }
    static maskFor(action) {
        switch (action) {
            case 'up': return input_codec_1.INPUT_BITS.UP;
            case 'down': return input_codec_1.INPUT_BITS.DOWN;
            case 'left': return input_codec_1.INPUT_BITS.LEFT;
            case 'right': return input_codec_1.INPUT_BITS.RIGHT;
            case 'swap': return input_codec_1.INPUT_BITS.SWAP;
            case 'raise': return input_codec_1.INPUT_BITS.RAISE;
            case 'wait':
            default: return 0;
        }
    }
}
exports.PanelAi = PanelAi;
//# sourceMappingURL=panel-ai.js.map