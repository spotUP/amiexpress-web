...
class GameValidator {
     ...
     */
    validateStatistics(result, replay) {
        const flags = [];
        const violations = [];
    ...
    calculateAnomalyScore(result) {
        ...
        if (result.finesseRate === 1.0)
            anomalyScore += 15;
        if (result.finesseErrors === 0 && result.lines > 100)
            anomalyScore += 10;
        ...
        if (result.grade === 'GM' && result.level < 999)
            anomalyScore += 50;
        if (result.grade.startsWith('M') && result.level < 500)
            anomalyScore += 30;
