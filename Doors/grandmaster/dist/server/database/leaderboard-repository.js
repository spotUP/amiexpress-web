...
class LeaderboardRepository {
    ...
    `);
        stmt.run(entry.id, entry.userId, entry.username, entry.mode, entry.score, entry.level, entry.grade, entry.lines, entry.time, entry.finesseRate || null, entry.finesseErrors || null, entry.validationScore, JSON.stringify(entry.flags), entry.replayId || null);
    }
    /**
    ...
    getUserBest(userId, mode) {
      ...
      LIMIT 1
    `);
        const row = stmt.get(userId, mode);
    ...
    `);
        const result = stmt.get(mode, userId, mode);
