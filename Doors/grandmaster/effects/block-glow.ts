...
export interface BlockGlow {
  x: number;
  y: number;
  timer: number;          // 0-12 base frames (at 60 FPS = 200ms)
  intensity: number;      // 0.0-1.0 (calculated from timer)
  type: GlowType;         // 'primary' or 'b2b' (back-to-back bonus)
  pattern: GlowPattern;   // 'linear', 'manhattan', or 'euclidean'
  centerX?: number;       // Center X for radial patterns
  centerY?: number;       // Center Y for radial patterns
 ...
 */
export class BlockGlowManager {
  private glows: Map<string, BlockGlow> = new Map();
  private readonly BASE_DURATION = 12; // 12 frames at 60 FPS = 200ms
  private readonly B2B_MULTIPLIER = 1.5; // Back-to-back glows last 50% longer
  ...
  addLineClearGlow(
    ...
    let pattern: GlowPattern = 'linear';
    let color = 'white';
   ...
   * @param cells - Array of {x, y} cell positions
   */
  addBackToBackGlow(cells: Array<{ x: number; y: number }>): void {
    if (!this.enabled) return;
   ...
   */
  private calculateRadialIntensity(glow: BlockGlow, x: number, y: number): number {
