...
export const DEVIL_GRADE_LADDER: readonly TGMGrade[] = [
  '1', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8', 'S9',
  'S10', 'S11', 'S12', 'S13',
];

/** The level a Devil run ends at (gamestart.c:11108). */
export const DEVIL_END_LEVEL = 1300;
 ...
 */
export const GOD_WINDOW_AT_1300 = { classic: 19200, world: 21000 } as const;
export const GOD_WINDOW_AFTER_ROLL = { classic: 18000, world: 19200 } as const;

/** isWRule(): the WORLD-family rotation systems get the longer window. */
export function isWorldRule(rotationSystem: RotationSystem): boolean {
 ...
 */
export function devilGradeForLevel(level: number): TGMGrade {
  const index = Math.floor(Math.max(0, level) / 100);
  return DEVIL_GRADE_LADDER[Math.min(index, DEVIL_GRADE_LADDER.length - 1)];
}
 ...
 */
export function devilFinalGrade(
  gametimeFrames: number,
  rotationSystem: RotationSystem
): TGMGrade {
  const window = isWorldRule(rotationSystem)
    ? GOD_WINDOW_AT_1300.world
    : GOD_WINDOW_AT_1300.classic;
