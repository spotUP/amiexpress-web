...
export class PanelMatch {
  ...
  private sourcesFor(targetIndex: number): MatchStack[] {
    const sources: MatchStack[] = [];
    this.garbageTargets.forEach((targets, sourceIndex) => {
      if (targets.includes(targetIndex)) sources.push(this.stacks[sourceIndex]);
    });
    return sources;
  }
   ...
   */
  private pushGarbageTo(targetIndex: number): void {
    ...
    for (const source of this.sourcesFor(targetIndex)) {
      ...
      if (delivery) this.receive(target, delivery);
    }
  }

  private receive(stack: MatchStack, garbage: Garbage[]): void {
  ...
  private isIrrecoverablyBehind(): boolean {
    for (let target = 0; target < this.stacks.length; target++) {
      for (const source of this.sourcesFor(target)) {
        if (source.clock + MAX_LAG < this.stacks[target].clock) return true;
      }
    }
    return false;
  }
  ...
  hasEnded(): boolean {
    ...
    if (this.timeLimit !== null && this.clock >= this.timeLimit) return true;
