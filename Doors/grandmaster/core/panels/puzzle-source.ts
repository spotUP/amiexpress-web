...
import { shakeFramesForGarbageSize } from './consts';
...
export class PuzzleSource implements PanelSource {
  /** The authored board, top row first. Consumed once, at startup. */
  private puzzleString: string;
  /** Panels the player raises into view after the board is exhausted. */
  private panelBuffer: string;
  /** Colours cleared garbage turns into. */
  private garbagePanelBuffer: string;
  /** Panels already built and not yet handed to the stack, TOP row first. */
  private pending: PanelSpec[] = [];
  private panelGenCount = 0;

  constructor(puzzleString: string, panelBuffer = '', garbageBuffer = '') {
    ...
    if (remainder !== 0) this.panelBuffer += '9'.repeat(6 - remainder);
  }
  ...
  nextRowPanels(stack: PanelSourceStack): PanelSpec[] {
    const puzzleStack = stack as PuzzleSourceStack;
  ...
  nextRowColors(stack: PanelSourceStack, metalPanelsQueued: number): {
    colors: number[]; metalPanelsQueued: number;
  } {
  ...
  private takeBufferRow(width: number): string {
...
export function buildPanels(
  puzzleString: string,
  stack: Pick<PuzzleSourceStack, 'width' | 'height' | 'nextGarbageId'>,
): PanelSpec[] {
  const width = stack.width;
  const stripped = puzzleString.replace(/\s/g, '');
  const rows: PanelSpec[][] = [];

  let garbageStartRow: number | null = null;
  let garbageStartColumn = 0;
  let isMetal = false;
  let connected: PanelSpec[] = [];
  let garbageId = 0;
  ...
  for (let row = 1; row <= rowCount; row++) {
    ...
    for (let column = width; column >= 1; column--) {
      ...
      if (CLOSES.has(char)) {
        ...
        const shakeTime = shakeFramesForGarbageSize(blockWidth, height);
        for (const member of connected) {
          ...
          if (row > stack.height) member.shakeTime = shakeTime;
        }
