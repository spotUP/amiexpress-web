 ...
 */
export function toSafePrecision(value: number): number {
  if (Math.floor(value) === value) return value;
  return Number(value.toPrecision(14));
}

/** JsonSafePrecision.fractionToSafePrecision. */
export function fractionToSafePrecision(numerator: number, denominator: number): number {
  return toSafePrecision(numerator / denominator);
}

function modernLevel(
  startingSpeed: number,
  shockFrequency: number,
  shockCap: number,
  colors: number,
 ...
 */
const MODERN: readonly LevelData[] = [
 ...
 */
const CLASSIC: readonly LevelData[] = [
...
function difficultyIndex(difficulty: ClassicDifficulty): number {
  return typeof difficulty === 'number' ? difficulty : DIFFICULTY_INDEX[difficulty];
}

function clone(data: LevelData): LevelData {
  return {
    ...data,
    stop: { ...data.stop },
...
export function getModern(level: number): LevelData {
  const preset = MODERN[level - 1];
  if (!preset) throw new Error(`trying to load inexistent level preset ${level}`);
  return clone(preset);
}

/** A copy of the classic preset for `difficulty` (1-4 or easy/normal/hard/ex). */
export function getClassic(difficulty: ClassicDifficulty): LevelData {
  const preset = CLASSIC[difficultyIndex(difficulty) - 1];
  if (!preset) throw new Error(`trying to load inexistent difficulty preset ${difficulty}`);
  return clone(preset);
}
 ...
 */
export function getClassicEndless(difficulty: ClassicDifficulty): LevelData {
