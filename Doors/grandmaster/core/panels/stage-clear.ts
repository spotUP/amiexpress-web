...
export const STAGES_PER_ROUND = 5;
export const STAGE_COUNT = ROUNDS * STAGES_PER_ROUND;
...
export function buildStages(): Stage[] {
  ...
  for (let round = 1; round <= ROUNDS; round++) {
    for (let index = 1; index <= STAGES_PER_ROUND; index++) {
      stages.push({
    ...
    if (round === 3 || round === ROUNDS) {
      stages.push({
        number: stages.length + 1,
        round,
        index: STAGES_PER_ROUND + 1,
        label: round === ROUNDS ? 'FINAL' : 'SPECIAL',
        startingHeight: 4 + round,
        clearLine: 0,
        boss: true,
      });
...
export function stageSpeed(stage: Stage): number {
 ...
 */
export function stageBoardString(stage: Stage, rows: number): string {
  const levelData = getModern(STAGE_LEVEL);
  ...
  const source = new GeneratorSource(stageSeed(stage), false).clone(stackLike);

  source.panelBuffer = '';
  for (let i = 0; i < rows; i++) source.growPanelBuffer(stackLike);
 ...
 */
export function stageSeed(stage: Stage): number {
  return 1000 + stage.number * 7919;
}

/** Bowser's health, which combos and chains chip away at. */
export function bossHealth(stage: Stage): HealthSettings {
...
export function stageStackOptions(stage: Stage): StackOptions {
  const board = stageBoardString(stage, stage.startingHeight);
  const riseBuffer = stageBoardString(stage, RISE_BUFFER_ROWS);
  const levelData = { ...getModern(STAGE_LEVEL), startingSpeed: stageSpeed(stage) };

  return {
    levelData,
    panelSource: new PuzzleSource(board, riseBuffer),
    doCountdown: true,
  };
}
 ...
 */
export class StageClearGame {
  readonly stack: Stack;
  readonly stage: Stage;
  private outcome: StageOutcome = 'playing';
  ...
  constructor(stage: Stage) {
    ...
    this.stage = stage;
    this.stack = new Stack(stageStackOptions(stage));
  ...
  highestPanelRow(): number {
    for (let row = this.stack.height; row >= 1; row--) {
      for (let col = 1; col <= this.stack.width; col++) {
        if (this.stack.panels[row][col].color !== 0) return row;
      }
    }
    return 0;
  }
  ...
  hasCleared(): boolean {
    if (this.stack.hasActivePanels() || this.stack.swapQueued()) return false;
  ...
  run(): StageOutcome {
    ...
    if (this.stack.gameEnded()) this.outcome = 'lost';
    else if (this.hasCleared()) this.outcome = 'cleared';
    return this.outcome;
  }
