...
export const DEFAULT_VERSUS_GOAL = 200;

/** What a run has to reach for this win type, or null when there is no goal. */
export function versusGoalTarget(winType: VersusWinType, goal: number): number | null {
  if (winType === 'survival' || goal <= 0) return null;
 ...
 */
export function versusGoalReached(
  winType: VersusWinType,
  goal: number,
