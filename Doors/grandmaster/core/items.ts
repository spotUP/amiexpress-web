 ...
 */
export const TGM_NOT_IMPLEMENTED_ITEMS: readonly number[] = [4, 5, 16, 20];

/** DEATH BLOCK's duration in pieces (gamestart.c:7097 `item_t > 1`). */
export const DEATH_BLOCK_PIECES = 2;
/** ROLL ROLL's duration in pieces (gamestart.c:7092 `item_t > 3`). */
export const ROLL_ROLL_PIECES = 4;
...
export function applyEnemyItem(
  ...
  switch (itemId) {
    case 1:
      ...
      return {};
    case 2:
      // ROLL ROLL - the target's piece starts rotating on its own
      // (gamestart.c:13497-13499 isrollroll, spent by the caller's engine).
      return { rollRollPieces: ROLL_ROLL_PIECES };
    case 3:
