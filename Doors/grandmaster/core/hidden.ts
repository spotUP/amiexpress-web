...
export const SHADOW_TIMER_FRAMES = 300;

/** Frames subtracted per tick, by mode. 0 = HIDDEN is off. */
export function shadowDecayRate(mode: HiddenMode | undefined): number {
  switch (mode) {
    case 'SLOW': return 1;      // the reference's normal rate
    case 'FAST': return 2;      // UNDER M2
    case 'FASTEST': return 3;   // UNDER M3
    default: return 0;
  }
}
 ...
 */
export function countHiddenCells(board: { grid: { shadowFrames?: number }[][] }): number {
  let hidden = 0;
  for (const row of board.grid) {
    for (const cell of row) if (isCellHidden(cell)) hidden++;
  }
  return hidden;
}

/** True when the cell has a shadow timer that has run out. */
export function isCellHidden(cell: { shadowFrames?: number }): boolean {
  return cell.shadowFrames !== undefined && cell.shadowFrames <= 0;
}
