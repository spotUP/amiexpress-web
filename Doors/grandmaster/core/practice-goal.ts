...
export const PRACTICE_GOAL_VALUES: readonly number[] = [5, 10, 20, 30, 50, 75, 100, 130, 160, 200];
...
export function practiceGoalTarget(goal: PracticeGoal): number | null {
  if (goal.type === 'none' || goal.value <= 0) return null;
  // gamestart.c:11231 - the level goal is the list value times ten.
  return goal.type === 'level' ? goal.value * 10 : goal.value;
}

export function practiceGoalReached(goal: PracticeGoal, run: PracticeProgress): boolean {
