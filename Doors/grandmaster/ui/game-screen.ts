...
export class GameScreen {
  ...
  private setupInput(): void {
    ...
    on('rotate_180', () => {
      ...
      if (this.state.settings.rotationSystem === 'SRS-X') {
        ...
        if (this.engine.rotate(2)) this.sounds.playSfx('rotate');
        return;
      }
  ...
  private addHardDropTrail(): void {
    ...
    this.hardDropTrails.push(...buildHardDropTrail(
      shape, currentPiece.x, currentPiece.y, dropDistance, color,
      { minY: 4, maxY: 24 }, Date.now()
    ));
  }
  ...
  private applyGlow(baseChar: string, glowColor: string, intensity: number): string {
    ...
    const bright = brightColorFor(glowColor);
