  ...
  private setupUI(): void {
    ...
    if (!layout.border) {
      ...
      const rule = (left: string, right: string): string =>
        left + '\u2500'.repeat(Math.max(0, layout.board.width)) + right;
      const ruleRow = (row: number, content: string): void => {
      ...
      if (layout.board.top >= 1) ruleRow(layout.board.top - 1, rule('\u250c', '\u2510'));
      ...
      if (floor < this.screen.height) ruleRow(floor, rule('\u2514', '\u2518'));
