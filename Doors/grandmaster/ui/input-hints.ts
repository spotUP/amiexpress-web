...
export function formatTriggerName(trigger: string): string {
...
export function keyFor(action: string, keys: Partial<KeyConfig>): string | null {
  const MAP: Record<string, keyof KeyConfig> = {
    left: 'left',
    right: 'right',
    rotate_cw: 'rotateCW',
  ...
  const bound = keys[field] as string[] | undefined;
...
export function padFor(action: string, bindings: Partial<Record<string, string[]>>): string | null {
...
export function buildHintLine(
  entries: HintEntry[],
  source: InputSource,
  keys: Partial<KeyConfig>,
  padBindings: Partial<Record<string, string[]>>
): string {
  const parts: string[] = [];
