  ...
  async show(): Promise<MenuSelection> {
    ...
    return new Promise((resolve) => {
      ...
      const recentre = () => {
        const x = offsetX();
        const y = offsetY();
        const compactNow = isCompactWidth(this.screen.width);
        if (compactNow) {
          title.left = x; title.top = 1 + y;
          title.width = this.screen.width;
          menuPanel.left = x; menuPanel.top = 3 + y;
          menuPanel.width = this.screen.width;
          instructions.left = x; instructions.top = 3 + y + panelHeight;
        } else {
          title.left = 2 + x; title.top = 1 + y;
          menuPanel.left = 2 + x; menuPanel.top = 5 + y;
          if (descBox) { descBox.left = 2 + x + 26; descBox.top = 5 + y; }
          if (info) { info.left = 2 + x + 26 + 30; info.top = 5 + y; }
