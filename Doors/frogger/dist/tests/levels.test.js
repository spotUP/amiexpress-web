...
exports.everyRoadLaneRunsLeftToRight = everyRoadLaneRunsLeftToRight;
 ...
 */
async function everyRoadLaneRunsLeftToRight() {
    const { data } = (0, fixture_1.startedLevel)(1);
    for (const lane of data.lanes.filter(l => l.type === 'road')) {
        assert_1.default.strictEqual(lane.direction, 1, `road lane ${lane.lane} should run left to right`);
    }
}
/**
