...
import {
  CellBuffer, Sprite, createBuffer, blitCells, frameAt, flipCellsH, bufferToTags,
} from '@amiexpress/bbs-door-sdk/engines/graphics/cell-art';
import { FroggerData, RiverObject, Vehicle } from './types';
import {
  GRID_WIDTH, GAME_AREA_HEIGHT, CELL_WIDTH, CELL_HEIGHT,
...
function fill(buffer: CellBuffer, top: number, height: number, colour: number): void {
...
function blitAtLane(
  ...
  if (!sprite) return;
  const laneTop = LANE_ROWS[laneY];
  if (laneTop === undefined) return;
 ...
 */
function turtleAnimation(obj: RiverObject): string {
  if (obj.diveStage === 'down' || obj.isDiving) return 'under';
  if (obj.diveStage === 'sinking') return 'sinking';
  return 'up';
}
...
export function buildBoard(
  ...
  const board = createBuffer(GRID_WIDTH * CELL_WIDTH, GAME_AREA_HEIGHT);
  ...
  for (const lane of data.lanes) {
    ...
    const colour =
      lane.type === 'road' ? LANE_COLOUR.road :
  ...
  for (const home of data.homes) {
    const animation =
      ...
      'empty';
    blitAtLane(board, sheet['home'], animation, tick, 0, cellPos(home.x + HOME_CENTRE_OFFSET));
    // A fly sits in the opening until something takes it.
    if (home.hasFly && !home.occupied && !home.hasAlligator) {
      blitAtLane(board, sheet['fly'], 'idle', tick, 0, cellPos(home.x + HOME_CENTRE_OFFSET));
    }
  }
  ...
  for (const lane of data.lanes) {
    for (const raw of lane.objects) {
      ...
      const animation =
        obj.type === 'turtle' ? turtleAnimation(obj) :
  ...
  const frogSheet = (LANE_HEIGHTS[frog.y] ?? CELL_HEIGHT) < CELL_HEIGHT
 ...
 */
function frogAnimation(data: FroggerData): string {
  const frog = data.frog;
  if (!frog.isJumping) return 'idle';
 ...
 */
export function boardToLines(board: CellBuffer): string[] {
  return bufferToTags(board, { char: ' ', fg: 7, bg: LANE_COLOUR.water });
}
