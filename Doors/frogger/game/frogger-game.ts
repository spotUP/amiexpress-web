...
import {
  ...
  LANE4_FAST_MULTIPLIER,
  LANE4_SPEEDUP_AFTER_MS,
  RIVER_HURRY_AFTER_SECONDS,
  RIVER_HURRY_MULTIPLIER,
  FLY_SPAWN_INTERVAL_MS,
  FLY_DURATION_MS,
  HOME_CROCODILE_INTERVAL_MS,
  HOME_CROCODILE_DURATION_MS,
  LADY_FROG_INTERVAL_MS,
  OTTER_INTERVAL_MS,
...
export class FroggerGame {
  ...
  private clock(): number {
    return this.data.frameCount * GAME_TICK_MS;
  }
  ...
  initLevel(): void {
    ...
    for (const laneConfig of LANE_CONFIG) {
      ...
      const lane: Lane = {
        ...
        speed: isLane4Road && config.lane4Fast ? base * LANE4_FAST_MULTIPLIER : base,
      };
      d.lanes.push(lane);
    }
    ...
    d.furthestRow = GRID_HEIGHT - 1;

    this.populateRoad(config);
    this.populateWater(config);
    this.placeSnakes(config);

    d.flyTimer = FLY_SPAWN_INTERVAL_MS;
    d.alligatorTimer = HOME_CROCODILE_INTERVAL_MS;
    d.ladyFrogTimer = LADY_FROG_INTERVAL_MS;
    d.otterTimer = OTTER_INTERVAL_MS;
   ...
   */
  private populateRoad(config: ReturnType<typeof getLevelConfig>): void {
    const d = this.data;
    ...
    for (const lane of d.lanes) {
      ...
      const width = OBJECT_WIDTHS[type];
      const spacing = GRID_WIDTH / Math.max(1, count);
  ...
  private vehicleForLane(lane: number): VehicleType {
    if (lane === 4) return 'racecar';
   ...
   */
  private populateWater(config: ReturnType<typeof getLevelConfig>): void {
    const d = this.data;
    ...
    for (const lane of d.lanes) {
      ...
      switch (lane.lane) {
        ...
        case 4:
          ...
          break;
        case 2:
          this.fillLogLane(lane, config.shortLogs, OBJECT_WIDTHS.shortLog);
          break;
        case 3:
          this.fillLogLane(lane, config.longLogs, OBJECT_WIDTHS.longLog);
          break;
        case 5:
   ...
   */
  private fillTurtleLane(lane: Lane, count: number): void {
    const d = this.data;
    const width = OBJECT_WIDTHS.turtle;
    const spacing = GRID_WIDTH / Math.max(1, count);
  ...
  private fillLogLane(lane: Lane, count: number, width: number): void {
    const d = this.data;
    const spacing = GRID_WIDTH / Math.max(1, count);
   ...
   */
  private fillLaneFive(lane: Lane, config: ReturnType<typeof getLevelConfig>): void {
    const d = this.data;

    if (config.lane5Crocodile) {
      const spacing = GRID_WIDTH / LANE5_CROCODILE_COUNT;
      for (let i = 0; i < LANE5_CROCODILE_COUNT; i++) {
        lane.objects.push(this.makeCrocodile(lane, i * spacing));
      }
      return;
    }

    const spacing = GRID_WIDTH / Math.max(1, config.mediumLogs);
    for (let i = 0; i < config.mediumLogs; i++) {
      const isCrocodile =
        config.crocEveryNth !== null && (i + 1) % config.crocEveryNth === 0;
  ...
  private makeCrocodile(lane: Lane, x: number): RiverObject {
    return {
   ...
   */
  private placeSnakes(config: ReturnType<typeof getLevelConfig>): void {
    const d = this.data;
    const median = d.lanes.find(l => l.type === 'safe' && l.y < GRID_HEIGHT - 1);
    ...
    for (let i = 0; i < config.snakes; i++) {
      ...
      if (onLog) {
        const log = logLane!.objects[
          Math.floor(Math.random() * logLane!.objects.length)
        ] as RiverObject;
  ...
  riverSpeedScale(): number {
    const elapsed = INITIAL_TIME - this.data.timeRemaining;
    return elapsed >= RIVER_HURRY_AFTER_SECONDS ? RIVER_HURRY_MULTIPLIER : 1;
  }
  ...
  lane4SpeedScale(): number {
    ...
    if (config.lane4Fast) return 1;

    const elapsed = Date.now() - this.data.frogStartTime;
    return elapsed >= LANE4_SPEEDUP_AFTER_MS ? LANE4_FAST_MULTIPLIER : 1;
  }
  ...
  handleDirection(direction: Direction): void {
    ...
    if (newY < d.furthestRow) {
      d.furthestRow = newY;
      if (d.hopPointsThisHome < SCORES.maxHopPerHome) {
        d.score += SCORES.hop;
        d.hopPointsThisHome += SCORES.hop;
      }
    }
  ...
  update(): void {
    ...
    if (d.frog.isDead) {
      d.frog.deathFrame++;
      if (d.frog.deathFrame >= 20) this.respawnFrog();
      this.render();
      return;
    }
    ...
    if (d.frog.isDead) { this.render(); return; }
  ...
  private awardExtraLife(): void {
    const d = this.data;
    if (d.extraLifeAwarded || d.score < EXTRA_LIFE_SCORE) return;
  ...
  private updateObjects(): void {
    ...
    for (const lane of this.data.lanes) {
      ...
      for (const obj of lane.objects) {
        ...
        if (obj.speed > 0 && obj.x > GRID_WIDTH) {
          obj.x = -width;
        } else if (obj.speed < 0 && obj.x + width < 0) {
          obj.x = GRID_WIDTH;
        }

        this.updateTurtle(obj as RiverObject);
      }
    }
  }
  ...
  private updateSnakes(): void {
    for (const snake of this.data.snakes) {
      snake.x += snake.speed * snake.direction * (GAME_TICK_MS / 1000);

      if (snake.x <= 0) { snake.x = 0; snake.direction = 1; }
      if (snake.x >= GRID_WIDTH - 1) { snake.x = GRID_WIDTH - 1; snake.direction = -1; }
    }
  }
  ...
  private updateVisitors(): void {
    ...
    if (config.crocInHome && now >= d.alligatorTimer) {
      d.alligatorTimer = now + HOME_CROCODILE_INTERVAL_MS;
      const free = d.homes.filter(h => !h.occupied && !h.hasFly && !h.hasAlligator);
      if (free.length > 0) {
        const home = free[Math.floor(Math.random() * free.length)];
        home.hasAlligator = true;
        home.visitorUntil = now + HOME_CROCODILE_DURATION_MS;
      }
    }

    // FAQ 7: the lady frog rides a log in water lane 2.
    if (now >= d.ladyFrogTimer) {
      d.ladyFrogTimer = now + LADY_FROG_INTERVAL_MS;
      const lane = d.lanes.find(l => l.type === 'water' && l.lane === 2);
      const logs = (lane?.objects ?? []) as RiverObject[];
    ...
    if (now >= d.otterTimer) {
      d.otterTimer = now + OTTER_INTERVAL_MS;
      const water = d.lanes.filter(l => l.type === 'water');
      const lane = water[Math.floor(Math.random() * water.length)];

      if (lane && !(lane.objects as RiverObject[]).some(o => o.type === 'otter')) {
        lane.objects.push({
          id: d.riverObjectIdCounter++,
          type: 'otter',
          x: lane.direction > 0 ? -OBJECT_WIDTHS.otter : GRID_WIDTH,
          y: lane.y,
          lane: lane.lane,
          width: OBJECT_WIDTHS.otter,
          speed: lane.speed * lane.direction,
          mouthWidth: 1,
          snakeAt: null,
          ladyFrogAt: null,
   ...
   */
  private updateFrogOnObject(): void {
    const d = this.data;
    const frog = d.frog;
  ...
  checkCollisions(): void {
    ...
    if (lane.type === 'road') {
      for (const obj of lane.objects) {
        if (this.overlaps(frog.x, obj.x, (obj as Vehicle).width)) {
          this.killFrog('car');
          return;
        }
      }
    ...
    for (const raw of lane.objects) {
      ...
      if (!this.overlaps(frog.x, obj.x, obj.width)) continue;

      // A turtle that has gone under is not footing.
      if (obj.type === 'turtle' && obj.isDiving) continue;
    ...
    if (!frog.isJumping) this.killFrog('water');
  }

  /** Is the frog on the leading, business end of a crocodile or otter? */
  private inMouth(frogX: number, obj: RiverObject): boolean {
  ...
  private checkMedianSnake(lane: Lane): void {
    const frog = this.data.frog;

    for (const snake of this.data.snakes) {
      if (snake.y !== lane.y) continue;
  ...
  checkHomeArrival(): void {
    const d = this.data;
    if (d.frog.y !== 0) return;

    const index = d.homes.findIndex(
      home => Math.round(d.frog.x) === home.x + HOME_CENTRE_OFFSET
    );
   ...
   * Kill the frog
   */
  private killFrog(deathType: 'car' | 'water' | 'timeout' | 'edge' | 'snake' | 'crocodile'): void {
    const d = this.data;
    if (d.frog.isDead) return;
