...
export class FroggerGame {
  ...
  private lastSpawnTime: Map<number, number> = new Map();
  ...
  initLevel(): void {
    ...
    if (this.data.homes.length === 0 || this.data.homesCompleted === 5) {
      this.data.homes = HOME_POSITIONS.map(x => ({
        x,
        occupied: false,
        hasFly: false,
        hasAlligator: false,
   ...
   */
  private spawnRiverObject(laneIndex: number): void {
    const lane = this.data.lanes[laneIndex];
    if (lane.type !== 'water') return;

    // Alternate between logs and turtles
    const isEvenLane = laneIndex % 2 === 0;
    const type: RiverObjectType = isEvenLane ? 'log' : 'turtle';
    const width = OBJECT_WIDTHS[type];

    // Random starting position
    const x = Math.floor(Math.random() * (GRID_WIDTH - width));

    const obj: RiverObject = {
  ...
  handleDirection(direction: Direction): void {
    if (this.data.frog.isDead || this.data.frog.isJumping) return;
    ...
    switch (direction) {
      case 'up':
        ...
        break;
      case 'down':
        newY = Math.min(GRID_HEIGHT - 1, frog.y + 1);
        break;
      case 'left':
        newX = Math.max(0, frog.x - 1);
        break;
      case 'right':
        newX = Math.min(GRID_WIDTH - 1, frog.x + 1);
        break;
    }
    ...
    for (const lane of this.data.lanes) {
      for (const obj of lane.objects) {
        ...
        if ('isDiving' in obj && obj.type === 'turtle') {
          const riverObj = obj as RiverObject;
  ...
  private updateFrogOnObject(): void {
    ...
    if (frog.onObject) {
      ...
      frog.x += obj.speed * (GAME_TICK_MS / 1000);
  ...
  private checkCollisions(): void {
    ...
    if (lane.type === 'road') {
      ...
      for (const obj of lane.objects) {
        if (this.isColliding(frog.x, obj.x, (obj as Vehicle).width)) {
          this.killFrog('car');
          return;
        }
      }
   ...
   */
  private isColliding(frogX: number, objX: number, objWidth: number): boolean {
  ...
  private checkHomeArrival(): void {
    ...
    for (const home of this.data.homes) {
      if (!home.occupied && Math.abs(frog.x - home.x) <= 2) {
        ...
        this.data.score += SCORES.home;

        // Bonus for fly
        if (home.hasFly) {
          this.data.score += SCORES.fly;
          home.hasFly = false;
        }

        // Time bonus
        this.data.score += Math.floor(this.data.timeRemaining) * SCORES.timeBonus;

        // Check if level complete
        if (this.data.homesCompleted >= 5) {
          this.data.score += SCORES.levelComplete;
          this.data.level++;
   ...
   * Kill the frog
   */
  private killFrog(deathType: 'car' | 'water' | 'timeout' | 'edge'): void {
  ...
  private resetFrogPosition(): void {
    this.data.frog.x = Math.floor(GRID_WIDTH / 2);
    this.data.frog.y = GRID_HEIGHT - 1;
        ...
        if ('type' in obj) {
          switch (obj.type) {
            ...
            case 'log':
              ...
              break;
            case 'turtle':
              char = (obj as RiverObject).isDiving ? ' ' : 'o';
              break;
          }
        }
