...
import { getToolHandler, insertTextChar, shiftHalfBlock, } from './drawing.js';
import { ToolSelectorModal, ColorPickerModal, FileDialogModal, ConfirmDialog, MessageDialog, HelpDialog, } from './modals.js';
import { loadFile, saveFile, listFiles, } from './file-ops.js';
// =============================================================================
// ANSI EDITOR DOOR
// =============================================================================
const door = new Door({
...
class ANSIEditor {
    ...
    async init() {
        ...
        this.showWelcome().then(() => {
            ...
            if (this.ctx.bbs?.enableMouseEvents) {
                this.ctx.bbs.enableMouseEvents();
            }
        });
        // Handle mouse events from socket
        this.ctx.socket.on('mouse', (event) => {
    ...
    async showColorPicker() {
        const modal = new ColorPickerModal(this.ui, this.state.currentFg, this.state.currentBg, this.state.iceColorsEnabled);
        const result = await modal.show();
        if (result) {
    ...
    async newFile() {
        if (this.state.modified) {
            const modal = new ConfirmDialog(this.ui, 'New File', 'Current file has unsaved changes. Continue?');
            if (!(await modal.show())) {
                this.refresh();
                return;
            }
    ...
    async openFile() {
        ...
        if (filename) {
            try {
                ...
                await this.showMessage(`Error: ${error.message}`, 'error');
            }
        }
        this.refresh();
    }
    ...
    // ===========================================================================
    async showWelcome() {
        const modal = new MessageDialog(this.ui, 'ANSI Editor', 'State-of-the-art ANSI/ASCII art editor\n\nPress F1 for help\nPress Tab for tools', 'info');
        await modal.show();
        this.refresh();
    }
    async showHelp() {
    ...
    toggleGuides() {
        const guides = ['none', '80x25', '80x40', '44x22', 'grid'];
    ...
    async quit() {
        if (this.state.modified) {
            const modal = new ConfirmDialog(this.ui, 'Quit', 'File has unsaved changes. Quit anyway?');
            if (!(await modal.show())) {
                this.refresh();
                return;
            }
