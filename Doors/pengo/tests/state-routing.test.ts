...
function switchBody(src: string): string {
  ...
  assert.ok(start >= 0, 'handleInput must still route on gameData.state');
 ...
 */
export async function levelCompleteIgnoresInputInsteadOfQuittingToMenu(): Promise<void> {
  const body = switchBody(indexSource());

  const caseMatch = body.match(
    /case "dying":\s*\n\s*case "levelComplete":\s*\n([\s\S]*?)\n\s*(?:case |default)/
  );
  assert.ok(caseMatch, '"dying" and "levelComplete" must be handled as one explicit case group');

  const caseBody = stripLineComments(caseMatch![1]);
  assert.ok(
    !/showMenu\(\)/.test(caseBody),
    'a keypress during levelComplete (or dying) must not call showMenu() - that quits to the ' +
    'main menu mid-handover, which is the reported bug'
  );
  assert.ok(
    !/gameData\.state\s*=/.test(caseBody),
    'input routing for these states must not itself change gameData.state - only the timed ' +
    'hand-over in PengoGame.update() may do that'
  );
  assert.ok(/break;/.test(caseBody), 'the case must terminate, not fall through further');
}

/** Same guarantee, spelled out for the death animation on its own. */
export async function dyingIgnoresInputInsteadOfQuittingToMenu(): Promise<void> {
  const body = switchBody(indexSource());
  assert.ok(
    /case "dying":\s*\n\s*case "levelComplete":/.test(body),
    '"dying" must route through the same input-ignoring case as "levelComplete"'
  );
}
 ...
 */
export async function theDefaultCaseIsExhaustivenessNotAMenuBailout(): Promise<void> {
  const body = switchBody(indexSource());

  assert.ok(
    /default:\s*\{[\s\S]*const _exhaustive: never = gameData\.state;/.test(body),
    'the default case must assign gameData.state to a `never`-typed const, so adding a new ' +
    'GameState member without a case above fails the typecheck'
  );

  const defaultMatch = body.match(/default:\s*\{([\s\S]*)\}\s*$/);
  assert.ok(defaultMatch, 'the default case must be present');
  assert.ok(
    !/showMenu\(\)/.test(stripLineComments(defaultMatch![1])),
    'the default case must not call showMenu() - an unreachable/unknown state must be a safe ' +
 ...
 */
export async function everyOtherRouteIsUnchanged(): Promise<void> {
  const body = switchBody(indexSource());

  assert.ok(/case "menu":\s*\n\s*handleMenuInput\(inputKey\);/.test(body), 'menu -> handleMenuInput');
  assert.ok(
    /case "highscores":\s*\n\s*case "help":\s*\n\s*showMenu\(\);/.test(body),
    'highscores/help -> showMenu (any key returns to the menu from these screens)'
  );
  assert.ok(/case "playing":\s*\n\s*handleGameInput\(inputKey\);/.test(body), 'playing -> handleGameInput');
  assert.ok(/case "paused":\s*\n\s*handlePausedInput\(inputKey\);/.test(body), 'paused -> handlePausedInput');
  assert.ok(/case "gameover":\s*\n\s*handleGameOverInput\(inputKey\);/.test(body), 'gameover -> handleGameOverInput');
  assert.ok(
    /case "enterName":\s*\n\s*handleNameEntryInput\(inputKey\);/.test(body),
    'enterName -> handleNameEntryInput'
  );
}

/** GameState must be imported so the exhaustiveness check can reference it. */
export async function gameStateIsImportedForTheExhaustivenessCheck(): Promise<void> {
  const src = indexSource();
  assert.ok(
    /import\s*\{[^}]*\bGameState\b[^}]*\}\s*from\s*"\.\/game\/types"/.test(src),
    'the never-check needs GameState in scope (or the assignment type-checks vacuously)'
  );
}
