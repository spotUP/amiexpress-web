...
function createInitialGameData(): PengoData {
  return {
    ...
    pengo: { x: 7, y: 6, direction: 'up', isPushing: false, pushFrame: 0, isDead: false, deathFrame: 0 },
    enemies: [],
    grid: [],
    eggs: [],
...
const door = new Door({ name: 'Pengo', version: '1.0.0', author: 'AmiExpress BBS' });
...
function initScreen(): void {
  screen = blessed.screen({
    smartCSR: true, dockBorders: true, title: 'Pengo', fullUnicode: false,
  ...
  gameArea = blessed.box({ parent: screen, top: 1, left: 0, width: GRID_WIDTH * 2, height: GRID_HEIGHT + 2, tags: true, style: { bg: 'black' } });
...
function showHelp(): void {
  const content = [
    ...
    '{cyan-fg}OBJECTIVE:{/}', 'Crush all Sno-Bees with ice blocks!', '',
    '{green-fg}CONTROLS:{/}', 'Arrow Keys - Move Pengo', 'Space - Push/Slide ice block', '',
    '{magenta-fg}TIPS:{/}', 'Push blocks against wall to stun enemies', 'Line up diamond blocks for bonus!', '',
...
door.onError((error: Error) => { console.error('[Pengo] Error:', error); cleanup(); });
