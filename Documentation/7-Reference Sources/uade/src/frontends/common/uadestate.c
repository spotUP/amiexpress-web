				       ...
				       struct uade_state *state)
{
	...
	char moduledir[PATH_MAX];
	char *moddir = NULL;
	if (state->song.info.modulefname[0]) {
		moddir = uade_dirname(moduledir, state->song.info.modulefname, sizeof moduledir);
		if (!moddir)
			uade_warning("Can not get moduledir for %s\n", state->song.info.modulefname);
	}

	if (uade_find_amiga_file(fname, sizeof fname, name, playerdir, moddir))
		return NULL;
...
{
// disable warnings from scan-build since as per UADE API it's developers responsibility to free the memory to avoid leak from strdup
...
{
#ifndef __clang_analyzer__ // see above
...
struct uade_state *uade_new_state(const struct uade_config *extraconfig)
{
	...
	int write_audio_fds[2] = {state->config.write_audio_fd, -1};
	int *keep_fds = (write_audio_fds[0] >= 0) ? write_audio_fds : NULL;
