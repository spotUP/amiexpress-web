#!/usr/bin/env node
/**
 * Test script for GA (GetAnswer) door
 * Properly handles BBS login and executes GA command
 */
...
async function testGACommand() {
  console.log('🧪 Testing GA Command\n');
  ...
  console.log('   5. Execute GA command');
  ...
  try {
    ...
    console.log('[WAIT] Waiting for door execution (15 seconds)...');
    console.log('[INFO] Monitor backend logs in another terminal:');
    console.log('   tail -f /tmp/backend.log | grep -E "GA|GetAnswer|🔄|LOOP|XIM"\n');
...
if (require.main === module) {
  testGACommand().catch(console.error);
}

module.exports = { testGACommand };
